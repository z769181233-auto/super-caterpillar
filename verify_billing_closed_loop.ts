...
async function run() {
    ...
    const initialCredits = (await prisma.organization.findUnique({ where: { id: orgId } }))?.credits || 0;
    ...
    const signature = createHmac('sha256', API_SECRET).update(API_KEY + nonce + timestamp + body).digest('hex');
    ...
    const finalCredits = (await prisma.organization.findUnique({ where: { id: orgId } }))?.credits || 0;
