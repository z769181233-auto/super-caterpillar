  ...
  try {
    ...
    for (let i = 0; i < CONFIG.JOBS_PER_CLIENT; i++) {
      const sigHeaders = signRequest(
        'POST',
        `/api/projects/${project.id}/novel/analyze`,
        {},
        key,
        secret
      );
      ...
      await axios.post(`${API_URL}/api/projects/${project.id}/novel/analyze`, {}, { headers });
      stats.created++;
    }
  } catch (e: any) {
...
async function runWorker(workerIndex: number) {
  ...
  try {
    ...
    // Complete Job
    const completeHeaders = signRequest(
      'POST',
    ...
    await axios.post(
      `${API_URL}/api/jobs/${job.id}/report`,
      { status: 'SUCCEEDED' },
...
async function main() {
  ...
  // Start workers
  const workers = Array.from({ length: CONFIG.WORKER_THREADS }, (_, i) =>
    (async () => {
      while (keepRunning) {
        await runWorker(i);
