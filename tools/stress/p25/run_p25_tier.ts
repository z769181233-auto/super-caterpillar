...
function generateHmacHeaders(
  ...
  console.log(
    `[DEBUG] Generated Headers: Nonce=${headers['X-Nonce']}, Timestamp=${headers['X-Timestamp']}`
  );
  return headers;
}
...
async function main() {
  ...
  console.log(
    `[P25 Runner] Rules: Org=${orgId}, ApiKey=${apiKey.substring(0, 6)}... (Business Route Fixed)`
  );
  ...
  const projectHeaders = generateHmacHeaders(
    apiKey,
    apiSecret,
    'POST',
    projectPath,
  ...
  const projectRes = await axios.post(`${apiBase}/projects`, projectBody, {
  ...
  const importHeaders = generateHmacHeaders(
    apiKey,
    apiSecret,
    'POST',
    importPath,
  ...
  const importRes = await axios.post(`${apiBase}/projects/${projectId}/novel/import`, importBody, {
    headers: importHeaders,
  });

  console.log(
    `[P25 Runner] Import Success. Task: ${importRes.data.data.taskId}. Status: ${importRes.data.message}`
  );
