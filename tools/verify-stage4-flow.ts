...
async function pollForCondition<T>(
    ...
    for (let i = 0; i < maxAttempts; i++) {
        try {
            ...
            lastError = e;
            console.log(`   ...transient error or still processing (attempt ${i + 1}/${maxAttempts}): ${e}`);
        }
...
async function verifyStage4() {
    ...
    try {
        ...
        const sceneRes = await fetchJson<Scene>(`${API_URL}/api/projects/episodes/${episodeId}/scenes`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                title: 'E2E Scene',
                index: 1,
                summary: 'The quick brown fox jumps over the lazy dog. This is a testing summary for semantic analysis.'
            })
        });
