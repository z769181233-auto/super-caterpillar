...
async function runVerification() {
    ...
    for (const tc of testCases) {
        ...
        console.log(`[Algorithm] ${tc.name}: Score=${output.visual_density_score.toFixed(2)} Engine=${output.audit_trail.engine_version}`);
    }
    ...
    if (apiKey) {
        ...
        for (const tc of testCases) {
            try {
                const output = await ce03RealEngine({ structured_text: tc.text });
                console.log(`[AI] ${tc.name}: Score=${output.visual_density_score.toFixed(2)} Engine=${output.audit_trail.engine_version}`);
