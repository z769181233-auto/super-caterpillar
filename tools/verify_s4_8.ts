    ...
    try {
        ...
        for (const shotId of shotIds) {
            ...
            for (let i = 0; i < 48; i++) {
                ...
                spawnSync('ffmpeg', [
                    '-f', 'lavfi', '-i', `color=c=${i % 2 === 0 ? 'green' : 'black'}:s=1280x720`,
                    '-frames:v', '1', '-y', fpath
                ]);
        ...
        const ffprobe = spawnSync('ffprobe', [
            '-v', 'error', '-show_entries', 'format=duration:stream=codec_name,pix_fmt,r_frame_rate,channels',
            '-of', 'json', absVideoPath
        ]);
        ...
        if (Math.abs(duration - 5.0) > 0.1) throw new Error(`Duration Mismatch! Expected ~5.0s, found ${duration}s`);
        ...
        console.log(`Found ${allAssets.length} assets for this shot:`, allAssets.map(a => a.storageKey));
