 ...
 * Output: *.render_plan.json + Evidence Package (Resolution, Continuity, Cost)
 */

const FPS = 24;
const assetsDir = path.join(__dirname, '../../docs/assets');
const COST_MODEL_SSOT_VERSION = '1.0.0'; // Must match docs/COST_MODEL_SSOT.md
const BASE_FRAME_CU = 1.0;
...
function getTemplateMultiplier(templateId) {
  if (!templateId) return 1.0;
  if (templateId.startsWith('STUB_')) return 0.1;
  if (templateId.includes('_HEAVY')) return 2.0;
  if (templateId.includes('_LITE')) return 0.5;
  return 1.0;
}
...
function compile(shotSpecPath, outputPath, eviDir) {
  ...
  shotSpec.beats.forEach((beat) => {
    ...
    shots.forEach((shot, idx) => {
      ...
      if (shot.dialogue && (!comboEntry || !comboEntry.templateId)) {
        console.error(
          `[ERROR] Render resolution failed for shot ${shot.id}. Combo ${comboKey} not found.`
        );
        process.exit(1);
      }
      ...
      // --- Cost Calculation (G3) ---
      const multiplier = getTemplateMultiplier(templateId);
      const shotCost = durationFrames * multiplier * BASE_FRAME_CU;
      totalCostUnits += shotCost;

      // Update Breakdown
      if (!templateBreakdown[templateId]) {
        templateBreakdown[templateId] = { count: 0, frames: 0, costUnits: 0 };
      }
      templateBreakdown[templateId].count++;
      templateBreakdown[templateId].frames += durationFrames;
      templateBreakdown[templateId].costUnits += shotCost;
  ...
  if (eviDir) {
    ...
    fs.writeFileSync(
      path.join(eviDir, 'shot_sha256.txt'),
      `${shotSpecSha}  ${path.basename(shotSpecPath)}`
    );
    fs.writeFileSync(
      path.join(eviDir, 'render_plan_sha256.txt'),
      `${renderPlanSha}  ${path.basename(outputPath)}`
    );
    ...
    if (
      ...
      continuityLog[continuityLog.length - 1].endFrame !== currentFrame
    ) {
      continuityReport.continuityVerified = false;
    ...
    fs.writeFileSync(
      path.join(eviDir, 'frame_continuity_report.json'),
      JSON.stringify(continuityReport, null, 2)
    );

    // Resolve Report
    const stubCount = renderShots.filter((s) => s.templateId.includes('STUB')).length;
    const resolveReport = {
    ...
    fs.writeFileSync(
      path.join(eviDir, 'resolve_report.json'),
      JSON.stringify(resolveReport, null, 2)
    );
    ...
    const costEstimate = {
      ...
      runKey: crypto
        .createHash('sha256')
        .update(episodeId + renderPlan.renderContractVersion + RENDER_MAP_SHA + shotSpecSha)
        .digest('hex'),
      renderContractVersion: renderPlan.renderContractVersion,
      renderMapSha256: RENDER_MAP_SHA,
      planSha256: renderPlanSha,
      totalFrames: currentFrame,
      durationSec: currentFrame / FPS,
      totalCostUnits: parseFloat(totalCostUnits.toFixed(2)),
      costModelVersion: COST_MODEL_SSOT_VERSION,
      breakdownByTemplateId: templateBreakdown,
    };
    const costPath = path.join(eviDir, 'cost_estimate.json');
    fs.writeFileSync(costPath, JSON.stringify(costEstimate, null, 2));
    const costSha = crypto.createHash('sha256').update(fs.readFileSync(costPath)).digest('hex');
    fs.writeFileSync(path.join(eviDir, 'cost_estimate_sha256.txt'), costSha);

    console.log(`Evidence package generated in ${eviDir} (Including Cost Estimate)`);
  }
...
if (input && output) {
  ...
  console.log(
    'Usage: node shot_to_render_plan.js <input.shot.json> <output.render_plan.json> [evidence_dir]'
  );
}
