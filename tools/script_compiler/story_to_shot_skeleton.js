...
function compile(storyPath) {
    ...
    beats.push(createBeat("beat_0", "常", baseDuration, "引入冲突: " + story.goal, "物件匹配", false, null, ["潜入", "观察", "开启"]));

    // Beat 1-3: Obstacles
    story.obstacles.slice(0, 3).forEach((obs, i) => {
        beats.push(createBeat(`beat_${i + 1}`, "快", baseDuration, "对抗阻碍: " + obs, "动作匹配", false, (i === 2 ? "Small" : null), ["冲刺", "躲避", "射击", "反击"]));
    });

    // Beat 4-6: Turns
    story.turns.forEach((turn, i) => {
        beats.push(createBeat(`beat_${i + 4}`, i % 2 === 0 ? "快" : "慢", baseDuration, "剧情转折: " + turn, "光影匹配", true, (i === 1 ? "Big" : "Small"), ["翻转", "对峙", "博弈"]));
    });

    // Beat 7: Cliffhanger
    beats.push(createBeat("beat_7", "慢", totalDuration - (baseDuration * (beatCount - 1)), "留下悬念: " + story.cliffhanger, "声音匹配", false, "Small", ["告退", "消失", "回响"]));
