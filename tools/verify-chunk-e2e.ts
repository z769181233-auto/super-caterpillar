...
import {
  ...
  serializeProgress,
  deserializeProgress,
} from '../apps/workers/src/processors/chunk-processor';
...
async function main() {
  ...
  const processedChunks = await mockProcessor.processChunks(chunks, progress, (currentProgress) => {
  ...
  const resumedChunks = await mockProcessor.processChunks(
    ...
    (currentProgress) => {
      if (currentProgress.processedChunks % 5 === 0 && currentProgress.processedChunks > 10) {
        console.log(
          `   恢复进度: ${currentProgress.processedChunks}/${currentProgress.totalChunks}`
        );
      }
    }
  );
