...
function createDummyPng(name: string, color: string): string {
  ...
  let base64 =
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=='; // Default Red
  if (color === 'blue')
    base64 =
...
async function main() {
  ...
  const user = await prisma.user.create({
    data: { email: `scene_runner_${suffix}@example.com`, passwordHash: 'x' },
  });
  const org = await prisma.organization.create({
    data: { name: `SceneOrg_${suffix}`, ownerId: user.id },
  });
  const project = await prisma.project.create({
    data: { name: `SceneProj_${suffix}`, organizationId: org.id, ownerId: user.id },
  });
