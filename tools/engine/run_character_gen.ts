...
async function main() {
  ...
  const user = await prisma.user.create({
    data: { email: `char_runner_${suffix}@example.com`, passwordHash: 'x' },
  });
  const org = await prisma.organization.create({
    data: { name: `CharOrg_${suffix}`, ownerId: user.id },
  });
  const project = await prisma.project.create({
    data: { name: `CharProj_${suffix}`, organizationId: org.id, ownerId: user.id },
  });
  ...
  try {
    ...
    if (res1.status !== 'SUCCESS') {
      console.error('FAIL: Run 1 failed');
      process.exit(1);
    }
    if (!res1.output.url?.startsWith('file://')) {
      console.error('FAIL: Run 1 missing file:// URL');
      process.exit(1);
    }
    ...
    if (!res1.output.url.endsWith('.png')) {
      console.error('FAIL: Output not png');
      process.exit(1);
    }
    ...
    if (res1.output.url !== res2.output.url) {
    ...
    if (res3.status !== 'FAILED') {
      console.error('FAIL: Run 3 should fail');
      process.exit(1);
    }
    if ((res3 as any).error?.code !== 'CHAR_NO_KEY') {
      console.error('FAIL: Run 3 expected CHAR_NO_KEY');
      process.exit(1);
    }
