...
import { VG05VFXCompositorAdapter } from '../../apps/api/src/engines/adapters/vg05_vfx_compositor.adapter';

async function main() {
    console.log("Initializing VG05 Runner...");
    ...
    const adapter = new VG05VFXCompositorAdapter(redis, audit, cost);

    const suffix = Math.random().toString(36).substring(7);
    const projectId = `p3_test_vg05_${suffix}`;
    const jobId = `job_vg05_${suffix}`;
    const userId = 'system';
    const orgId = 'org1';
    ...
    await (prisma as any).project.create({ data: { id: projectId, name: 'VG05 Test', ownerId: userId, organizationId: orgId } });
    ...
    try {
        ...
        console.log("✅ VG05 Verified");
        process.exit(0);
    } catch (e) {
        console.error("❌ VG05 Failed", e);
        process.exit(1);
    }
}
main();
