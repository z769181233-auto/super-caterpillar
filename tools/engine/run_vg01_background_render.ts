...
import { VG01BackgroundRenderAdapter } from '../../apps/api/src/engines/adapters/vg01_background_render.adapter';

async function main() {
    console.log("Initializing VG01 Runner...");
    ...
    const adapter = new VG01BackgroundRenderAdapter(redis, audit, cost);

    const suffix = Math.random().toString(36).substring(7);
    const projectId = `p3_test_vg01_${suffix}`;
    const jobId = `job_vg01_${suffix}`;
    const userId = 'system';
    const orgId = 'org1';
    ...
    await (prisma as any).project.create({ data: { id: projectId, name: 'VG01 Test', ownerId: userId, organizationId: orgId } });
    ...
    try {
        const res = await adapter.invoke({
            jobType: 'VG_RENDER',
            engineKey: 'vg01_background_render',
            payload: { prompt: "dense forest", style: "oil painting" },
            context: { projectId, userId, traceId: `trace_vg01_${suffix}`, jobId, organizationId: orgId }
        });
        console.log("Res:", JSON.stringify(res, null, 2));
        if (!res.output?.assetUrl.startsWith('file://')) throw new Error("Expected file:// assetUrl");
        console.log("✅ VG01 Verified");
        process.exit(0);
    } catch (e) {
        console.error("❌ VG01 Failed", e);
        process.exit(1);
    }
}
main();
