...
async function main() {
  ...
  const user = await prisma.user.create({
    data: { email: `style_runner_${suffix}@example.com`, passwordHash: 'x' },
  });
  const org = await prisma.organization.create({
    data: { name: `StyleOrg_${suffix}`, ownerId: user.id },
  });
  const project = await prisma.project.create({
    data: { name: `StyleProj_${suffix}`, organizationId: org.id, ownerId: user.id },
  });
  ...
  }); // Use generic SHOT_RENDER type for now
  ...
  try {
    ...
    if ((res3 as any).error?.code !== 'STYLE_NO_KEY') {
      console.error('FAIL: Run 3 expected STYLE_NO_KEY');
      process.exit(1);
    }
