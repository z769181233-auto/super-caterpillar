...
async function main() {
    ...
    const user = await prisma.user.create({ data: { email: `trans_runner_${suffix}@example.com`, passwordHash: 'x' } });
    const org = await prisma.organization.create({ data: { name: `TransOrg_${suffix}`, ownerId: user.id } });
    const project = await prisma.project.create({ data: { name: `TransProj_${suffix}`, organizationId: org.id, ownerId: user.id } });
    const task = await prisma.task.create({ data: { organizationId: org.id, projectId: project.id, type: 'CE_CORE_PIPELINE', status: 'RUNNING' } });
    const job = await prisma.shotJob.create({ data: { organizationId: org.id, projectId: project.id, taskId: task.id, type: 'CE06_NOVEL_PARSING', status: 'RUNNING', attempts: 1 } });
    ...
    try {
        ...
        if (res1.output.source !== 'provider') { console.error("FAIL: Run 1 source != provider"); exitCode = 1; }
        if (res2.output.source !== 'cache') { console.error("FAIL: Run 2 source != cache"); exitCode = 1; }
        if (res1.output.text !== res2.output.text) { console.error("FAIL: Run 1 text != Run 2 text"); exitCode = 1; }

        // --- Test 2: Failure (No Key) ---
        console.log("--- Run 3: No Key (Expect FAIL) ---");
        delete process.env.TRANSLATION_API_KEY;
        const colInput = { ...input, payload: { ...input.payload, sourceText: "New Unique Text " + Date.now() } };
        const res3 = await adapter.invoke(colInput);
        console.log(JSON.stringify(res3, null, 2));

        if (res3.status !== 'FAILED') { console.error("FAIL: Run 3 expected FAILED, got " + res3.status); exitCode = 1; }
        if ((res3 as any).error?.code !== 'TRANSLATION_NO_KEY') { console.error("FAIL: Run 3 expected TRANSLATION_NO_KEY"); exitCode = 1; }
        ...
        if (auditLogs.length !== 3) { console.error(`FAIL: Expected 3 audit logs, got ${auditLogs.length}`); exitCode = 1; }

        // Confirm Run 3 logic: Failed -> Audit(MISS, failed_request)
        const failedAudit = auditLogs.find(l => (l.details as any).status === 'FAILED');
        if (!failedAudit) { console.error("FAIL: Missing FAILED audit log"); exitCode = 1; }
