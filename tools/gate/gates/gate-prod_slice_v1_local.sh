...
psql -d "${DATABASE_URL}" -c "INSERT INTO novel_sources (id, "projectId", "organizationId", "rawText", "fileName", "fileKey", "fileSize", "createdAt", "updatedAt") VALUES (\$gate\$${NOVEL_SOURCE_ID}\$gate\$, \$gate\$${PROJECT_ID}\$gate\$, \$gate\$org-gate\$gate\$, \$gate\$${ISAFE_TEXT_JSON}\$gate\$, \$gate\$sample.txt\$gate\$, \$gate\$keys/sample\$gate\$, 100, NOW(), NOW()) ON CONFLICT (id) DO NOTHING;"
