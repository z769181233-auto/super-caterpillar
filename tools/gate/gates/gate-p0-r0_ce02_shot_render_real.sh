#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R0: CE02 Mother -> shot_render (L2)
...
EVID_DIR="${EVID_ROOT}/p0_r0_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R0: shot_render (L2) START ---"
...
TRACE_ID="gate-P0-R0-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R0-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "shot_render",
  "payload": {"jobId": "${JOB_ID}", "model": "sd15"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
