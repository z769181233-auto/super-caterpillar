...
# Ensure export WORKER_SUPPORTED_ENGINES="pipeline_timeline_compose,timeline_render,pipeline_orchestrator,ce11,ce11_timeline_preview"
...
generate_hmac_headers_lines() {
  ...
  BODY_CONTENT="$body" NONCE="$nonce_override" TIMESTAMP="$ts_override" node -e "
    const crypto = require(    const apiKey =     const secret =     // Use override or generate
    const nonce = process.env.NONCE || crypto.randomBytes(8).toString(    const timestamp = process.env.TIMESTAMP || Math.floor(Date.now()/1000).toString();
    const body = process.env.BODY_CONTENT ||     
    // V1.1 Canonical
    const canonical = apiKey + nonce + timestamp + body; 
    const signature = crypto.createHmac(    
    console.log(    console.log(    console.log(    console.log(    console.log(    process.stderr.write(    process.stderr.write(    process.stderr.write(  " 2> "$EVIDENCE_DIR/hmac_meta.tmp"
}

# 4. Engine Registration & Asset Prep (Specific to this test)
GATE_UID="$(date +%s)_$RANDOM"
# Register Engine (TIMELINE_PREVIEW)
psql "$DATABASE_URL" -c "INSERT INTO \"engines\" (id, \"engineKey\", \"adapterName\", \"adapterType\", mode, config, enabled, code, \"isActive\", name, type, \"createdAt\", \"updatedAt\") VALUES ( # $gate$
...
psql "$DATABASE_URL" -c "INSERT INTO \"assets\" (id, \"projectId\", \"ownerId\", \"ownerType\", type, status, \"storageKey\", \"createdAt\") VALUES (psql "$DATABASE_URL" -c "INSERT INTO \"assets\" (id, \"projectId\", \"ownerId\", \"ownerType\", type, status, \"storageKey\", \"createdAt\") VALUES ( # $gate$
...
JOB_ID="$(node -e "try{const j=require(if [ -z "$JOB_ID" ]; then
...
CURL_HEADERS_REPLAY=()
while   CURL_HEADERS_REPLAY+=(-H "$line")
...
if [ "$HTTP_CODE_REPLAY" == "200" ] || [ "$HTTP_CODE_REPLAY" == "201" ]; then
    ...
    REPLAY_CODE=$(node -e "try{const o=JSON.parse(require(    if [ "$REPLAY_CODE" == "4004" ]; then
...
AUDIT_CHECK=$(psql "$DATABASE_URL" -t -c "SELECT count(*) FROM audit_logs WHERE details::text LIKE if [ "$AUDIT_CHECK" == "0" ]; then # $gate$
...
for ((i=1; i<=MAX_RETRIES; i++)); do
    ...
    CURL_HEADERS_POLL=()
    while         CURL_HEADERS_POLL+=(-H "$line")
    ...
    if [ "$HTTP_CODE_POLL" != "200" ]; then
        ...
        # Extract Status
        JOB_STATUS=$(node -e "try{const o=JSON.parse(require(        log "Poll $i: $JOB_STATUS"
...
ASSET_KEY=$(psql "$DATABASE_URL" -t -c "SELECT \"storageKey\" FROM assets WHERE \"ownerId\"= # $gate$
if [ -z "$ASSET_KEY" ]; then
