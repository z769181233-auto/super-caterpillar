#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R3: CE02 Mother -> ce04_visual_enrichment (L2)
...
EVID_DIR="${EVID_ROOT}/p0_r3_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R3: ce04_visual_enrichment (L2) START ---"
...
TRACE_ID="gate-P0-R3-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R3-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "ce04_visual_enrichment",
  "payload": {"structured_text": "扩写提示词"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
