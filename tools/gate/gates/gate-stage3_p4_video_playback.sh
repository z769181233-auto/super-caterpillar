#!/bin/bash
# Gate: Video Playback Signed URL + Range Support Verification
# Purpose: Verify the video rendering pipeline produces playable, secure assets
# Evidence: Audit API response + HTTP headers for no-range/range requests
...
ACCESS_TOKEN=$(echo "$LOGIN_RESP" | jq -r '.accessToken // .data.accessToken // empty')
...
echo "$AUDIT_RESP" | jq '.' > "$EVID_DIR/audit_response.json" 2>/dev/null || {
  echo "$AUDIT_RESP" > "$EVID_DIR/audit_response.txt"
  error_exit "Audit API returned non-JSON. See $EVID_DIR/audit_response.txt"
}

# 5. Extract videoAsset
VIDEO_STATUS=$(echo "$AUDIT_RESP" | jq -r '.videoAsset.status // "MISSING"')
SECURE_URL=$(echo "$AUDIT_RESP" | jq -r '.videoAsset.secureUrl // ""')
JOB_ID=$(echo "$AUDIT_RESP" | jq -r '.videoAsset.jobId // ""')
...
if ! echo "$SECURE_URL" | grep -q 'expires='; then
  error_exit "secureUrl missing 'expires' parameter"
fi
if ! echo "$SECURE_URL" | grep -q 'signature='; then
  error_exit "secureUrl missing 'signature' parameter"
fi
if ! echo "$SECURE_URL" | grep -q 'tenantId='; then
  error_exit "secureUrl missing 'tenantId' parameter"
fi
if ! echo "$SECURE_URL" | grep -q 'userId='; then
  error_exit "secureUrl missing 'userId' parameter"
fi
...
NO_RANGE_STATUS=$(head -1 "$EVID_DIR/video_headers_no_range.txt" | awk '{print $2}')
NO_RANGE_CT=$(grep -i 'content-type:' "$EVID_DIR/video_headers_no_range.txt" | awk '{print $2}' | tr -d '\r\n')

if [ "$NO_RANGE_STATUS" != "200" ]; then
  error_exit "No-Range request failed (status=$NO_RANGE_STATUS). Expected 200."
fi

if ! echo "$NO_RANGE_CT" | grep -qi 'video'; then
  error_exit "Content-Type is not video/* (got: $NO_RANGE_CT)"
fi
...
RANGE_STATUS=$(head -1 "$EVID_DIR/video_headers_range_206.txt" | awk '{print $2}')
RANGE_AR=$(grep -i 'accept-ranges:' "$EVID_DIR/video_headers_range_206.txt" | awk '{print $2}' | tr -d '\r\n')
