...
TS=$(date +%s)
PAYLOAD=$(jq -n --arg nt "Part A: Event-driven DAG content..." '{novelText: $nt}' | jq -c .)
...
PROJECT_ID_A=$(echo "$RESPONSE_A" | jq -r '.data.projectId // .projectId // empty')
...
TS_B=$(date +%s)
PAYLOAD_B=$(jq -n --arg nt "Part B: Real regression content..." '{novelText: $nt}' | jq -c .)
...
PROJECT_ID_B=$(echo "$RESPONSE_B" | jq -r '.data.projectId // .projectId // empty')
...
if ! echo "$DB_RESULT" | jq -e '.ok == true' > /dev/null; then
...
COMPLETED_JOB_ID=$(npx tsx -e "
  ...
  prisma.shotJob.findFirst({
    ...
    select: { id: true }
  }).then(j => { console.log(j?.id || ''); process.exit(0); });
" 2>/dev/null | tail -1)
