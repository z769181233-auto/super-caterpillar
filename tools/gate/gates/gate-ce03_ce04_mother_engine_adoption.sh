...
sha256_hex() { shasum -a 256 | awk hmac_sha256_hex() { local secret="$1"; openssl dgst -sha256 -hmac "$secret" | awk now_ms() { python3 -c make_nonce() { uuidgen | tr 
invoke_hub() {
  local engine_key="$1"
  local job_type="$2"
  ...
  local body_hash="$(printf   local msg="POST\n$path\n$ts\n$nonce\n$body_hash"
  local sig="$(printf 
...
DB_CHECK=$(psql "${DATABASE_URL}" -t -c "SELECT count(*) FROM audit_logs WHERE action =  # $gate$
