...
ORG_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM organizations LIMIT 1" | xargs)
...
PGPASSWORD=postgres psql -h localhost -U postgres -d scu -c "
  ...
  ('$JOB_ID_CE04', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'CE04_VISUAL_ENRICHMENT', 10, 'trace-p1a-ce04', '{\"shotId\": \"$SHOT_ID\"}', NOW(), NOW()),
  ('$JOB_ID_SDXL', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'SHOT_RENDER', 10, 'trace-p1a-sdxl', '{\"shotId\": \"$SHOT_ID\", \"qualityThreshold\": 0.7}', NOW(), NOW()),
  ('$JOB_ID_FLUX', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'SHOT_RENDER', 10, 'trace-p1a-flux', '{\"shotId\": \"$SHOT_ID\", \"qualityThreshold\": 0.99}', NOW(), NOW());
" > /dev/null
...
# 1. 验证 CE04 -> Gemini
CE04_MODEL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT \"metadata\"->'billingUsage'->>'model' FROM quality_metrics WHERE \"jobId\"='$JOB_ID_CE04'" | xargs)
if [ "$CE04_MODEL" != "gemini-2.0-flash" ]; then
...
QS_COUNT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT count(*) FROM quality_metrics WHERE \"jobId\"='$JOB_ID_CE04' AND engine='CE04' AND \"enrichmentQuality\" IS NOT NULL" | xargs)
if [ "$QS_COUNT" -lt 1 ]; then
...
# 3. 验证 SDXL 路由
SDXL_MODEL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT \"metadata\"->>'model' FROM quality_metrics WHERE \"jobId\"='$JOB_ID_SDXL'" | xargs)
if [ "$SDXL_MODEL" != "sdxl-v1.5-real" ]; then
...
# 4. 验证 Flux 路由
FLUX_MODEL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT \"metadata\"->>'model' FROM quality_metrics WHERE \"jobId\"='$JOB_ID_FLUX'" | xargs)
if [ "$FLUX_MODEL" != "flux-v1-dev" ]; then
...
# 5. 验证计费幂等/模型标识
FLUX_LEDGER_MODEL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT \"metadata\"->>'model' FROM cost_ledgers WHERE \"jobId\"='$JOB_ID_FLUX'" | xargs)
if [ "$FLUX_LEDGER_MODEL" != "flux-v1-dev" ]; then
