...
CHAR_ID=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT id FROM characters WHERE \"projectId\"='$PROJECT_ID' LIMIT 1" | xargs)

if [ -z "$CHAR_ID" ]; then
    log "Creating dummy character..."
    CHAR_ID="dummy-char-$(date +%s)"
    PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -c "INSERT INTO characters (id, \"projectId\", name, \"updatedAt\") VALUES ('$CHAR_ID', '$PROJECT_ID', 'GateTestChar', NOW())"
fi
...
PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -c "INSERT INTO shot_jobs (id, status, type, \"projectId\", \"organizationId\", \"episodeId\", \"sceneId\", \"shotId\", \"traceId\", payload, \"maxRetry\", attempts, priority, \"updatedAt\") VALUES ('$JOB_ID', 'PENDING', 'CE02_IDENTITY_LOCK', '$PROJECT_ID', '$ORG_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', '$TRACE_ID', '{\"characterId\": \"$CHAR_ID\", \"traceId\": \"$TRACE_ID\"}', 3, 0, 100, NOW())"
