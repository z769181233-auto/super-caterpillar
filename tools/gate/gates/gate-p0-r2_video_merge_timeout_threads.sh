...
npx tsx - <<'NODE'
  import fs from 'fs';
  const minPng = Buffer.from('89504e470d0a1a0a0000000d4948445200000001000000010802000000907753de0000000c4944415408d763606060000000020001e221bc330000000049454e44ae426082', 'hex');
  fs.mkdirSync(".tmp/p0r2_frames", { recursive: true });
  for (let i = 0; i < 3; i++) {
    fs.writeFileSync(`.tmp/p0r2_frames/f${i}.png`, minPng);
  }
NODE
