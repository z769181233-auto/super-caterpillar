...
IDENTITY_KEY=$(grep "IDENTITY_KEY=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
CE05_JOB_ID=$(grep "CE05_JOB_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
CE06_JOB_ID=$(grep "CE06_JOB_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
...
while true; do
    ...
    STATUS=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT status FROM shot_jobs WHERE id='$CE05_JOB_ID'" | xargs)
    if [ "$STATUS" == "SUCCEEDED" ]; then log "CE05 Succeeded"; break; fi
...
while true; do
    ...
    if [ "$STATUS" == "SUCCEEDED" ]; then log "CE06 Succeeded"; break; fi
...
# Find the Generated Shot (Not the dummy one from trigger, but the 'Analysis Placeholder' one)
# We search for shots in the project
...
while true; do
    ...
    # Better: Inspect shots linked to this Project's scenes.
    
    SHOT_ROW=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "
      ...
      AND s.\"director_control_snapshot_id\" IS NOT NULL
      LIMIT 1
    ")
    
    SHOT_ID=$(echo "$SHOT_ROW" | awk -F '|' '{print $1}' | xargs)
    SSOT_ID=$(echo "$SHOT_ROW" | awk -F '|' '{print $2}' | xargs)
    SHOT_TOKEN=$(echo "$SHOT_ROW" | awk -F '|' '{print $3}' | xargs)
