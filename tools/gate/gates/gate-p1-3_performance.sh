...
print(f'header = "x-api-key: {key_id}"')
print(f'header = "x-nonce: {nonce}"')
print(f'header = "x-timestamp: {timestamp}"')
print(f'header = "x-signature: {sig}"')
PY
  return 0
}
...
snapshot_metrics () {
  ...
  jobs_total="$(psqlq -Atc "select count(*) from ${JOB_TABLE};" | tr -d '[:space:]')"

  terminal="$(psqlq -Atc "
select count(*)
from ${JOB_TABLE}
where \"${STATUS_COL}\"::text in ('SUCCEEDED','COMPLETED','FAILED','CANCELED','CANCELLED');" | tr -d '[:space:]')"

  succeeded="$(psqlq -Atc "
select count(*)
from ${JOB_TABLE}
where \"${STATUS_COL}\"::text in ('SUCCEEDED','COMPLETED');" | tr -d '[:space:]')"

  failed="$(psqlq -Atc "
select count(*)
from ${JOB_TABLE}
where \"${STATUS_COL}\"::text in ('FAILED','CANCELED','CANCELLED');" | tr -d '[:space:]')"

  pending="$(psqlq -Atc "
select count(*)
from ${JOB_TABLE}
where \"${STATUS_COL}\"::text not in ('SUCCEEDED','COMPLETED','FAILED','CANCELED','CANCELLED');" | tr -d '[:space:]')"

  ledger_rows="$(psqlq -Atc "select count(*) from cost_ledger;" | tr -d '[:space:]')"
  # Ensure duplicates count is robust (handle empty)
  ledger_dups="$(psqlq -Atc "
select count(*)
from (
  ...
  if [[ -z "$ledger_dups" ]]; then ledger_dups="0"; fi
...
probe_code="$(echo "$probe_resp" | tail -1 | tr -d '[:space:]')"
probe_body="$(echo "$probe_resp" | sed '$d')"
echo "$probe_body" > "$EVID/raw/auth_probe_body.json"
...
JSON
  ...
  resp="$(curl -sS -w "\n%{http_code}" -X POST "$CREATE_URL" \
    ...
    --config "$hdr" \
    -d "$payload" || true)"
  httpcode="$(echo "$resp" | tail -1 | tr -d '[:space:]')"
  ...
  local job_id
  job_id="$(echo "$body" | "${PYTHON_BIN}" - <<'PY'
import sys, json
def pick(o, path):
  cur=o
  for k in path:
...
PY
  ...
  if [[ -z "$job_id" ]]; then
     ...
     job_id="$(echo "$body" | sed -n 's/.*"data"[[:space:]]*:[[:space:]]*{[[:space:]]*"id"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')"
     if [[ -n "$job_id" ]]; then
...
cat "$EVID/raw/_load_idx.txt" | xargs -P "$LOAD_CONCURRENCY" -n 1 -I{} bash -lc 'create_one "$@"' _ {}

# de-dup + sanitize job ids
sort -u "$EVID/raw/job_ids.txt" | sed '/^\s*$/d' > "$EVID/raw/job_ids.unique.txt"
mv "$EVID/raw/job_ids.unique.txt" "$EVID/raw/job_ids.txt"
created_cnt="$(wc -l < "$EVID/raw/job_ids.txt" | tr -d '[:space:]')"
log "Created jobs (parsed data.id): ${created_cnt}/${LOAD_N}"

if [[ "$created_cnt" -eq 0 ]]; then
  log "❌ Create phase produced 0 job ids. Dumping diagnostics..."
  log "Last 10 httpcodes:"
  tail -n 10 "$EVID/raw/create_httpcodes.log" 2>/dev/null | sed 's/^/[httpcode] /' | tee -a "$EVID/gate.log" || true
  log "Last 3 responses:"
  tail -n 3 "$EVID/raw/create_responses.jsonl" 2>/dev/null | sed 's/^/[resp] /' | tee -a "$EVID/gate.log" || true
  exit 30
fi

# compute create throughput
end_create_ms="$("${PYTHON_BIN}" - <<'PY'
import time; print(int(time.time()*1000))
PY
)"
...
while : ; do
  ...
  while read -r jid; do
    ...
    resp_with_code="$(curl -sS -w "\n%{http_code}" -X GET "${STATUS_URL_BASE}/${jid}" \
      --config "$hdr" || true)"
    httpcode="$(echo "$resp_with_code" | tail -1 | tr -d '[:space:]')"
    resp="$(echo "$resp_with_code" | sed '$d')"
    echo "$resp" >> "$EVID/raw/status_samples.jsonl"
...
PY
  ...
  done_cnt="$(find "$EVID/raw/map_done" -type f 2>/dev/null | wc -l | tr -d '[:space:]')"
  if [[ "$done_cnt" -ge "$created_cnt" && "$created_cnt" -gt 0 ]]; then
...
ledger_dups_last="$(
  psql "$DB" -v ON_ERROR_STOP=1 -X -q -t -A <<SQL
\\set QUIET 1
CREATE TEMP TABLE run_job_ids(job_id text) ON COMMIT DROP;
\\copy run_job_ids(job_id) FROM '${EVID}/raw/job_ids.txt' WITH (FORMAT text)
SELECT count(*)
FROM (
...
ledger_dups_last="$(echo "$ledger_dups_last" | tr -d '[:space:]')"
if [[ -z "$ledger_dups_last" ]]; then ledger_dups_last="0"; fi
...
pending_end="$(tail -1 "$EVID/metrics.csv" | awk -F',' '{print $7}' | tr -d '[:space:]')"
if [[ -z "$pending_end" ]]; then pending_end="0"; fi
