...
EVID_DIR="./evidence/vg12_rig_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$EVID_DIR"
mkdir -p tools/p3

echo "=== VG12 动态灯光组引擎验证 ===" | tee "$EVID_DIR/gate.log"

cat <<'EOF' > tools/p3/test_vg12.ts
...
npx ts-node -r tsconfig-paths/register tools/p3/test_vg12.ts 2>&1 | tee "$EVID_DIR/test_output.log"
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ VG12 Gate PASS" | tee -a "$EVID_DIR/gate.log"
    exit 0
else
    echo "❌ VG12 Gate FAIL" | tee -a "$EVID_DIR/gate.log"
    exit 1
fi
