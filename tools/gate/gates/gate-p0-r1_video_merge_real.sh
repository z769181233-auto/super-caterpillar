...
import { videoMergeRealEngine } from '../../../../packages/engines/video_merge/real';
import { VideoMergeInput } from '../../../../packages/engines/video_merge/types';
...
ASSET_URI=$(echo "$JSON_OUTPUT" | grep '"uri":' | awk -F'"' '{print $4}')
DURATION=$(echo "$JSON_OUTPUT" | grep '"durationSeconds":' | awk -F': ' '{print $2}' | tr -d ',')
CPU_SECONDS=$(echo "$JSON_OUTPUT" | grep '"cpuSeconds":' | awk -F': ' '{print $2}' | tr -d ',')
MIME=$(echo "$JSON_OUTPUT" | grep '"mimeType":' | awk -F'"' '{print $4}')
...
SIGNATURE=$(head -c 12 "$ASSET_URI" | xxd -p | grep -o "66747970") # hex for 'ftyp' is 66747970
# Actually ftyp is usually at offset 4. 000000XX 66747970 ...
# Let's just run file command if available
FILE_TYPE=$(file -b "$ASSET_URI")
