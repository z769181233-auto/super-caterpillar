...
EVID_DIR="./evidence/ce19_summary_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$EVID_DIR"
mkdir -p tools/p3

echo "=== CE19 故事大纲生成引擎验证 ===" | tee "$EVID_DIR/gate.log"

cat <<'EOF' > tools/p3/test_ce19.ts
...
npx ts-node -r tsconfig-paths/register tools/p3/test_ce19.ts 2>&1 | tee "$EVID_DIR/test_output.log"
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ CE19 Gate PASS" | tee -a "$EVID_DIR/gate.log"
    exit 0
else
    echo "❌ CE19 Gate FAIL" | tee -a "$EVID_DIR/gate.log"
    exit 1
fi
