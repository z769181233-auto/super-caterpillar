...
# gate_emotion_analysis.sh <evidence_dir>
EVI="${1:-docs/_evidence/emotion_analysis_v1}"
# Ensure absolute path
EVI=$(mkdir -p "$EVI" && cd "$EVI" && pwd)

RUNNER="tools/p2/run_emotion_analysis.ts"
# Root dir
cd "$(dirname "$0")/../../.."
...
echo "=== Gate P2.3: Emotion Analysis Engine ===" | tee "$EVI/gate.log"
...
if grep -q "Emotion Analysis Verified" "$EVI/runner_output.txt"; then
...
{
  "gate": "gate_emotion_analysis.sh",
  "status": "PASS",
  "runner": "run_emotion_analysis.ts",
  "evidence": ["env_snapshot.txt", "gate.log", "runner_output.txt"],
  "engine": "emotion_analysis",
