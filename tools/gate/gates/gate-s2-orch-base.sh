...
JOB_ID=$(grep "Created Job:" "${EVIDENCE_DIR}/seed.log" | awk USER_ID=$(grep "UserId:" "${EVIDENCE_DIR}/seed.log" | awk 
...
for i in {1..20}; do
  ...
  # Trim whitespace
  RAW_OUTPUT=$(echo "${RAW_OUTPUT}" | tr -d   
  CURRENT_STATUS=$(echo "${RAW_OUTPUT}" | cut -d  CURRENT_WORKER=$(echo "${RAW_OUTPUT}" | cut -d  
