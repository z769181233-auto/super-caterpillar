...
# Note: Since it's a REAL engine call, we check the audit trail in response first
PROVIDER=$(echo "${RESP}" | jq -r '.data.output.audit_trail.providerSelected')
ASSET_URI=$(echo "${RESP}" | jq -r '.data.output.asset.uri')
...
LEDGER_COUNT=$(psql "${DB_URL}" -t -A -c "SELECT COUNT(*) FROM cost_ledgers WHERE metadata->>'traceId' = '${TRACE_ID}'")
