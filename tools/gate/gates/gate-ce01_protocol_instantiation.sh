#!/usr/bin/env bash
# Gate: CE01 Protocol Instantiation
# Purpose: 验证 CE01 角色基准协议层实例化逻辑
# - 实例化调用
# - 产生 referenceSheetId
# - 幂等复用验证
...
build_hmac_headers() {
  ...
  sig="$(printf '%b' "$msg" | hmac_sha256_hex "$TEST_TOKEN")"
...
if ! echo "$PING_RESP" | grep -qE '^HTTP/.* 200 '; then
  echo "❌ HMAC PING FAILED (Path: $PING_PATH)"
  exit 1
fi
...
BODY="$(printf '{"projectId":"%s","characterId":"%s","posePreset":"front","styleSeed":"default"}' "$PROJECT_ID" "$CHARACTER_ID")"
...
# 提取 referenceSheetId
REF_SHEET_ID_1=$(echo "$RESPONSE_1" | jq -r '.data.referenceSheetId // empty')
FINGERPRINT_1=$(echo "$RESPONSE_1" | jq -r '.data.fingerprint // empty')
ENGINE_KEY_1=$(echo "$RESPONSE_1" | jq -r '.data.engineKey // empty')
...
DB_COUNT=$(psql "$DATABASE_URL" -t -c "
  SELECT COUNT(*) FROM job_engine_bindings WHERE id = '$REF_SHEET_ID_1';
" 2>/dev/null || echo "0")
...
METADATA=$(psql "$DATABASE_URL" -t -c "
  SELECT metadata::text FROM job_engine_bindings WHERE id = '$REF_SHEET_ID_1';
" 2>/dev/null || echo "{}")
...
REF_SHEET_ID_2=$(echo "$RESPONSE_2" | jq -r '.data.referenceSheetId // empty')
...
BODY3="$(printf '{"projectId":"%s","characterId":"%s","posePreset":"side","styleSeed":"default"}' "$PROJECT_ID" "$CHARACTER_ID")"
read -r TS3 NONCE3 SIG3 <<<"$(build_hmac_headers "POST" "$INSTANTIATE_PATH" "$BODY3")"
...
REF_SHEET_ID_3=$(echo "$RESPONSE_3" | jq -r '.data.referenceSheetId // empty')
...
JOBS_BODY='{"type":"SHOT_RENDER","payload":{}}'
...
# 提取 HTTP Code
HTTP_CODE=$(echo "$REJECT_RESPONSE" | head -n1 | awk '{print $2}')
BODY=$(echo "$REJECT_RESPONSE" | sed '1,/^\r$/d')
...
ERROR_CODE=$(echo "$BODY" | jq -r '.code // .error.code // empty')
if [[ "$ERROR_CODE" != "REFERENCE_SHEET_REQUIRED" ]]; then
