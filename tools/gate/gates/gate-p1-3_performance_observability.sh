...
# My code: app.setGlobalPrefix('api', { exclude: ['/metrics'] }); 
# And controller path is 'metrics'. So it should be /metrics or /api/metrics?
# The controller uses decorator @Controller('metrics'). NestJS uses global prefix by default unless excluded.
# Exclusion list has '/metrics'. So it should be /metrics.
API_METRICS_CHECK=$(curl -s http://localhost:3000/metrics || true)
...
# We can use a script or curl. Let's use curl to /api/test/job or similar if available, or just rely on startup logs?
# Startup logs won't have traceId for job manually.
# Let's hit the health check or ops endpoint and see if x-trace-id is returned.
TRACE_CHECK=$(curl -I -s http://localhost:3000/health)
TRACE_ID_HEADER=$(echo "$TRACE_CHECK" | grep -i "x-trace-id" | awk '{print $2}' | tr -d '\r')
...
# Since we didn't trigger a real full job execution flow easily without DB seed, we check if API logs contain the traceId.
if grep -q "$TRACE_ID_HEADER" "$EVIDENCE_DIR/api.log"; then
