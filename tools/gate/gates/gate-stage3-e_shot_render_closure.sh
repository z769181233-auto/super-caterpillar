...
ORG_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM organizations WHERE slug=SHOT_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM shots LIMIT 1" | xargs) # $gate$

JOB_ID=$(node -e "const crypto = require(TRACE_ID="trace-render-${TS}"
...
LEDGER_TRACE=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT \"traceId\" FROM ${TABLE_LEDGER} WHERE \"jobId\"=if [ "$LEDGER_TRACE" != "$TRACE_ID" ]; then log "❌ FAIL: Trace mismatch $LEDGER_TRACE vs $TRACE_ID"; exit 1; fi
log "✅ Assertion 4: Trace ID Matches"

# 5. Asset DB Record
ASSET_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT id FROM ${TABLE_ASSET} WHERE \"storageKey\"=if [ -z "$ASSET_ID" ]; then log "❌ FAIL: Asset DB record missing"; exit 1; fi
