...
V1_ID=$(grep "^V1_SNAPSHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
V2_ID=$(grep "^V2_SNAPSHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
...
LATEST_VERSION=$(echo "$LATEST_RESP" | grep -o '"version":[0-9]*' | head -1 | cut -d: -f2 || echo "0")
...
PREFER_V1_VERSION=$(echo "$PREFER_V1_RESP" | grep -o '"version":[0-9]*' | head -1 | cut -d: -f2 || echo "0")
