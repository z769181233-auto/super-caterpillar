...
EVID_DIR="${EVID_ROOT}/p0_r2_ce02_ce03_real_${TS}"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R2: CE02 Mother -> CE03 Real START ---"
...
TRACE_ID="gate-p0r2-ce03-$(date +%s)"
...
echo "[SQL] Verifying CE03 Output Structure and Ledger Isolation..."

# A) Output Structure Assertion
# CE03 must return 'visual_density_score'
SCORE=$(echo "${RESP}" | jq -r '.data.output.visual_density_score')
if [ "${SCORE}" == "null" ]; then
  echo "❌ FAIL: CE03 output 'visual_density_score' missing."
  write_exit_code 4
  exit 4
fi
...
echo "Visual Density Score: ${SCORE}" > "${EVID_DIR}/sql_evidence_jobs.txt"

cat > "${EVID_DIR}/SUMMARY.md" <<MD
# P0-R2: CE02 Mother -> CE03 Real Gate Summary
...
2) Real CE03 Logic execution (Score returned)
...
echo "--- [GATE] P0-R2: CE02 Mother -> CE03 Real PASS ---"
exit 0
