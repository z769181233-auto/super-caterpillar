...
JOB_ID=$(echo $RESPONSE | jq -r if [ "$JOB_ID" == "null" ] || [ -z "$JOB_ID" ]; then
...
while [ "$STATUS" != "SUCCEEDED" ] && [ "$STATUS" != "FAILED" ] && [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  STATUS=$(echo $JOB_CHECK | jq -r   echo "Current status: ${STATUS}..."
...
VIDEO_URI=$(echo $JOB_CHECK | jq -r echo "Video URI: ${VIDEO_URI}"
