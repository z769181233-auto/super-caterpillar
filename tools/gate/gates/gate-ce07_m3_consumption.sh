...
PROJECT_ID=$(grep "^PROJECT_ID=" "$EVID_DIR/setup_output.txt" | cut -d= -f2 | tr -d '\r')
IDENTITY_KEY=$(grep "^IDENTITY_KEY=" "$EVID_DIR/setup_output.txt" | cut -d= -f2 | tr -d '\r')
...
BEFORE_COUNT=$(PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "
    SELECT COUNT(*) FROM memory_snapshots WHERE project_id='$PROJECT_ID'
" | xargs)
...
VERSION=$(echo "$CONSUME_RESP" | grep -o '"version":[0-9]*' | head -1 | cut -d: -f2 || echo "0")
SNAPSHOT_ID=$(echo "$CONSUME_RESP" | grep -o '"snapshotId":"[^"]*"' | head -1 | cut -d'"' -f4 || echo "")
...
IDENTITY_IN_RESP=$(echo "$CONSUME_RESP" | grep -o '"identityLockToken":"[^"]*"' | head -1 | cut -d'"' -f4 || echo "")
...
PREFER_VERSION=$(echo "$PREFER_RESP" | grep -o '"version":[0-9]*' | head -1 | cut -d: -f2 || echo "0")
...
AFTER_COUNT=$(PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "
    SELECT COUNT(*) FROM memory_snapshots WHERE project_id='$PROJECT_ID'
" | xargs)
