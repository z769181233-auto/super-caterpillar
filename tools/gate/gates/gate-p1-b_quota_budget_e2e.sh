...
( export API_PORT=$PORT PORT=$PORT ENABLE_GATE_TEST_BYPASS=1 DATABASE_URL="$DATABASE_URL" NODE_ENV=development; npx ts-node -r tsconfig-paths/register apps/api/src/main.ts > .runtime/api_p1b.log 2>&1 ) &
API_PID=$!
sleep 25
...
if grep -q '"jobMaxInFlight":[[:space:]]*2' .runtime/worker_p1b.log; then
  ...
  echo "FAIL: SAFE_MODE failed to suppress concurrency"
  exit 1
fi
...
npx ts-node tools/gate/common/gate_seed.ts --action=setup_test_project --orgId=$ORG_QUOTA

RESP_Q=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  -H "Content-Type: application/json" \
  -H "X-SCU-Test-Auth-Bypass: p1b-gate-key" \
  -H "X-SCU-Test-Org-Id: $ORG_QUOTA" \
...
npx ts-node tools/gate/common/gate_seed.ts --action=setup_test_project --orgId=$ORG_B80

RESP_B80=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  -H "Content-Type: application/json" \
  -H "X-SCU-Test-Auth-Bypass: p1b-gate-key" \
  -H "X-SCU-Test-Org-Id: $ORG_B80" \
  -H "X-Nonce: gate-test-nonce-$(date +%s)" \
  -H "X-Signature: gate-test-signature-placeholder" \
  -H "X-Timestamp: $(date +%s)000" \
  -d '{"type":"SHOT_RENDER","payload":{}}')
...
npx ts-node tools/gate/common/gate_seed.ts --action=setup_test_project --orgId=$ORG_B120

RESP_B120=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  -H "Content-Type: application/json" \
  -H "X-SCU-Test-Auth-Bypass: p1b-gate-key" \
  -H "X-SCU-Test-Org-Id: $ORG_B120" \
  -H "X-Nonce: gate-test-nonce-$(date +%s)" \
  -H "X-Signature: gate-test-signature-placeholder" \
  -H "X-Timestamp: $(date +%s)000" \
  -d '{"type":"SHOT_RENDER","payload":{}}')
