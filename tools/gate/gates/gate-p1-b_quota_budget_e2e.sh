...
if grep -q   echo "PASS: SAFE_MODE successfully suppressed concurrency"
else
...
generate_jwt() {
  ...
  cd apps/api && node -e "
    try {
      const jwt = require(      console.log(jwt.sign({ 
        sub:         orgId:         roles: [      },     } catch (e) { console.error(e); process.exit(1); }
  " && cd ../..
}
...
RESP_Q=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  ...
  -d 
if echo "$RESP_Q" | grep -q   echo "PASS: QuotaGuard successfully blocked job creation (402)"
else
...
RESP_B80=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  ...
  -H "X-Timestamp: $(date +%s)000" \
  -d 
if echo "$RESP_B80" | grep -q   echo "PASS: BudgetGuard 80% - Job created successfully (warning level)"  
else
...
RESP_B100_STD=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  ...
  -H "X-Timestamp: $(date +%s)000" \
  -d 
if echo "$RESP_B100_STD" | grep -q   echo "PASS: BudgetGuard 100% Standard Cost - Job created successfully"
else
...
RESP_B100_HIGH=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  ...
  -H "X-Timestamp: $(date +%s)000" \
  -d 
if echo "$RESP_B100_HIGH" | grep -q   echo "PASS: BudgetGuard 100% High Cost - Job blocked (402)"
else
...
RESP_B120=$(curl -s -X POST "http://localhost:$PORT/api/shots/p1b-test-shot/jobs" \
  ...
  -H "X-Timestamp: $(date +%s)000" \
  -d 
if echo "$RESP_B120" | grep -q   echo "PASS: BudgetGuard 120% Blocked"
else
