...
EVID_DIR="./evidence/ce16_story_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$EVID_DIR"
mkdir -p tools/p3

echo "=== CE16 故事分支协调引擎验证 ===" | tee "$EVID_DIR/gate.log"

cat <<'EOF' > tools/p3/test_ce16.ts
...
npx ts-node -r tsconfig-paths/register tools/p3/test_ce16.ts 2>&1 | tee "$EVID_DIR/test_output.log"
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ CE16 Gate PASS" | tee -a "$EVID_DIR/gate.log"
    exit 0
else
    echo "❌ CE16 Gate FAIL" | tee -a "$EVID_DIR/gate.log"
    exit 1
fi
