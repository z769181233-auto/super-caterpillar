...
TRACE_ID=$(echo $RESPONSE | jq -r if [ "$TRACE_ID" == "null" ]; then
...
LEDGER_DATA=$(psql -t -A -c "SELECT json_agg(t) FROM (SELECT \"jobId\", \"costAmount\", \"engineKey\", \"metadata\" FROM cost_ledgers WHERE \"jobId\" LIKE  # $gate$
...
REAL_COST_FOUND=$(echo "${LEDGER_DATA}" | jq -r 
