...
# Orchestrator V2: Audio Integration Gate
# Goal: Verify "Sound + Vision" dual-track pipeline.

TS=$(date +%s)
EVIDENCE_DIR="docs/_evidence/orch_v2_audio_gate_$TS"
mkdir -p "$EVIDENCE_DIR"

echo "=============================================="
echo "GATE: Orchestrator V2 Audio Integration"
echo "=============================================="

# 1. Enable Feature Flag
export ORCH_V2_AUDIO_ENABLED="1"
echo "[Gate] Feature Flag: ORCH_V2_AUDIO_ENABLED=$ORCH_V2_AUDIO_ENABLED"

# 2. Use a specialized test script for E2E flow
cat > tools/ops/test_orch_v2_audio.ts <<'EOF'
...
npx ts-node -T -r tsconfig-paths/register tools/ops/test_orch_v2_audio.ts > "$EVIDENCE_DIR/gate_stdout.log" 2>&1

if [ $? -eq 0 ]; then
    echo "✅ GATE PASS: $EVIDENCE_DIR"
    cat "$EVIDENCE_DIR/gate_stdout.log"
else
    echo "❌ GATE FAIL"
    cat "$EVIDENCE_DIR/gate_stdout.log"
    exit 1
fi
rm tools/ops/test_orch_v2_audio.ts
