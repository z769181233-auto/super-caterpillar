...
EVID_DIR="./evidence/qc06_flicker_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$EVID_DIR"
mkdir -p tools/p3

echo "=== QC06 画面闪烁检测引擎验证 ===" | tee "$EVID_DIR/gate.log"

cat <<'EOF' > tools/p3/test_qc06.ts
...
npx ts-node -r tsconfig-paths/register tools/p3/test_qc06.ts 2>&1 | tee "$EVID_DIR/test_output.log"
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ QC06 Gate PASS" | tee -a "$EVID_DIR/gate.log"
    exit 0
else
    echo "❌ QC06 Gate FAIL" | tee -a "$EVID_DIR/gate.log"
    exit 1
fi
