...
# Since we can# For this sub-script, we focus on the positive real generation.
...
SUCCESS=$(echo $RESPONSE | jq -r if [ "$SUCCESS" != "true" ]; then
  echo "FAIL: Shot Render trigger failed."
  echo $RESPONSE | jq .
  exit 1
fi

ASSET_URI=$(echo $RESPONSE | jq -r SHA256=$(echo $RESPONSE | jq -r MOCKED=$(echo $RESPONSE | jq -r 
# Save input record
...
CALC_SHA256=$(shasum -a 256 "${ASSET_URI}" | awk if [ "${SHA256}" != "${CALC_SHA256}" ]; then
