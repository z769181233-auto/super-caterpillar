...
# Note: rg return 0 if found (which is BAD here), 1 if not found.
...
# 3) 运行一个最小 node 脚本调用 sha256File
echo "Running memory check..." | tee -a "$LOG"
node - "$TMP" <<const fs = require(const { createHash } = require(
const argPath = process.argv[2];
const fallbackPath = "docs/_evidence/tmp_bigfile_512mb.bin";
const filePath = argPath || fallbackPath;

console.log("filePath:", filePath);

function sha256File(filePath) {
  return new Promise((resolve, reject) => {
    const hash = createHash(    const stream = fs.createReadStream(filePath, { highWaterMark: 1024 * 1024 });
