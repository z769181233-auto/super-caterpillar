...
EVID_DIR="./evidence/ce17_culture_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$EVID_DIR"
mkdir -p tools/p3

echo "=== CE17 文化一致性校验引擎验证 ===" | tee "$EVID_DIR/gate.log"

cat <<'EOF' > tools/p3/test_ce17.ts
...
async function main() {
    ...
    await (prisma as any).project.upsert({ where: { id: context.projectId }, update: {}, create: { id: context.projectId, name: 'CE17 Test', ownerId: 'system', organizationId: 'org1' } });
...
npx ts-node -r tsconfig-paths/register tools/p3/test_ce17.ts 2>&1 | tee "$EVID_DIR/test_output.log"
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ CE17 Gate PASS" | tee -a "$EVID_DIR/gate.log"
    exit 0
else
    echo "❌ CE17 Gate FAIL" | tee -a "$EVID_DIR/gate.log"
    exit 1
fi
