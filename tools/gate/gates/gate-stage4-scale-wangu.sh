...
psql "$DATABASE_URL" -c "INSERT INTO users (id, email, \"passwordHash\", role, \"createdAt\", \"updatedAt\") VALUES ('user_wangu', 'wangu@test.com', 'pass', 'ADMIN', NOW(), NOW()) ON CONFLICT DO NOTHING;"
psql "$DATABASE_URL" -c "INSERT INTO organizations (id, name, \"ownerId\", \"createdAt\", \"updatedAt\", credits) VALUES ('org_wangu', 'Wangu Org', 'user_wangu', NOW(), NOW(), 1000) ON CONFLICT DO NOTHING;"
psql "$DATABASE_URL" -c "UPDATE users SET \"defaultOrganizationId\"='org_wangu' WHERE id='user_wangu';"
psql "$DATABASE_URL" -c "INSERT INTO projects (id, name, \"ownerId\", \"organizationId\", \"createdAt\", \"updatedAt\", status) VALUES ('$PROJECT_ID', 'Wangu Story', 'user_wangu', 'org_wangu', NOW(), NOW(), 'in_progress');"

# 2.5 Create NovelSource Record
NS_ID="ns_wangu_$(date +%s)"
echo "Creating NovelSource Record ($NS_ID)..."
psql "$DATABASE_URL" -c "psql "$DATABASE_URL" -c "INSERT INTO novel_sources (id, "projectId", "organizationId", "rawText", "fileName", "fileKey", "fileSize", "createdAt", "updatedAt") VALUES ('$NS_ID', '$PROJECT_ID', 'org_wangu', '$ABS_FILE_PATH', '万古神帝.txt', 150000000, 'PENDING', 0, 0, NOW(), NOW());"
...
psql "$DATABASE_URL" -c "INSERT INTO shot_jobs (id, \"projectId\", \"organizationId\", type, status, payload, priority, \"createdAt\", \"updatedAt\") VALUES ('$JOB_ID', '$PROJECT_ID', 'org_wangu', 'NOVEL_SCAN_TOC', 'PENDING', '$PAYLOAD', 100, NOW(), NOW());"
