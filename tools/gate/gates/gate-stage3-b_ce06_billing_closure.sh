...
while [ $MAX_RETRY -gt 0 ]; do
  STATUS=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT status FROM shot_jobs WHERE id=  log "Status: $STATUS" # $gate$
...
LEDGER_COUNT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT count(*) FROM ${LEDGER_TABLE} WHERE \"jobId\"=
...
TOKENS=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT quantity FROM ${LEDGER_TABLE} WHERE \"jobId\"=
...
CREDITS=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT \"totalCredits\" FROM ${LEDGER_TABLE} WHERE \"jobId\"=
...
DUP=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
...
MODEL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT \"modelName\" FROM ${LEDGER_TABLE} WHERE \"jobId\"=
if [ -z "$MODEL" ]; then
...
UNIT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \ # $gate$
  "SELECT \"unitCostCredits\" FROM ${LEDGER_TABLE} WHERE \"jobId\"=
