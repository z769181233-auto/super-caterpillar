#!/usr/bin/env bash
# gate-engine_entry_ssot.sh
# 强制保持引擎系统唯一入口 (SSOT)
# 禁止在非适配层新增对 src/engine/ 或 src/engines/ 的引用
...
# 规则：禁止 import ... from '../engine/' 或 '../engines/'

# 我们主要关注 apps/api/src 下的所有目录，排除 engine-hub 和底层基座
VIOLATIONS=$(grep -rE "from '../engine/|from '../engines/" apps/api/src \
  --exclude-dir="engine-hub" \
  --exclude-dir="engine" \
  --exclude-dir="engines" \
  --exclude-dir="job" \
