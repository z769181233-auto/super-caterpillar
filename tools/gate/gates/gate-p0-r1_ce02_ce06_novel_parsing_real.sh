#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R1: CE02 Mother -> ce06_novel_parsing (L2)
...
EVID_DIR="${EVID_ROOT}/p0_r1_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R1: ce06_novel_parsing (L2) START ---"
...
TRACE_ID="gate-P0-R1-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R1-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "ce06_novel_parsing",
  "payload": {"structured_text": "测试数据"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
