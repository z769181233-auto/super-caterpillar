...
EVID_DIR="docs/_evidence/hmac_regression_$(date +mkdir -p "$EVID_DIR"

# 清理数据库
PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -c " # $gate$
DELETE FROM shot_jobs WHERE type=DELETE FROM worker_nodes WHERE \"workerId\" LIKE " > /dev/null
...
for i in $(seq 1 $MAX_WAIT); do
    ...
    SELECT j.status, j.attempts, wn.\"workerId\" FROM shot_jobs j LEFT JOIN worker_nodes wn ON wn.id = j.\"workerId\" WHERE j.id=    ")
    S=$(echo "$ROW" | awk -F    A=$(echo "$ROW" | awk -F    W=$(echo "$ROW" | awk -F
