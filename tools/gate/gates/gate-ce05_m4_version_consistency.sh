...
PROJECT_ID=$(grep "^PROJECT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d V1_SNAPSHOT_ID=$(grep "^V1_SNAPSHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d V2_SNAPSHOT_ID=$(grep "^V2_SNAPSHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d V1_SHOT_ID=$(grep "^V1_SHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d V2_SHOT_ID=$(grep "^V2_SHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d 
...
V1_SHOT_SNAPSHOT=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT director_control_snapshot_id FROM shots WHERE id= # $gate$
...
V2_SHOT_SNAPSHOT=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT director_control_snapshot_id FROM shots WHERE id= # $gate$
