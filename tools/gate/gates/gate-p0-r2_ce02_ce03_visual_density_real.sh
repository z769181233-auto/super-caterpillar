#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R2: CE02 Mother -> ce03_visual_density (L2)
...
EVID_DIR="${EVID_ROOT}/p0_r2_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R2: ce03_visual_density (L2) START ---"
...
TRACE_ID="gate-P0-R2-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R2-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "ce03_visual_density",
  "payload": {"structured_text": "测量视觉密度"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
