...
INSERT INTO shot_jobs (
  ...
  'PENDING', 'CE04_VISUAL_ENRICHMENT', 10, 3, '$TRACE_ID', 
  '{\"structured_text\": \"Night scene in cyberpunk city\"}', 
  NOW(), NOW()
);
" > /dev/null
...
# Asserts 1-8
METRICS_JSON=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \
  "SELECT row_to_json(m) FROM (SELECT \"enrichmentQuality\", metadata FROM ${TABLE_METRICS} WHERE \"jobId\"='$JOB_ID' LIMIT 1) m")

LEDGER_COUNT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \
  "SELECT count(*) FROM ${TABLE_LEDGER} WHERE \"jobId\"='$JOB_ID' AND \"jobType\"='CE04_VISUAL_ENRICHMENT'" | xargs)

TOTAL_CREDITS=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \
  "SELECT \"totalCredits\" FROM ${TABLE_LEDGER} WHERE \"jobId\"='$JOB_ID' AND \"jobType\"='CE04_VISUAL_ENRICHMENT' LIMIT 1" | xargs)
...
METRICS_COUNT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \
  "SELECT count(*) FROM ${TABLE_METRICS} WHERE \"jobId\"='$JOB_ID'" | xargs)
...
ENRICHED_PROMPT=$(echo $METRICS_JSON | jq -r '.metadata.enrichedPrompt')
LEN=${#ENRICHED_PROMPT}
if [ "$LEN" -lt 10 ]; then log "❌ FAIL: Prompt too short ($LEN)"; exit 1; fi
log "✅ Assertion 3: Prompt Length ($LEN) > 10"

# 4. promptParts contains keys
STYLE=$(echo $METRICS_JSON | jq -r '.metadata.promptParts.style')
if [ "$STYLE" == "null" ] || [ -z "$STYLE" ]; then log "❌ FAIL: Missing style"; exit 1; fi
...
if [ "$LEDGER_TRACE" != "$TRACE_ID" ]; then log "❌ FAIL: Trace mismatch"; exit 1; fi
