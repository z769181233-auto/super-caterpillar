 ...
 */
function computeSignature(apiKey: string, secret: string, nonce: string, timestamp: string, body: string): string {
    const stringToSign = `${apiKey}${nonce}${timestamp}${body}`;
    return crypto.createHmac('sha256', secret).update(stringToSign).digest('hex');
}

async function request(method: string, path: string, bodyObj: any, customNonce?: string, customTimestamp?: string) {
