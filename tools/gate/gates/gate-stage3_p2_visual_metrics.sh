...
RUN_ID="$(uuidgen | tr EVID_DIR="docs/_evidence/p2_visual_metrics_${TS}"
mkdir -p "$EVID_DIR"
...
VALUES (
psql -d "${DATABASE_URL}" -c "INSERT INTO novel_sources (id, "projectId", "organizationId", "rawText", "fileName", "fileKey", "fileSize", "createdAt", "updatedAt") VALUES ('$NOVEL_SOURCE_ID', '$PROJECT_ID', '$ORG_ID', 'Dummy text for visual metrics', 'sample.txt', 'keys/sample', 100, NOW(), NOW()) ON CONFLICT (id) DO NOTHING;"
VALUES (
...
CE03_PAYLOAD=CE03_OUT=$(JOB_TYPE="CE03_VISUAL_DENSITY" SHOT_ID="$SHOT_ID" ORG_ID="$ORG_ID" USER_ID="$USER_ID" JOB_PAYLOAD="$CE03_PAYLOAD" \
...
CE04_PAYLOAD=CE04_OUT=$(JOB_TYPE="CE04_VISUAL_ENRICHMENT" SHOT_ID="$SHOT_ID" ORG_ID="$ORG_ID" USER_ID="$USER_ID" JOB_PAYLOAD="$CE04_PAYLOAD" \
  npx ts-node -P apps/api/tsconfig.json apps/api/src/scripts/p2_trigger.ts 2>&1)
...
wait_for_job() {
    ...
    local type=$2
    for i in {1..30}; do
        STATUS=$(psql -d scu -t -c "SELECT status FROM shot_jobs WHERE id=        log "   [$type] Status: $STATUS" # $gate$
        if [ "$STATUS" == "SUCCEEDED" ]; then return 0; fi
        if [ "$STATUS" == "FAILED" ]; then
            log "❌ [$type] Job Failed!"
            psql -d scu -c "SELECT \"lastError\" FROM shot_jobs WHERE id=            return 1
        fi
        sleep 2
    done
    return 1
...
CE03_SCORE=$(psql -d scu -t -c "SELECT qm.\"visualDensityScore\" FROM quality_metrics qm JOIN shot_jobs sj ON sj.\"projectId\" = qm.\"projectId\" WHERE qm.\"projectId\"=log "   [CE03] Density Score: $CE03_SCORE (TraceID: $CE03_TRACE_ID)" # $gate$
if [ -n "$CE03_SCORE" ] && [ "$CE03_SCORE" != "null" ] && (( $(echo "$CE03_SCORE > 0" | bc -l) )); then
    log "✅ CE03 Valid (score: $CE03_SCORE > 0)"
else
    log "❌ CE03 Invalid: expected > 0, got     psql -d scu -c "SELECT * FROM quality_metrics WHERE \"projectId\"=    exit 1
fi

# CE04: Bind by projectId + traceId, assert > 0
CE04_QUALITY=$(psql -d scu -t -c "SELECT qm.\"enrichmentQuality\" FROM quality_metrics qm JOIN shot_jobs sj ON sj.\"projectId\" = qm.\"projectId\" WHERE qm.\"projectId\"=log "   [CE04] Enrichment Quality: $CE04_QUALITY (TraceID: $CE04_TRACE_ID)" # $gate$
if [ -n "$CE04_QUALITY" ] && [ "$CE04_QUALITY" != "null" ] && (( $(echo "$CE04_QUALITY > 0" | bc -l) )); then
    log "✅ CE04 Valid (quality: $CE04_QUALITY > 0)"
else
    log "❌ CE04 Invalid: expected > 0, got     psql -d scu -c "SELECT * FROM quality_metrics WHERE \"projectId\"=    exit 1
fi
...
API_METRICS_COUNT=$(echo "$INSIGHT_RESP" | jq log "   Found $API_METRICS_COUNT visual metrics via API."
...
psql -d scu -c "SELECT id, type, status FROM shot_jobs WHERE id IN (echo "$INSIGHT_RESP" | jq . > "$EVID_DIR/api_insight_proof.json" # $gate$
