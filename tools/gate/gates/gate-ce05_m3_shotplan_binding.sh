...
SNAPSHOT_ID=$(grep "SNAPSHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
SHOTPLANNING_ID=$(grep "SHOTPLANNING_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
SHOT_ID=$(grep "SHOT_ID=" "$EVID_DIR/trigger_output.txt" | cut -d= -f2 | tr -d '\r')
...
SNAPSHOT_VERSION=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "SELECT version FROM director_control_snapshots WHERE id='$SNAPSHOT_ID'" | xargs)
...
SP_ROW=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -c "
  SELECT director_control_snapshot_id, identity_lock_token
  FROM shot_plannings
  WHERE id='$SHOTPLANNING_ID'
")

SP_SNAPSHOT_ID=$(echo "$SP_ROW" | awk -F '|' '{print $1}' | xargs)
SP_IDENTITY_TOKEN=$(echo "$SP_ROW" | awk -F '|' '{print $2}' | xargs)
