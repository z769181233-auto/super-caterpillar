...
PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -v ON_ERROR_STOP=1 <<'SQL'
UPDATE shot_jobs
SET status = 'FAILED',
    "lastError" = 'gate-preflight-cancel',
    "updatedAt" = NOW()
WHERE type = 'CE07_MEMORY_UPDATE'
...
CE07_JOB_ROW=$(PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -t -A -c "
SELECT id || '|' || status || '|' || attempts || '|' || COALESCE(\"workerId\",'') || '|' || to_char(\"createdAt\",'YYYY-MM-DD HH24:MI:SS')
FROM shot_jobs
WHERE id='$CE07_JOB_ID';
")

CE07_JOB_STATUS=$(echo "$CE07_JOB_ROW" | awk -F'|' '{print $2}' | xargs)
CE07_JOB_ATTEMPTS=$(echo "$CE07_JOB_ROW" | awk -F'|' '{print $3}' | xargs)
CE07_JOB_WORKER_ID=$(echo "$CE07_JOB_ROW" | awk -F'|' '{print $4}' | xargs)
CE07_JOB_CREATED_AT=$(echo "$CE07_JOB_ROW" | awk -F'|' '{print $5}' | xargs)
...
for i in $(seq 1 60); do
  ...
  W=$(echo "$ROW" | awk -F'|' '{print $2}' | xargs)
  S=$(echo "$ROW" | awk -F'|' '{print $3}' | xargs)
  U=$(echo "$ROW" | awk -F'|' '{print $4}' | xargs)
...
for i in $(seq 1 $MAX_WAIT); do
    ...
    if echo "$PROBE" | grep -q "SUCCEEDED"; then
        A_TEMP=$(echo "$PROBE" | awk -F'|' '{print $2}' | xargs)
        if [ "$A_TEMP" -ge 2 ]; then
...
FINAL_STATUS=$(echo "$FINAL_ROW" | awk -F '|' '{print $1}' | xargs)
FINAL_ATTEMPTS=$(echo "$FINAL_ROW" | awk -F '|' '{print $2}' | xargs)
FINAL_LAST_ERROR=$(echo "$FINAL_ROW" | awk -F '|' '{print $3}' | xargs)
FINAL_WORKER_ID=$(echo "$FINAL_ROW" | awk -F '|' '{print $4}' | xargs)
...
LAST_ERROR_CLEAN=$(echo "$FINAL_LAST_ERROR" | tr -d '| \t\n\r')
if [ ! -z "$LAST_ERROR_CLEAN" ]; then
...
WORKER_ID_CLEAN=$(echo "$FINAL_WORKER_ID" | tr -d '| \t\n\r')
if [ ! -z "$WORKER_ID_CLEAN" ]; then
