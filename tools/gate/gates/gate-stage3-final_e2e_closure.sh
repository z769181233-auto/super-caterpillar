...
run_chain() {
  ...
  # Let's use SAME Business Objects (Project/Shot) but New Job IDs for Business Idempotency check, OR Re-submit same Job ID? 
  # Actually, "Ledger count = 1" per JobType usually means per Business Action.
  # Let's stick to "Same Business Objects, New Job IDs" and check if systems handle it gracefully (e.g. Asset reused if hash matches).
  # Wait, if New Job ID, Ledger will log a new entry unless we deduplicate by hash. 
  ...
  # So let's reuse Job IDs for the "Retry/Re-entrant" test case. 
  # BUT we have a chain.
  # Simplified approach: 
  ...
  # Our workers currently don't have "Skip if done" logic fully explicitly except via DB uniqueness. 
  # Actually, `processShotRenderJob` uses `upsert` for Asset. Ledger? `record*Billing` usually logs every time unless we have dedup logic.
  ...
  # Let's try Re-running Same Job IDs.
  ...
  if [ "$RUN_NAME" == "Run1" ]; then
     ...
     SCENE_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM scenes LIMIT 1" | xargs)
     SHOT_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM shots LIMIT 1" | xargs)
     
     EPISODE_ID=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT id FROM episodes LIMIT 1" | xargs)
     ...
     PGPASSWORD=postgres psql -h localhost -U postgres -d scu -c "
      ...
      ('$JOB_ID_CE06', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'CE06_NOVEL_PARSING', 10, 3, 'trace-ce06', '{\"novelId\": \"$PROJECT_ID\"}', NOW(), NOW()),
      ('$JOB_ID_CE03', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'CE03_VISUAL_DENSITY', 10, 3, 'trace-ce03', '{\"sceneId\": \"$SCENE_ID\"}', NOW(), NOW()),
      ('$JOB_ID_CE04', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'CE04_VISUAL_ENRICHMENT', 10, 3, 'trace-ce04', '{\"sceneId\": \"$SCENE_ID\"}', NOW(), NOW()),
      ('$JOB_ID_SHOT', '$ORG_ID', '$PROJECT_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', 'PENDING', 'SHOT_RENDER', 10, 3, 'trace-shot', '{\"shotId\": \"$SHOT_ID\", \"prompt\": \"Final Gate Shot\"}', NOW(), NOW());
     " > /dev/null
     ...
     PGPASSWORD=postgres psql -h localhost -U postgres -d scu -c "UPDATE shot_jobs SET status='PENDING', \"workerId\"=NULL, \"lease_until\"=NULL, \"locked_by\"=NULL, \"attempts\"=0, \"retryCount\"=0, \"lastError\"=NULL, \"updatedAt\"=NOW() WHERE id IN ('$JOB_ID_CE06', '$JOB_ID_CE03', '$JOB_ID_CE04', '$JOB_ID_SHOT');" > /dev/null
  fi
...
wait_for_job() {
  ...
  while [ $MAX_RETRY -gt 0 ]; do
    STATUS=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT status FROM shot_jobs WHERE id='$JID'" | xargs)
...
# 1. Check Ledger Counts (Should be 1 because of DB Unique constraint on jobId+jobType, Worker will fail or ignore but Gate shouldn't see 2)
# Actually if Worker tries to insert duplicate Ledger with same JobID, it fails hard (Job Failed) OR stays 1 (if ignore).
# If Job Failed, wait_for_job would have crashed. So if we are here, jobs SUCCEEDED.
# This means Worker handled "Ledger already exists" gracefully OR didn't try to insert.
# Let's verify counts are exactly 1.

check_ledger() {
  local JID=$1
  local TYPE=$2
  COUNT=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c "SELECT count(*) FROM ${TABLE_LEDGER} WHERE \"jobId\"='$JID'" | xargs)
  if [ "$COUNT" != "1" ]; then log "❌ FAIL: Ledger Count for $TYPE is $COUNT (Expected 1)"; exit 1; fi
  log "✅ Ledger $TYPE: 1"
}
...
TRACE_VAL=$(PGPASSWORD=postgres psql -h localhost -U postgres -d scu -t -c \
  "SELECT \"traceId\" FROM ${TABLE_LEDGER} WHERE \"jobId\"='$JOB_ID_SHOT'" | xargs)
if [ "$TRACE_VAL" != "trace-shot" ]; then log "❌ FAIL: Trace Mismatch ($TRACE_VAL)"; exit 1; fi
log "✅ Trace ID Valid"
