...
SSOT_ENGINES=$(grep -E '^\| `[a-z0-9_]+`' "$SSOT_FILE" | sed -E 's/^\| `([a-z0-9_]+)`.*/\1/' | sort -u)
echo "   Registered engines: $(echo $SSOT_ENGINES | tr '\n' ' ')"
...
# 提取 REAL 引擎及其封印 Tag
while IFS='|' read -r _ engine_key _ status _ _ _ seal_tag _; do
    # 清理空格
    engine_key=$(echo "$engine_key" | xargs | sed 's/`//g')
    status=$(echo "$status" | xargs)
    seal_tag=$(echo "$seal_tag" | xargs | sed 's/`//g')
    ...
    if [ -n "$STUB_FOUND" ]; then
        ...
        if [[ "$seal_tag" == *"_sealed_"* ]]; then
            echo "❌ SEALED REAL engine '$engine_key' contains STUB features!"
            echo "   Seal Tag: $seal_tag"
