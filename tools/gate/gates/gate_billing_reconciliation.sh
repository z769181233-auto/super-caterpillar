...
DUPES=$(PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d scu -t -c "SELECT COUNT(*) FROM (SELECT \"tenantId\", \"traceId\", \"itemType\", \"itemId\", \"chargeCode\", COUNT(*) FROM billing_ledgers GROUP BY \"tenantId\", \"traceId\", \"itemType\", \"itemId\", \"chargeCode\" HAVING COUNT(*) > 1) AS dupes;")
...
FAILED_POSTED=$(PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d scu -t -c "SELECT COUNT(*) FROM billing_ledgers bl INNER JOIN shot_jobs sj ON bl.\"traceId\" = sj.id WHERE sj.status = 'FAILED' AND bl.status = 'POSTED';")
