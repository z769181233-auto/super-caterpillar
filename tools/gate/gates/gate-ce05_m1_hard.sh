...
PGPASSWORD="${POSTGRES_PASSWORD:-postgres}" psql -h "${POSTGRES_HOST:-localhost}" -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-scu}" -c "INSERT INTO shot_jobs (id, status, type, \"projectId\", \"organizationId\", \"episodeId\", \"sceneId\", \"shotId\", \"traceId\", payload, \"maxRetry\", attempts, priority, \"updatedAt\") VALUES ('$JOB_ID', 'PENDING', 'CE05_DIRECTOR_CONTROL', '$PROJECT_ID', '$ORG_ID', '$EPISODE_ID', '$SCENE_ID', '$SHOT_ID', '$TRACE_ID', '{\"projectId\": \"$PROJECT_ID\", \"traceId\": \"$TRACE_ID\"}', 3, 0, 100, NOW())"
