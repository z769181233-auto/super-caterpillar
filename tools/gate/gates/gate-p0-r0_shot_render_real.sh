...
# 从 seed 输出解析 ID
SHOT_ID=$(echo "$SEED_OUTPUT" | grep -o '"shotId":"[^"]*"' | cut -d'"' -f4)
PROJECT_ID=$(echo "$SEED_OUTPUT" | grep -o '"projectId":"[^"]*"' | cut -d'"' -f4)
ORG_ID=$(echo "$SEED_OUTPUT" | grep -o '"orgId":"[^"]*"' | cut -d'"' -f4)
TRACE_ID="trace-p0r0-$(date +%s)"
...
# 使用 ts-node 直接调用引擎
RENDER_SCRIPT=$(cat <<'EOF'
const { runShotRenderSDXL } = require('./packages/engines/shot_render/real/sdxl.adapter');
...
# 从输出中提取纯 JSON
RENDER_JSON=$(sed -n '/__RESULT_START__/,/__RESULT_END__/p' "$RENDER_OUTPUT" | grep -v '__RESULT')
echo "$RENDER_JSON" > docs/_evidence/P0_R0_SHOT_RENDER_RESULT.json
...
# 5.1 图片文件存在
ASSET_URI=$(echo "$RENDER_JSON" | jq -r '.asset.uri')
...
SHA256=$(echo "$RENDER_JSON" | jq -r '.asset.sha256')
...
# 5.5 检查 audit_trail
ENGINE_KEY=$(echo "$RENDER_JSON" | jq -r '.audit_trail.engineKey')
if [ "$ENGINE_KEY" != "shot_render_real" ]; then
    echo "❌ FAIL: Expected engineKey 'shot_render_real', got '$ENGINE_KEY'"
    exit 1
fi
echo "✅ Assertion 5: Engine key is 'shot_render_real'"

# 5.6 检查 billing_usage
GPU_SECONDS=$(echo "$RENDER_JSON" | jq -r '.billing_usage.gpuSeconds')
if [ "$GPU_SECONDS" == "0" ] || [ "$GPU_SECONDS" == "null" ]; then
