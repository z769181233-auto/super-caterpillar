...
RESULT=$(PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d scu -t -c "
  INSERT INTO billing_ledger (id, tenant_id, trace_id, item_type, item_id, charge_code, amount, status)
...
# Cleanup
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d scu -c "DELETE FROM billing_ledger WHERE id LIKE 'test-dup-%';" > /dev/null
...
# Cleanup
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d scu -c "DELETE FROM billing_ledger WHERE id LIKE 'test-neg-%';" > /dev/null
