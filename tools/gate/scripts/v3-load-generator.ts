...
async function pollJob(jobId: string, endpointType: 'story' | 'shot', timeoutMs: number = 180000): Promise<any> {
...
async function fetchSceneId(projectId: string, traceId: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= 20; attempt++) {
        try {
            ...
            if (output) {
                ...
                if (ids.length > 1) {
                    console.log(`[Metric] MULTI_SCENE_MATCH for Trace=${traceId}, Proj=${projectId}. Using latest.`);
                }
    ...
    throw new Error(`fetchSceneId FAILED after 100s. Trace=${traceId}, Proj=${projectId}\nDetails: ${lastError}`);
}
...
async function runScenario(index: number, semaphore: Semaphore) {
    ...
    try {
        ...
        metrics.push({
            traceId: 'init_fail', storyJobId: '', startTime: Date.now(),
            parseEndTime: 0, shotGenEndTime: 0, totalDuration: 0,
            status: 'FAILED', error: 'Project Init Failed'
        });
        semaphore.release();
        return;
    }
    ...
    try {
        ...
        if (parseJob.status !== 'SUCCEEDED') throw new Error(`Parse failed: ${parseJob.error?.message}`);
        ...
        if (shotJob.status !== 'SUCCEEDED') throw new Error(`ShotGen failed: ${shotJob.error?.message}`);
        ...
        metric.assetId = preview.asset_id || (preview.shots_count > 0 ? `shots_v3_${preview.shots_count}` : null);
...
async function main() {
    ...
    console.log(`Starting Load Generator: Concurrency=${CONCURRENCY}, Targets=${TARGET_JOBS}, Output=${EVIDENCE_DIR}`);
