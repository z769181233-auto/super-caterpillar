...
async function main() {
  ...
  try {
    ...
    console.error(
      `[MockWorker-${WORKER_SUFFIX}] Registration FAILED: ${e.response?.status} ${JSON.stringify(e.response?.data)}`
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < 200; i++) {
    try {
      ...
      await axios.post(`${API_URL}/api/workers/${workerId}/heartbeat`, hHb.data, {
        headers: hHb.headers,
      });

      // Next Job
      const nHb = generateHmacHeaders({}, workerId);
      const nRes = await axios.post(`${API_URL}/api/workers/${workerId}/jobs/next`, nHb.data, {
        headers: nHb.headers,
      });
      ...
      if (job) {
        ...
        const compPayload = {
          status: 'SUCCEEDED',
          result: { output: { storageKey: `mock-${job.id}.png` } },
          workerId,
        };
        const cHb = generateHmacHeaders(compPayload, workerId);
        await axios.post(`${API_URL}/api/jobs/${job.id}/complete`, cHb.data, {
          headers: cHb.headers,
        });
      ...
      console.error(
        `[MockWorker-${WORKER_SUFFIX}] Error: ${e.response?.status} ${JSON.stringify(e.response?.data)}`
      );
