...
class MockApiClient extends ApiClient {
    constructor() { super('http://mock', 'token'); }
    async postAuditLog(data: any) { return { success: true }; }
}
...
async function run() {
    ...
    try {
                    ...
                    if (prop === 'auditLog') {
                        return {
                            create: async (args: any) => {
                                ...
                                const safeData = { ...args.data, orgId: undefined, userId: undefined, apiKeyId: undefined };
        ...
        if (!fs.existsSync(path.dirname(mockFile))) fs.mkdirSync(path.dirname(mockFile), { recursive: true });
        ...
        await processShotRenderJob({
            ...
            job: { id: job1Id, shotId: shot1Id, payload: { pipelineRunId: 'run-c', characterIds: [charId] } } as any,
            apiClient: mockApi
        });
        ...
        await processShotRenderJob({
            ...
            job: { id: job2Id, shotId: shot2Id, payload: { pipelineRunId: 'run-c', characterIds: [charId] } } as any,
            apiClient: mockApi
        });
