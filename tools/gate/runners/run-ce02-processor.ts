    ...
    try {
        ...
        (mockApi as any).request = async (method: string, url: string, data: any) => {
            if (url.includes('/api/_internal/engine/invoke')) {
                ...
                const dummyImgPath = path.resolve(process.cwd(), 'apps/workers/.runtime/assets/dummy_gate.png');
                if (!fs.existsSync(path.dirname(dummyImgPath))) fs.mkdirSync(path.dirname(dummyImgPath), { recursive: true });
        ...
        // Assert DB
        const anchor = await prisma.characterIdentityAnchor.findUnique({ where: { id: result.anchor.id } });
        ...
        const result2 = await processIdentityLockJob({
            prisma,
            job: { ...mockJob, id: mockJob.id + '_2', payload: { ...mockJob.payload, forceRebuild: true } } as any,
            apiClient: mockApi,
            workerId: 'gate-worker-2'
        });

        const anchor2 = await prisma.characterIdentityAnchor.findUnique({ where: { id: result2.anchor.id } });
