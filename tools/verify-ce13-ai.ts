...
async function runVerification() {
    ...
    for (const tc of testCases) {
        ...
        console.log(`[Rule] ${tc.name}: Pacing=${output.pacing_score.toFixed(2)} Tension=${output.tension_level}`);
    }
    ...
    for (const tc of testCases) {
        try {
            const output = await ce13RealEngine({ structured_text: tc.text });
            console.log(`[AI] ${tc.name}: Pacing=${output.pacing_score.toFixed(2)} Tension=${output.tension_level}`);
            console.log(`    Emotional: ${output.emotional_intensity.toFixed(2)} Indicators: ${JSON.stringify(output.indicators)}`);
        } catch (e: any) {
