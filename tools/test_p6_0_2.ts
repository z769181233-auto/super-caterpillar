...
async function runTest(validSig: boolean, validHashContent: boolean) {
    ...
    const headerSha = validHashContent ? realSha : crypto.createHash('sha256').update('FAKE').digest('hex');
    ...
    const signature = validSig ? validSignature : 'deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef';
