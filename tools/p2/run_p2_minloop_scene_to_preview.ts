...
function createDummyPng(name: string, color: string): string {
  ...
  try {
    execSync(`ffmpeg -f lavfi -i color=c=${color}:s=100x100 -frames:v 1 -y "${fpath}"`, {
      stdio: 'ignore',
    });
  } catch (e) {
...
async function main() {
  ...
  const user = await prisma.user.create({
    data: { email: `minloop_${suffix}@example.com`, passwordHash: 'x' },
  });
  const org = await prisma.organization.create({
    data: { name: `MinloopOrg_${suffix}`, ownerId: user.id },
  });
  const project = await prisma.project.create({
    data: { name: `MinloopProj_${suffix}`, organizationId: org.id, ownerId: user.id },
  });
  ...
  try {
    ...
    if (sceneRes.status !== 'SUCCESS')
      throw new Error(`SceneComp Failed: ${JSON.stringify(sceneRes)}`);
    ...
    if (previewRes.status !== 'SUCCESS')
      throw new Error(`ShotPreview Failed: ${JSON.stringify(previewRes)}`);
