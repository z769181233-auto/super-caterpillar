...
async function main() {
  ...
  const user = await prisma.user.create({
    data: { email: `minloop_${suffix}@test.com`, passwordHash: 'x' },
  });
  ...
  const job1 = await prisma.shotJob.create({
    data: {
  ...
  try {
    ...
    const totalCost =
      costs1.reduce((a, c) => a + c.costAmount, 0) +
    ...
    await prisma.costLedger.deleteMany({
      where: { OR: [{ jobId: job1.id }, { jobId: job2.id }, { jobId: job3.id }] },
    });
