...
async function main() {
    ...
    const user = await prisma.user.create({ data: { email: `emo_${suffix}@test.com`, passwordHash: 'x' } });
    const org = await prisma.organization.create({ data: { name: `EmoOrg_${suffix}`, ownerId: user.id } });
    const project = await prisma.project.create({ data: { name: `EmoProj_${suffix}`, organizationId: org.id, ownerId: user.id } });
    ...
    try {
        ...
        if (res1.output.source !== 'generated') throw new Error(`Expected generated, got ${res1.output.source}`);
        ...
        if (res2.output.primary !== 'sadness') throw new Error(`Expected sadness, got ${res2.output.primary}`);
