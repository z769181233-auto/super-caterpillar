...
async function main() {
    ...
    if (!fs.existsSync(path.dirname(dummyImageAbs))) fs.mkdirSync(path.dirname(dummyImageAbs), { recursive: true });
    // Minimal 1x1 black PNG
    const onePixelPng = Buffer.from('89504e470d0a1a0a0000000d49484452000000010000000108000000003a7e920b0000000a4944415408d76360000000020001e221bc330000000049454e44ae426082', 'hex');
    ...
    for (const id of shotIds) {
        ...
        if (!fs.existsSync(path.dirname(framesTxtAbs))) fs.mkdirSync(path.dirname(framesTxtAbs), { recursive: true });
    ...
    if (!fs.existsSync(path.dirname(timelineAbsPath))) fs.mkdirSync(path.dirname(timelineAbsPath), { recursive: true });
    ...
    if (!securedAsset?.storageKey.includes('secure/')) throw new Error('Asset storageKey not updated to secure path');
    ...
    if (!fs.existsSync(path.dirname(legacyAssetAbs))) fs.mkdirSync(path.dirname(legacyAssetAbs), { recursive: true });
    ...
    if (!securedShotAsset?.storageKey.includes('secure/')) throw new Error('Legacy Shot Asset not secured');
