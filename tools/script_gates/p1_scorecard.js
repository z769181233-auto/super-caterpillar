...
function calculateScore(filePath) {
  ...
  const beatStats = content.beats.map((beat, bIdx) => {
    ...
    beat.shotLines.forEach((shot) => {
      ...
      if (shot.dialogue && shot.dialogue.trim().length > 0) {
        // Check Prop Usage
        if (shot.usesProp !== true && !shot.propAction) {
          // Support bool or string per user request options
          propFail = true;
        }
        // Parallel Task
    ...
    return {
      ...
      moves: beat.shotLines.map(
        (s) => `${s.poseId || 'NA'}+${s.motionId || 'NA'}+${s.cameraMoveId || 'NA'}`
      ),
    };
  });
  ...
  if (densityDeductions.length === 0) {
    ...
    const totalDed = Math.min(
      WEIGHTS.DENSITY,
      densityDeductions.reduce((s, d) => s + d.penalty, 0)
    );
  ...
  if (totalShots >= SHOT_BUDGET_MIN && totalShots <= SHOT_BUDGET_MAX) {
    ...
    report.deductions.push({
      rule: 'BUDGET',
      penalty: WEIGHTS.BUDGET,
      msg: `Shot count ${totalShots} out of range [${SHOT_BUDGET_MIN}, ${SHOT_BUDGET_MAX}]`,
    });
  }
  ...
  if (validatedReversals >= 3) {
    ...
    report.deductions.push({
      rule: 'REVERSAL',
      penalty: WEIGHTS.REVERSAL,
      msg: `Validated Reversals ${validatedReversals} < 3`,
    });
    report.rewriteDirectives.push({
      target: 'Global',
      instruction: 'Add more reversal beats with evidence',
    });
  }
  ...
  if (validatedBigClimax >= 1 && validatedSmallClimax >= 4) {
    ...
    report.deductions.push({
      rule: 'CLIMAX',
      penalty: WEIGHTS.CLIMAX,
      msg: `Validated Climax: Big=${validatedBigClimax}, Small=${validatedSmallClimax}`,
    });
  ...
  if (beatsWithPropFail.length === 0) {
    ...
    report.deductions.push({
      rule: 'PROP_USAGE',
      penalty: WEIGHTS.PROP,
      msg: `${beatsWithPropFail.length} beats with dialogue missing usesProp=true`,
    });
    beatsWithPropFail.forEach((b) =>
      report.rewriteDirectives.push({
        target: b.id,
        instruction: 'Set usesProp=true for dialogue shots',
      })
    );
  }
  ...
  if (beatsWithZhanzhuang.length === 0) {
    ...
    report.deductions.push({
      rule: 'PARALLEL_COVERAGE',
      penalty: WEIGHTS.PARALLEL,
      msg: `${beatsWithZhanzhuang.length} beats having shots with dialogue but no parallelTask`,
    });
  }
  ...
  if (beatsNoTrans.length === 0) {
    ...
    report.deductions.push({
      rule: 'TRANSITION_COVERAGE',
      penalty: WEIGHTS.TRANSITION,
      msg: `${beatsNoTrans.length} beats missing transitionTag`,
    });
  }
  ...
  if (totalShots > 0) {
      ...
      if (currentRun > UNIQUENESS_THRESHOLDS.MAX_RUN) {
        ...
        if (!report.rewriteDirectives.some((d) => d.target === currentBeatId)) {
          report.rewriteDirectives.push({
            ...
            instruction:
              'Action Chain is repetitive. Replace action motifs or change subject to break sequence.',
          });
        }
      }
    ...
    if (actionRatio < UNIQUENESS_THRESHOLDS.ACTION_RATIO) {
      ...
      report.deductions.push({
        rule: 'UNIQ_ACTION',
        penalty: 0.4,
        msg: `Action variability ${actionRatio.toFixed(2)} < ${UNIQUENESS_THRESHOLDS.ACTION_RATIO}`,
      });
      isHardFail = true;
    }
    if (subjectRatio < UNIQUENESS_THRESHOLDS.SUBJECT_RATIO) {
      ...
      report.deductions.push({
        rule: 'UNIQ_SUBJECT',
        penalty: 0.3,
        msg: `Subject diversity ${subjectRatio.toFixed(2)} < ${UNIQUENESS_THRESHOLDS.SUBJECT_RATIO}`,
      });
      isHardFail = true;
    }
    if (moveRatio < UNIQUENESS_THRESHOLDS.MOVE_RATIO) {
      ...
      report.deductions.push({
        rule: 'UNIQ_MOVE_ID',
        penalty: 1.0,
        msg: `Gesture combination var ${moveRatio.toFixed(2)} < ${UNIQUENESS_THRESHOLDS.MOVE_RATIO} (HARD FAIL)`,
      });
      isHardFail = true;
    }
    if (maxMoveRun > UNIQUENESS_THRESHOLDS.MAX_RUN) {
      ...
      report.deductions.push({
        rule: 'MAX_MOVE_RUN',
        penalty: 0.5,
        msg: `Consecutive repeated (Pose+Motion) ${maxMoveRun} > ${UNIQUENESS_THRESHOLDS.MAX_RUN}`,
      });
      isHardFail = true;
    }
    ...
    if (isHardFail) {
      report.rewriteDirectives.push({
        target: 'Global',
        instruction:
          'CRITICAL: Diversity threshold hard failed. Script rejected due to lack of action/subject variety.',
      });
    }
    if (uniqPenalty > 0 && report.rewriteDirectives.length === 0) {
      report.rewriteDirectives.push({
        target: 'Global',
        instruction:
          'Increase action chain diversity and subject variety. Avoid copy-pasting shots.',
      });
    }
  }
...
if (report.rewriteDirectives.length > 0) {
  fs.writeFileSync(
    path.join(EVI_PATH, 'rewrite_directives.json'),
    JSON.stringify(report.rewriteDirectives, null, 2)
  );
}
