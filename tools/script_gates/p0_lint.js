...
const SFX_MAP = JSON.parse(
  fs.readFileSync(path.join(ASSETS_DIR, 'sfx_catalog.json'), 'utf8')
).reduce((acc, cur) => {
  acc[cur.id] = cur.name;
  return acc;
}, {});
...
const PLACEHOLDER_REGEX =
  /\[占位|TODO|TBD|\[核心道具\]|\[视觉奇观\]|\[待填充|\[待确定|\[主角\]|\[起势姿态\]|\[落点姿态\]|\[占位音效\]|\[待填充台词\]/;
...
function checkPlaceholder(val, path, errors, beatId, shotId) {
  if (typeof val === 'string' && PLACEHOLDER_REGEX.test(val)) {
    ...
    Object.keys(val).forEach((key) =>
      checkPlaceholder(val[key], `${path}.${key}`, errors, beatId, shotId)
    );
  }
}
...
function lintShotSpec(filePath) {
  ...
  content.beats.forEach((beat, bIdx) => {
    ...
    if (beat.beatGoal) {
      ...
      if (!hasVerb) {
        errors.push({
          ruleId: 'BEAT_GOAL_VERB',
          beatId,
          message: `beatGoal must contain a strong verb (e.g., 逼, 抢, 摔). Found: "${beat.beatGoal}"`,
        });
      }
      ...
      FORBIDDEN_ABSTRACT_WORDS.forEach((word) => {
        if (beat.beatGoal.includes(word)) {
          errors.push({
            ruleId: 'NO_ABSTRACT',
            beatId,
            message: `beatGoal contains abstract word: ${word}`,
          });
        }
      });
    }
    ...
    if (!beat.thirdActorProp || beat.thirdActorProp.trim() === '') {
      errors.push({
        ruleId: 'PROP_REQ',
        beatId,
        message: 'thirdActorProp must be present and non-empty',
      });
    } else {
      FORBIDDEN_PROP_WORDS.forEach((word) => {
        if (beat.thirdActorProp.includes(word)) {
          errors.push({
            ruleId: 'PROP_SPECIFIC',
            beatId,
            message: `thirdActorProp contains generic word: "${word}". Must be specific.`,
          });
        }
      });
    }
    ...
    if (beat.transitionTag && !VALID_TRANSITIONS.includes(beat.transitionTag)) {
      errors.push({
        ruleId: 'TRANSITION_ENUM',
        beatId,
        message: `Invalid transitionTag: "${beat.transitionTag}". Allowed: ${VALID_TRANSITIONS.join(', ')}`,
      });
    }
    ...
    if (beat.reversalTag === true) {
      if (!beat.reversalEvidence) {
        errors.push({
          ruleId: 'REVERSAL_EVIDENCE_REQ',
          beatId,
          message: 'reversalTag=true but reversalEvidence missing',
        });
      }
    }
    if (beat.climaxTag && beat.climaxTag !== null) {
      if (!beat.climaxEvidence) {
        errors.push({
          ruleId: 'CLIMAX_EVIDENCE_REQ',
          beatId,
          message: `climaxTag=${beat.climaxTag} but climaxEvidence missing`,
        });
      }
    }
    ...
    if (beat.locationId && !VALID_LOC_IDS.has(beat.locationId)) {
      errors.push({
        ruleId: 'ASSET_ID_VALID',
        beatId,
        message: `Invalid locationId: "${beat.locationId}". Not found in locations.json.`,
      });
    } else if (!beat.locationId) {
      errors.push({
        ruleId: 'ASSET_ID_MISSING',
        beatId,
        message: 'Required locationId is missing.',
      });
    }
    ...
    if (beat.thirdActorPropId && !VALID_PROP_IDS.has(beat.thirdActorPropId)) {
      errors.push({
        ruleId: 'ASSET_ID_VALID',
        beatId,
        message: `Invalid thirdActorPropId: "${beat.thirdActorPropId}". Not found in props.json.`,
      });
    } else if (!beat.thirdActorPropId) {
      errors.push({
        ruleId: 'ASSET_ID_MISSING',
        beatId,
        message: 'Required thirdActorPropId is missing.',
      });
    }
      ...
      beat.sfxIds.forEach((id, idx) => {
        if (!VALID_SFX_IDS.has(id)) {
          errors.push({
            ruleId: 'ASSET_ID_VALID',
            beatId,
            message: `Invalid sfxId: "${id}". Not found in sfx_catalog.json.`,
          });
    ...
    if (beat.estDurationSec === undefined || beat.estDurationSec === null) {
      errors.push({
        ruleId: 'DURATION_BEAT_REQ',
        beatId,
        message: 'Missing estDurationSec in beat',
      });
    }
      ...
      beat.shotLines.forEach((shot, sIdx) => {
        ...
        if (shot.actionChain) {
          ...
          if (missingMarkers.length > 0) {
            errors.push({
              ruleId: 'ACTION_CHAIN_STRUCT',
              beatId,
              shotId,
              message: `Missing ActionChain markers: ${missingMarkers.join(', ')}. Must have (起势)(过程)(落点)(反应)`,
            });
          }
          ...
          FORBIDDEN_ABSTRACT_WORDS.forEach((word) => {
            if (shot.actionChain.includes(word)) {
              errors.push({
                ...
                beatId,
                shotId,
                message: `actionChain contains abstract word: "${word}"`,
              });
            }
          });
        }
        ...
        if (shot.dialogue && shot.dialogue.trim().length > 0) {
          ...
          if (shot.dialogue.length > 15) {
            errors.push({
              ...
              beatId,
              shotId,
              message: `Dialogue too long (${shot.dialogue.length} > 15).`,
            });
          }
          // Check 2: Parallel Task
          if (!shot.parallelTask || shot.parallelTask.trim().length === 0) {
            errors.push({
              ruleId: 'NO_ZHANZHUANG',
              beatId,
              shotId,
              message: 'Dialogue exists but parallelTask is empty (Standing Still Ban).',
            });
          }
          // D2: Dialogue must have Pose & Motion
          if (!shot.poseId || !shot.motionId) {
            errors.push({
              ruleId: 'DIALOGUE_ACTION_REQ',
              beatId,
              shotId,
              message: 'Dialogue exists but poseId or motionId is missing (D2 Rule).',
            });
          }
        }
        ...
        if (shot.characterId && !VALID_CHAR_IDS.has(shot.characterId)) {
          errors.push({
            ...
            beatId,
            shotId,
            message: `Invalid characterId: "${shot.characterId}". Not found in characters.json.`,
          });
          ...
          errors.push({
            ...
            beatId,
            shotId,
            message: 'Required characterId is missing in shot.',
          });
        }
        ...
        if (shot.poseId) {
          if (!VALID_POSE_IDS.has(shot.poseId)) {
            errors.push({
              ...
              beatId,
              shotId,
              message: `Invalid poseId: "${shot.poseId}". Not found in poses.json.`,
            });
          } else if (!RENDERABLE_POSES.has(shot.poseId)) {
            errors.push({
              ruleId: 'RENDER_RESOLVE_FAIL',
              beatId,
              shotId,
              message: `poseId "${shot.poseId}" exists but has no entry in render_map.json.`,
            });
          }
        }
        if (shot.motionId) {
          if (!VALID_MOTION_IDS.has(shot.motionId)) {
            errors.push({
              ...
              beatId,
              shotId,
              message: `Invalid motionId: "${shot.motionId}". Not found in motions.json.`,
            });
            ...
            errors.push({
              ...
              beatId,
              shotId,
              message: `motionId "${shot.motionId}" exists but has no entry in render_map.json.`,
            });
          }
        }
        if (shot.cameraMoveId) {
          if (!VALID_CAM_IDS.has(shot.cameraMoveId)) {
            errors.push({
              ...
              beatId,
              shotId,
              message: `Invalid cameraMoveId: "${shot.cameraMoveId}". Not found in camera_moves.json.`,
            });
            ...
            errors.push({
              ...
              beatId,
              shotId,
              message: `cameraMoveId "${shot.cameraMoveId}" exists but has no entry in render_map.json.`,
            });
          }
        }
        ...
        if (shot.dialogue && shot.dialogue.trim().length > 0) {
          if (shot.poseId && shot.motionId && shot.cameraMoveId) {
            ...
            errors.push({
              ...
              shotId,
              message:
                'Dialogue exists but structural action IDs (pose/motion/camera) are incomplete.',
            });
          }
        }
  ...
  if (content.episodeMeta && content.episodeMeta.durationSec) {
    ...
    errors.push({
      ruleId: 'DURATION_META_MISSING',
      message: 'episodeMeta.durationSec is missing or zero at root',
    });
  }
