...
function lintStory(filePath) {
    ...
    if (!data.obstacles || !Array.isArray(data.obstacles) || data.obstacles.length < 4) {
        errors.push({ ruleId: "STORY_OBSTACLES_COUNT", message: `Obstacles count ${data.obstacles ? data.obstacles.length : 0} < 4` });
    }

    if (!data.turns || !Array.isArray(data.turns) || data.turns.length < 3 || data.turns.length > 4) {
        errors.push({ ruleId: "STORY_TURNS_COUNT", message: `Turns count ${data.turns ? data.turns.length : 0} out of range [3, 4]` });
    }
