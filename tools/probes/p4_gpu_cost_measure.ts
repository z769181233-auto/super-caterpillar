...
async function measureGPUCost() {
    ...
    try {
      const res = await axios.post(
        'https://api.replicate.com/v1/predictions',
        {
          ...
          input: {
            prompt:
              'A beautiful majestic cyber caterpillar in an epic sci-fi neon city, 4k, masterpiece, highly detailed',
            width: 1024,
            height: 1024,
            num_outputs: 1,
            scheduler: 'K_EULER',
      ...
      countFailed++;
      console.error(
        `Iter ${i + 1} API Error (${status || 'UNKNOWN'}):`,
        error.response?.data || error.message
      );
    }
  }

  const isValidSampleSet =
    countFailed === 0 && count429 === 0 && count401 === 0 && predictTimes.length === RUN_COUNT;
  ...
  console.log(
    `Sample Set Valid: ${isValidSampleSet ? 'YES (SEAL CANDIDATE)' : 'NO (INVALID_SAMPLE_SET: ' + invalidReason + ')'}`
  );
  console.log(`----------------------------------`);
  if (isValidSampleSet) {
    ...
    console.log(
      `Threshold: Queue Delay AVG <= 2.0s  | Current: ${avgQueueDelay <= 2.0 ? 'PASS' : 'WARN - HIGH DELAY'}`
    );
  ...
  fs.writeFileSync(
    'p4_gpu_cost_measure_result.log',
    JSON.stringify(
      {
        attempted: RUN_COUNT,
