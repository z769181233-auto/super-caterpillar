...
async function main() {
    if (!fs.existsSync(SCRIPT_JSON_PATH)) {
        console.error(`Error: Script JSON found at ${SCRIPT_JSON_PATH}. Run compile_video_script.ts first.`);
        process.exit(1);
    }
    ...
    for (const scene of scriptData) {
        for (const shot of scene.shots) {
            console.log(`[Trigger] Enqueuing SHOT_RENDER for Shot Index ${shot.index} (Scene ID: ${shot.sceneId})`);
