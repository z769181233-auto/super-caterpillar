...
import { PrismaClient, AssetOwnerType, AssetType } from '../../packages/database/src/generated/prisma';
...
const TEMPLATE_FILE = path.join(process.cwd(), 'packages/engines/shot_render/providers/templates/ce02_identity_triview.json');
...
async function main() {
        ...
        for (const view of ['front', 'side', 'back']) {
            ...
            if (!fs.existsSync(outFile)) {
                ...
                try {
                    ...
                    console.warn(`      FAILED to generate ${view} view for ${name}: ${e.message}. Using stub image.`);
    ...
    if (!fs.existsSync(path.dirname(artifactBible))) fs.mkdirSync(path.dirname(artifactBible), { recursive: true });
