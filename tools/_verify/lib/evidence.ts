...
export function safeSummary(input: any, opts?: { maxDepth?: number; maxArray?: number; maxString?: number }) {
    ...
    function walk(v: any, depth: number): any {
        if (v == null) return v;
        if (typeof v === "string") return v.length > maxString ? v.slice(0, maxString) + `…(truncated:${v.length})` : v;
