...
async function main() {
    ...
    try {
        await prisma.user.create({ data: { id: userId, email: `test-${userId}@example.com`, passwordHash: 'hash' } });
        ...
        for (let i = 0; i < shotIds.length; i++) {
            ...
            execSync(`ffmpeg -f lavfi -i color=c=red:s=1280x720 -frames:v 1 -update 1 -y "${imgPath}" 2>/dev/null`);

            const framesContent = Array.from({ length: 24 }).map(() => `file '${imgPath}'\nduration 0.041666`).join('\n');
        ...
        if (!fs.existsSync(path.dirname(bgmPath))) fs.mkdirSync(path.dirname(bgmPath), { recursive: true });
        ...
        await prisma.shot.update({ where: { id: shotIds[0] }, data: { params: { voiceAssetStorageKey: voiceKey1 } } });

        const composeJob1 = await submitJob('PIPELINE_TIMELINE_COMPOSE', { sceneId, pipelineRunId: pipelineRunId + '_1' }, orgId, projectId, episodeId, sceneId, shotIds[0]);
        ...
        const renderJob1 = await submitJob('TIMELINE_RENDER', {
            timelineStorageKey: res1.timelineStorageKey,
            pipelineRunId: pipelineRunId + '_1'
        }, orgId, projectId, episodeId, sceneId, shotIds[0]);
        ...
        const composeJob2 = await submitJob('PIPELINE_TIMELINE_COMPOSE', {
            sceneId,
        ...
        if (timeline2.audio.tracks.length < 2) console.error('❌ Case 2 Failed: Expected >1 tracks', timeline2.audio.tracks);
        else console.log(`✅ Case 2 Compose Verified: ${timeline2.audio.tracks.length} Tracks found`);

        const renderJob2 = await submitJob('TIMELINE_RENDER', {
...
async function submitJob(type: string, payload: any, orgId: string, projectId: string, episodeId: string, sceneId: string, shotId: string) {
    const id = uuidv4();
...
async function verifyOutput(key: string) {
    ...
    const outp = execSync(`ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${fullPath}"`).toString().trim();
