...
async function main() {
    ...
    try {
        ...
        if (!user) {
            throw new Error(`❌ User ${userId} NOT FOUND in DB ${process.env.DATABASE_URL}. Connection mismatch!`);
        } else {
        ...
        if (!shot) {
            season = await prisma.season.findFirst({ where: { projectId: project.id } })
                || await prisma.season.create({ data: { projectId: project.id, index: 1, title: 'S1' } });
            episode = await prisma.episode.findFirst({ where: { seasonId: season.id } })
                || await prisma.episode.create({ data: { seasonId: season.id, index: 1, name: 'E1' } });
            scene = await prisma.scene.create({ data: { episodeId: episode.id, index: 1, title: 'SC1', summary: 'Bypass Gate' } });
            shot = await prisma.shot.create({ data: { sceneId: scene.id, index: 1, type: 'setup', params: {} } });
        } else {
        ...
        if (workerKey && workerSecret) {
            ...
            console.warn('⚠️ WORKER_API_KEY or WORKER_API_SECRET not found in env. Worker registration may fail.');
        }
        ...
        const fsRec = await prisma.season.create({ data: { projectId: failProject.id, index: 1, title: 'FS1' } });
        const feRec = await prisma.episode.create({ data: { seasonId: fsRec.id, index: 1, name: 'FE1' } });
        const fscRec = await prisma.scene.create({ data: { episodeId: feRec.id, index: 1, title: 'FSC1', summary: 'Fail' } });
        const fshotRec = await prisma.shot.create({ data: { sceneId: fscRec.id, index: 1, type: 'setup', params: {} } });
        ...
        try {
            await request('POST', `/shots/${fshotRec.id}/jobs`, {
                type: 'NOVEL_ANALYSIS',
                payload: {}
            }, newToken);
            ...
            if (err.status === 400 && err.response &&
                (JSON.stringify(err.response).includes('NOVEL_SOURCE_MISSING') || err.message.includes('NOVEL_SOURCE_MISSING'))) {
                console.log('✅ Fail Fast Case Passed (API correctly rejected with NOVEL_SOURCE_MISSING)');
            } else {
                throw new Error(`❌ API failed with unexpected error: ${err.message} ${JSON.stringify(err.response)}`);
            }
        }

        // 5. Happy Path
        console.log('\n🧪 Test Case B: Happy Path (Novel Import API)');
        const importRes = await request('POST', `/projects/${project.id}/novel/import`, {
            title: 'Test Novel',
        ...
        if (fs.existsSync(logPath)) {
            ...
            if (claimedIdx === -1) console.warn('⚠️ Could not find JOB_CLAIMED_SUCCESS in api.log (Might be rotated or missing)');
            ...
            if (claimedIdx !== -1 && succeededIdx !== -1) {
                if (claimedIdx < succeededIdx) {
                    console.log(`✅ Log Order Verified: CLAIMED (L${claimedIdx}) -> SUCCEEDED (L${succeededIdx})`);
                } else {
                    throw new Error(`❌ Log Order Violation! SUCCEEDED came before CLAIMED. This implies state machine regression.`);
                }
            }
        } else {
            console.warn('⚠️ api.log not found, checking skipped. (Verify file path matches execution context)');
        }
...
async function pollJob(jobId: string, token: string, targetStatus: string) {
    ...
    for (let i = 0; i < maxRetries; i++) {
        ...
        if (i % 5 === 0) console.log(`   Poll ${i + 1}: ${job.status} Binding: ${JSON.stringify(job.engineBinding)} LastError: ${JSON.stringify(job.lastError)}`);

        if (job.status === targetStatus) {
            if (targetStatus === 'FAILED') {
                const errStr = JSON.stringify(job);
                if (errStr.includes('NO_SOURCE_TEXT') || errStr.includes('未找到') || errStr.includes('Novel source') || errStr.includes('not found') ||
                    (job.lastError && (job.lastError.includes('NO_SOURCE_TEXT') || job.lastError.includes('未找到') || job.lastError.includes('Novel source')))) {
                    return;
                }
            } else {
                return;
