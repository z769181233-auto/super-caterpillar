...
async function main() {
  ...
  const email = process.env.SMOKE_USER_EMAIL || 'smoke@test.com';
  ...
  if (!user)
    throw new Error(
      `User ${email} not found under DATABASE_URL=${process.env.DATABASE_URL?.substring(0, 15)}...`
    );
  ...
  if (targetOrg) {
    ...
    if (membership) {
      ...
      console.warn(
        `[SEED] User not member of ${TENANT_SLUG}, falling back to default/first found.`
      );
