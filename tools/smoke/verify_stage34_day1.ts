...
async function main() {
    ...
    const duplicateOrgs = await prisma.$queryRaw<
        Array<{ ownerId: string; org_count: bigint }>
    >`
  ...
  `;
    ...
    if (orphanMembers.length === 0) {
        ...
        orphanMembers.forEach((row) => {
            console.log(
                `   - Member: ${row.id} (User: ${row.userId}, Org: ${row.organizationId})`,
            );
        });
    }
  ...
  `;
    ...
    if (dirtyDefaults.length === 0) {
        ...
        dirtyDefaults.forEach((row) => {
            console.log(
                `   - User: ${row.email} (defaultOrg: ${row.defaultOrganizationId})`,
            );
        });
    }
