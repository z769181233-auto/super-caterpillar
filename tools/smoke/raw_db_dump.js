...
async function run() {
    ...
    const resAssets = await client.query('SELECT id, "ownerId", "storageKey", type, "createdAt", "createdByJobId" FROM "Asset" ORDER BY "createdAt" DESC LIMIT 5');
    process.stderr.write("--- Assets ---\n");
    resAssets.rows.forEach(r => process.stderr.write(JSON.stringify(r) + "\n"));

    const resJobs = await client.query('SELECT id, type, status, "lastError" FROM shot_jobs ORDER BY "createdAt" DESC LIMIT 5');
