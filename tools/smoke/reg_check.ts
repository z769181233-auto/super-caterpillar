...
async function runCheck() {
  ...
  try {
    ...
    const structRes = await axios.get(`${API_URL}/projects/${projectId}/structure`, {
      headers: authHeader,
    });
    ...
    if (meRes.data.data.organizations?.length > 1) {
      const orgs = meRes.data.data.organizations.filter(
        (o: any) => o.organization.type === 'PERSONAL'
      );
      if (orgs.length > 1) {
        throw new Error(
          `DB Integrity Failed: Multiple personal organizations found for user ${email}`
        );
      }
    }
