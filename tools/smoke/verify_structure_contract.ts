...
async function main() {
    ...
    if (!scRes.data || !Array.isArray(scRes.data)) fail(`Scenes list format invalid: ${JSON.stringify(scRes)}`);
    ...
    if (seasons === 0 || episodes === 0 || scenes === 0 || shots === 0) {
        fail(`❌ GATE FAILED: At least one count is 0. Se=${seasons}, Ep=${episodes}, Sc=${scenes}, Sh=${shots}`);
    }
