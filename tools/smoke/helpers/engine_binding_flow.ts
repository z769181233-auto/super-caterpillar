  ...
  try {
    ...
    const importNovelResult = await makeHmacRequest({
      ...
      body: {
        ...
        content: 'This is a demo novel text for smoke testing purpose. It contains enough content to be parsed into chapters if the parser allows.\n\n第1章 开端\n这是第一章的内容。',
      },
    });
    steps.push({ step: 'Import Novel Source (Pre-condition for Analyze)', result: importNovelResult });
    if (!importNovelResult.success || (importNovelResult.status !== 201 && importNovelResult.status !== 200)) {
      console.error('[EngineBindingFlow] Novel import failed:', JSON.stringify(importNovelResult.response));
      throw new Error(`Failed to import novel: status=${importNovelResult.status}, body=${JSON.stringify(importNovelResult.response)}`);
    }
    ...
    steps.push({ step: 'Create NOVEL_ANALYSIS Job (via /api/projects/:projectId/novel/analyze)', result: createJobResult });

    if (!createJobResult.success || (createJobResult.status !== 201 && createJobResult.status !== 200)) {
    ...
    const taskId = createJobResult.response?.data?.task?.id ||
      createJobResult.response?.data?.taskId ||
      createJobResult.response?.task?.id ||
      createJobResult.response?.taskId;

    if (!taskId) {
      throw new Error(`Failed to extract taskId from response: ${JSON.stringify(createJobResult.response)}`);
    }
    ...
    if (!jobId) {
      throw new Error(`Failed to extract jobId from task ${taskId}. Response: ${JSON.stringify(getTaskJobsResult.response)}`);
    }
