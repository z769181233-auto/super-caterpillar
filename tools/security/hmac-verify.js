...
async function testCase(name, method, path, body, headers, expectedErrorCode) {
  ...
  log(
    `Headers: X-Api-Key=${hasApiKey}, X-Nonce=${hasNonce}, X-Timestamp=${hasTimestamp}, X-Signature=${hasSignature}`
  );
  ...
  if (expectedErrorCode === null) {
    ...
    log(
      `Result: ${passed ? '✅ PASS' : '❌ FAIL'} (正常签名不应出现 error.code=4003/4004, got: ${errorCode || 'none'})`
    );
  } else {
    // 期望特定错误码
    passed = errorCode === expectedErrorCode;
    log(
      `Result: ${passed ? '✅ PASS' : '❌ FAIL'} (expected error.code=${expectedErrorCode}, got: ${errorCode || 'none'})`
    );
  }
...
async function main() {
  ...
  const endpoints = [
    ...
    {
      method: 'POST',
      path: `/api/projects/${PROJECT_ID}/novel/import`,
      body: { title: 'Test Novel', rawText: 'Test content' },
    },
  ...
  for (const endpoint of endpoints) {
    ...
    const message1 = hmacLib.buildMessage(
      endpoint.method,
      endpoint.path,
      expiredTimestamp,
      nonce1,
    ...
    const message2 = hmacLib.buildMessage(
      endpoint.method,
      endpoint.path,
      validTimestamp,
      nonce2,
    ...
    const message3 = hmacLib.buildMessage(
      endpoint.method,
      endpoint.path,
      newTimestamp,
      nonce2,
    ...
    const message4 = hmacLib.buildMessage(
      endpoint.method,
      endpoint.path,
      validTimestamp2,
      nonce4,
  ...
  results.forEach((r) => {
    const expected =
      r.expectedErrorCode === null ? 'no 4003/4004' : `error.code=${r.expectedErrorCode}`;
    log(
      `  ${r.name}: ${r.passed ? '✅' : '❌'} (status: ${r.status}, error.code: ${r.errorCode || 'none'}, expected: ${expected})`
    );
  });
  ...
  const validSignature = results.filter(
    (r) => r.name.includes('正常签名') && !r.name.includes('第一次')
  );

  log('\n按用例类型统计:');
  log(`  不带签名头: ${missingHeader.filter((r) => r.passed).length}/${missingHeader.length} 通过`);
  log(
    `  timestamp 过期: ${expiredTimestamp.filter((r) => r.passed).length}/${expiredTimestamp.length} 通过`
  );
  log(
    `  重复 nonce: ${duplicateNonce.filter((r) => r.passed).length}/${duplicateNonce.length} 通过`
  );
  ...
  log('\n关键结论:');
  if (
    missingHeader.every((r) => r.passed) &&
    expiredTimestamp.every((r) => r.passed) &&
    duplicateNonce.every((r) => r.passed)
  ) {
