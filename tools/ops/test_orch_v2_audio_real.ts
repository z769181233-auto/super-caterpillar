...
async function test() {
  ...
  if (audioJobs.length === 0 || videoJobs.length === 0)
    throw new Error('DAG Transition Failed (Jobs missing)');
  if (
    audioJobs.some((v) => v.status !== 'SUCCEEDED') ||
    videoJobs.some((v) => v.status !== 'SUCCEEDED')
  )
    throw new Error('Parallel Tracks Timeout or Error');
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { pipelineRunId, audioCount: audioJobs.length, videoCount: videoJobs.length },
      null,
      2
    )
  );
