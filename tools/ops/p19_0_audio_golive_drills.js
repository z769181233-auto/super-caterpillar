...
async function runDrills(outPath) {
  ...
  const rK1 = await svc.generateAndMix({
    ...commonReq,
  ...
  const rW1 = await svc.generateAndMix({
    ...commonReq,
    projectSettings: { audioRealEnabled: false, audioBgmEnabled: true },
  });
  ...
  const rW2 = await svc.generateAndMix({
    ...commonReq,
    projectSettings: { audioRealEnabled: true, audioBgmEnabled: true },
  });
  ...
  const rC1c = await svc.generateAndMix({
    ...commonReq,
    preview: false,
    projectSettings: { ...commonReq.projectSettings, audioBgmLibraryId: 'bgm_lib_v2_com' },
  });
  ...
  fs.writeFileSync(
    path.join(outPath, 'results_sanitized.json'),
