...
async function test() {
  ...
  while (Date.now() - start < 150000) {
    ...
    console.log(
      `Audit: ${verifiedCount}/2 S, ${running} R. Job types=${results.map((x) => x.type + ':' + x.status).join(',')}`
    );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ verifiedCount, ts: new Date().toISOString() }, null, 2)
  );
}
