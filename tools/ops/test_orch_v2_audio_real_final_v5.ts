...
async function test() {
    console.log("--- L2 REAL SEAL: FINAL AUDIT RUN ---");
    ...
    const project = await prisma.project.create({ data: { id: randomUUID(), name: 'L2 Final Seal Success', organizationId, ownerId, status: 'in_progress' } as any });
    ...
    for (const j of jobsConfig) {
        const job = await prisma.shotJob.create({
            data: {
                ...
                payload: { pipelineRunId: 'FINAL', isVerification: true }
            }
        });
    ...
    while (Date.now() - start < 150000) {
        ...
        verifiedCount = results.filter(j => j.status === 'SUCCEEDED').length;
        const running = results.filter(j => j.status === 'RUNNING').length;
        
        console.log(`Audit: ${verifiedCount}/2 S, ${running} R. Job types=${results.map(x=>x.type+":"+x.status).join(',')}`);
        if (verifiedCount === 2) break;
        await new Promise(r => setTimeout(r, 5000));
    }

    fs.writeFileSync('gate_real_output.json', JSON.stringify({ verifiedCount, ts: new Date().toISOString() }, null, 2));
}
