...
async function test() {
    console.log("--- L2 REAL SEAL: TRACK SWITCH VERIFICATION ---");
    ...
    const project = await prisma.project.create({ data: { id: randomUUID(), name: 'L2 Parallel Proof', organizationId, ownerId, status: 'in_progress' } as any });
    ...
    // P1-1: Parallel tracks SHOT_RENDER vs CE09_MEDIA_SECURITY
    const jobsConfig = [
        { type: 'SHOT_RENDER', engineId: 'engine-shot', engineKey: 'default_shot_render' },
        { type: 'CE09_MEDIA_SECURITY', engineId: 'engine-sec', engineKey: 'ce09_security_real' }
    ];
    ...
    for (const j of jobsConfig) {
        const job = await prisma.shotJob.create({
            data: {
                ...
                payload: { pipelineRunId: 'FINAL_PROOF', isVerification: true }
            }
        });
    ...
    fs.writeFileSync('gate_real_output.json', JSON.stringify({ verified: final.every(x=>x.status==='SUCCEEDED') }, null, 2));
}
