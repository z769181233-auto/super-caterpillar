...
async function test() {
    ...
    const project = await prisma.project.create({
        data: { id: randomUUID(), name: 'L2 Dual Track Seal', organizationId: org.id, ownerId: user.id, status: 'in_progress' } as any
    });

    // Mock Reference Sheet (Required by Stage 1 API)
    const refJob = await prisma.shotJob.create({
        data: { organizationId: org.id, projectId: project.id, type: 'NOVEL_ANALYSIS' as any, status: 'SUCCEEDED' as any }
    });
    const refBinding = await prisma.jobEngineBinding.create({
        data: { jobId: refJob.id, engineId: 'engine-shot', engineKey: 'default_shot_render', status: 'COMPLETED' as any } as any
    });
