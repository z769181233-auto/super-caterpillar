...
async function test() {
  ...
  if (resA.signals.bgm_track_id !== resB.signals.bgm_track_id)
    throw new Error('Fixed seed consistency failed');
  console.log(`Consistency (fixed_seed): OK (${resA.signals.bgm_track_id})`);

  // 3. Write hard evidence
  fs.writeFileSync(
    path.join(evidenceDir, 'tracks_selected.json'),
    JSON.stringify(tracksSelected, null, 2)
  );
  fs.writeFileSync(
    path.join(evidenceDir, 'selection_counts.json'),
    JSON.stringify(selectionCounts, null, 2)
  );
