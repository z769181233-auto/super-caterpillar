...
async function test() {
    ...
    const project = await prisma.project.create({ data: { id: randomUUID(), name: 'L2 Concurrency Seal', organizationId, ownerId, status: 'in_progress' } as any });
    ...
    while (Date.now() - start < 180000) {
        ...
        console.log(`Audit: ${succeeded}/2 S, ${running} R. States=${results.map(x=>x.type+":"+x.status).join(',')}`);
        if (succeeded === 2) break;
        await new Promise(r => setTimeout(r, 5000));
    }

    const final = await prisma.shotJob.findMany({ where: { id: { in: jobIds } } });
    fs.writeFileSync('gate_real_output.json', JSON.stringify({ succeeded: final.filter(x=>x.status==='SUCCEEDED').length }, null, 2));
}
