...
async function test() {
  ...
  const cases = [
    ...
    {
      name: 'Lib V2 Routing',
      settings: { audioBgmLibraryId: 'bgm_lib_v2_com' },
      expectedLib: 'bgm_lib_v2_com',
    },
    {
      name: 'Invalid Lib Recovery',
      settings: { audioBgmLibraryId: 'not_exist' },
      expectedLib: 'bgm_lib_v1',
    },
  ];
  ...
  for (const c of cases) {
    ...
    console.log(
      `Selected Lib: ${res.signals.bgm_library_version} (ID: ${res.signals.bgm_track_id})`
    );

    // P18-5.2 Validation
    // v1 tracks start with bgm_00, v2 tracks start with bgm_20
    if (c.expectedLib === 'bgm_lib_v2_com') {
      if (!res.signals.bgm_track_id.startsWith('bgm_20'))
        throw new Error(`Routing to V2 failed, got ${res.signals.bgm_track_id}`);
      if (res.signals.bgm_library_version !== 'v2.0.0-exp') throw new Error('Wrong version for V2');
    } else {
      if (!res.signals.bgm_track_id.startsWith('bgm_00'))
        throw new Error(`Routing to V1 failed, got ${res.signals.bgm_track_id}`);
