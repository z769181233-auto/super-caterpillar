...
async function test() {
    ...
    const project = await prisma.project.create({ data: { id: randomUUID(), name: 'L2 Final Structural', organizationId, ownerId, status: 'in_progress' } as any });
    ...
    if (!finalResults.every(j => j.status === 'SUCCEEDED')) throw new Error("Verification Timeout.");
