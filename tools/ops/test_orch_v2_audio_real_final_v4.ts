...
async function test() {
    ...
    const project = await prisma.project.create({ data: { id: randomUUID(), name: 'L2 Final Seal V4', organizationId, ownerId, status: 'in_progress' } as any });
    const season = await prisma.season.create({ data: { projectId: project.id, index: 1, title: 'S1' } as any });
    const episode = await prisma.episode.create({ data: { seasonId: season.id, projectId: project.id, index: 1, name: 'E1' } as any });
    const scene = await prisma.scene.create({ data: { episodeId: episode.id, projectId: project.id, sceneIndex: 1, title: 'Scene 1' } as any });
    const shot = await prisma.shot.create({ data: { sceneId: scene.id, index: 1, title: 'Shot 1', type: 'ce_core', organizationId } as any });
    ...
    console.log(`Initial PENDING count: ${await prisma.shotJob.count({ where: { status: 'PENDING' } })}`);
    ...
    fs.writeFileSync('gate_real_output.json', JSON.stringify({ 
        pipelineRunId, summary: final.map(j => ({ type: j.type, status: j.status }))
    }, null, 2));
}
