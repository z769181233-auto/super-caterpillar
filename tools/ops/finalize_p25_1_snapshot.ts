...
async function main() {
  ...
  fs.writeFileSync(
    path.join(outDir, 'job_summary_final.json'),
  ...
  fs.writeFileSync(
    path.join(outDir, 'l1_l2_throughput_final.json'),
  ...
  fs.writeFileSync(
    path.join(outDir, 'l3_sample_manifest.json'),
    JSON.stringify({ seed, samples: auditResults }, null, 2)
  );
  fs.writeFileSync(
    path.join(outDir, 'audit_coverage_final.json'),
    JSON.stringify(auditCoverage, null, 2)
  );

  console.log(`[Finalizer] Snapshot Sealed. Coverage: ${auditCoverage.asset_persist_rate * 100}%`);

  if (
    auditCoverage.asset_persist_rate < 1.0 ||
    auditCoverage.costledger_coverage_rate < 1.0 ||
    jobSummary.failed > 0
  ) {
    process.exit(1);
  }
