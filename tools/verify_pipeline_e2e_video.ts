...
function generateSignatureV1_1(
  apiKey: string,
  secret: string,
  nonce: string,
  timestamp: string,
...
const EVD = ensureEvidenceDir(
  path.resolve(__dirname, `../docs/_evidence/pipeline_e2e_video_${iso}`)
);
  ...
  try {
    ...
    try {
      ...
      await prisma.project.create({
        data: {
          id: projectId,
          name: 'Commercial Pipeline Project',
    ...
    try {
      ...
      if (
        ...
        e.response?.data?.error?.code === '4003' ||
    ...
    try {
      ...
      if (code === '4004' || e.response?.status === 403) {
        ...
        console.warn(
          `⚠️ Replay Test Warning: Expected 4004, got ${e.response?.status}. Body:`,
          safeSummary(e.response?.data)
        );
    ...
    while (Date.now() - pollStarted < TIMEOUT_MS) {
      ...
      process.stdout.write(
        `\r   Jobs: ${types.length} | Sequence: ${types.join(' -> ')} | Status: ${statuses.join(',')}`
      );
    ...
    const securityAudit = audits.find(
      (a) => a.action === 'ce09.media_security.success' && a.resourceId === asset.id
    );

    if (!securityAudit)
      console.warn('⚠️ CE09 Audit Log missing (Action: ce09.media_security.success)');
