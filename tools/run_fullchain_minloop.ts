...
async function run() {
  ...
  try {
    ...
    await prisma.organization.upsert({
      ...
      update: {},
      create: { id: 'default-org', name: 'Default', ownerId: userId },
    });
    await prisma.project.upsert({
      where: { id: projectId },
      update: {},
      create: { id: projectId, name: 'FullChain', organizationId: 'default-org', ownerId: userId },
    });
    ...
    const enginesToInvoke = [
      ...
      {
        key: 'scene_composition',
        payload: { background_url: `file://${dummyAssetPath}`, elements: [] },
      },
    ];
    ...
    for (const key of qcEngines) {
      ...
      if (res.status !== 'SUCCESS')
        throw new Error(`QC ${key} failed: ${JSON.stringify(res.error)}`);
