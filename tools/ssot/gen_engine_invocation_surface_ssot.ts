...
function collectCallSites(repoRoot: string): CallSite[] {
  const patterns: Array<{ pattern: string; type: string; notes?: string; endpoint?: string }> = [
    {
      pattern: 'engineClient.invoke',
      type: 'remote_hub_invoke',
      notes: 'Engine client invocation',
    },
    { pattern: 'EngineHubClient', type: 'remote_hub_invoke', notes: 'Engine hub client usage' },
    {
      pattern: '/_internal/engine/invoke',
      type: 'internal_endpoint_invoke',
      endpoint: '/_internal/engine/invoke',
    },
  ...
  const uniq = uniqBy(
    normed,
    (x) => `${x.file}|${x.function}|${x.lineRange}|${x.type}|${x.endpoint || ''}`
  );
  return stableSort(
    uniq,
    (x) => `${x.file}|${x.function}|${x.lineRange}|${x.type}|${x.endpoint || ''}`
...
function collectEngineSurface(repoRoot: string): EngineSurfaceItem[] {
  ...
  while ((m = keyRe.exec(txt))) {
    ...
    if (braceIndex !== -1) {
      ...
      const alreadyProcessed = processedRanges.some(
        (r) => braceIndex >= r.start && braceIndex < r.end
      );
      if (alreadyProcessed) {
        continue;
      }
  ...
  const uniqItems = uniqBy(
    items,
    (x) =>
      `${x.engineKey}|${x.version}|${x.mode}|${x.adapterToken || ''}|${x.httpConfig?.baseUrl || ''}|${x.httpConfig?.path || ''}|${x.registeredIn}`
  );
...
function main() {
  ...
  const outputDir =
    outDirArgIdx >= 0 && args[outDirArgIdx + 1] ? args[outDirArgIdx + 1] : 'docs/ssot';
