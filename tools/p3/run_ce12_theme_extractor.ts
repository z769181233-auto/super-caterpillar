...
async function main() {
    ...
    await (prisma as any).project.create({ data: { id: projectId, name: 'CE12 Test', ownerId: userId, organizationId: orgId } });
    ...
    try {
        ...
        if (!res.output?.analysis.detected_themes.includes('LOVE')) throw new Error("Expected LOVE theme");
        if (!res.output?.analysis.detected_themes.includes('JUSTICE')) throw new Error("Expected JUSTICE theme");
