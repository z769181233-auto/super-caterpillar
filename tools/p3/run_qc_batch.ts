...
async function main() {
    ...
    // Seed
    await (prisma as any).user.upsert({ where: { id: userId }, update: {}, create: { id: userId, email: `qc_sys_${suffix}@scu`, passwordHash: 'mock' } });
    await (prisma as any).organization.upsert({ where: { id: orgId }, update: { credits: 2000 }, create: { id: orgId, name: 'QC Org', ownerId: userId, credits: 2000 } });
    await (prisma as any).project.upsert({ where: { id: projectId }, update: {}, create: { id: projectId, name: 'QC Batch Project', ownerId: userId, organizationId: orgId } });
    ...
    for (const e of engines) {
        ...
        await (prisma as any).shotJob.create({ data: { id: jobId, projectId, status: 'RUNNING', type: 'QC_CHECK', attempts: 1, organizationId: orgId } });
        const res = await e.adapter.invoke({
            jobType: 'QC_CHECK',
            engineKey: `${e.name}_...`,
            payload: {},
            context: { projectId, userId, traceId: `t_${e.name}_${suffix}`, jobId, organizationId: orgId }
        });
