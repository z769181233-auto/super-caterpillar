...
async function main() {
  ...
  await (prisma as any).user.upsert({
    where: { id: userId },
    update: {},
    create: { id: userId, email: `pp_sys_${suffix}@scu`, passwordHash: 'mock' },
  });
  await (prisma as any).organization.upsert({
    where: { id: orgId },
    update: { credits: 2000 },
    create: { id: orgId, name: 'PP Org', ownerId: userId, credits: 2000 },
  });
  await (prisma as any).project.upsert({
    where: { id: projectId },
    update: {},
    create: { id: projectId, name: 'PP Batch Project', ownerId: userId, organizationId: orgId },
  });
  ...
  const r1 = await pp01.invoke({
    jobType: 'PP_RENDER',
    engineKey: 'pp01_video_stitch',
    payload: {},
    context: { projectId, userId, traceId: `t_pp01_${suffix}`, jobId: j1, organizationId: orgId },
  });
  ...
  const r2 = await pp02.invoke({
    jobType: 'PP_RENDER',
    engineKey: 'pp02_subtitle_overlay',
    payload: { sourceUrl: r1.output.assetUrl },
    context: { projectId, userId, traceId: `t_pp02_${suffix}`, jobId: j2, organizationId: orgId },
  });
  ...
  const r3 = await pp03.invoke({
    jobType: 'PP_RENDER',
    engineKey: 'pp03_watermark',
    payload: { sourceUrl: r2.output.assetUrl },
    context: { projectId, userId, traceId: `t_pp03_${suffix}`, jobId: j3, organizationId: orgId },
  });
  ...
  const r4 = await pp04.invoke({
    jobType: 'PP_RENDER',
    engineKey: 'pp04_hls_package',
    payload: { sourceUrl: r3.output.assetUrl },
    context: { projectId, userId, traceId: `t_pp04_${suffix}`, jobId: j4, organizationId: orgId },
  });
