...
async function main() {
    ...
    await (prisma as any).project.create({ data: { id: projectId, name: 'CE13 Test', ownerId: userId, organizationId: orgId } });
    ...
    try {
        ...
        const resFast = await adapter.invoke({
            ...
            context: { projectId, userId, traceId: `trace_ce13_fast_${suffix}`, jobId, organizationId: orgId }
        });
        ...
        await (prisma as any).shotJob.create({ data: { id: jobId2, projectId, status: 'RUNNING', type: 'NOVEL_ANALYSIS', attempts: 1, organizationId: orgId } });
        const resSlow = await adapter.invoke({
            jobType: 'NOVEL_ANALYSIS',
            engineKey: 'ce13_pacing_analyzer',
            payload: { text: "The very long and winding road lead to a place where time stood still, according to the ancient legends of the mountain people." },
            context: { projectId, userId, traceId: `trace_ce13_slow_${suffix}`, jobId: jobId2, organizationId: orgId }
        });
