...
async function main() {
  ...
  const metrics = new (require('../../apps/api/src/ops/ops-metrics.service').OpsMetricsService)(
    prisma
  );
  const audioService = new (require('../../apps/api/src/audio/audio.service').AudioService)(
    metrics
  );
