...
async function main() {
    ...
    // Seed
    await (prisma as any).user.upsert({ where: { id: userId }, update: {}, create: { id: userId, email: `au_sys_${suffix}@scu`, passwordHash: 'mock' } });
    await (prisma as any).organization.upsert({ where: { id: orgId }, update: { credits: 1000 }, create: { id: orgId, name: 'AU Org', ownerId: userId, credits: 1000 } });
    await (prisma as any).project.upsert({ where: { id: projectId }, update: {}, create: { id: projectId, name: 'AU Batch Project', ownerId: userId, organizationId: orgId } });
    ...
    for (const e of engines) {
        ...
        await (prisma as any).shotJob.create({ data: { id: jobId, projectId, status: 'RUNNING', type: 'AU_RENDER', attempts: 1, organizationId: orgId } });
        const res = await e.adapter.invoke({
            jobType: 'AU_RENDER',
            engineKey: `${e.name}_...`, // handled by adapter.supports
            payload: e.payload,
            context: { projectId, userId, traceId: `trace_${e.name}_${suffix}`, jobId, organizationId: orgId }
        });
    ...
    await (prisma as any).shotJob.create({ data: { id: mixJobId, projectId, status: 'RUNNING', type: 'AU_RENDER', attempts: 1, organizationId: orgId } });
    const mixRes = await au04.invoke({
        jobType: 'AU_RENDER',
        engineKey: 'au04_audio_mix',
        payload: { tracks: results.map(r => ({ url: r.output.assetUrl })) },
        context: { projectId, userId, traceId: `trace_au04_${suffix}`, jobId: mixJobId, organizationId: orgId }
    });
