...
function has(marker){ return s.includes(`<!-- SSOT_TABLE:${marker} -->`); }
...
if(!hSealed || !hInprog || !hPlanned){
  console.error('[FATAL] headings missing', {sealed:!!hSealed,inprog:!!hInprog,planned:!!hPlanned});
  process.exit(3);
}

function insertAt(pos, text){ s = s.slice(0,pos) + text + s.slice(pos); }
