...
if (!SSOT) { console.error('Usage: plan34_fix_ssot_paths.js <SSOT_PATH>'); process.exit(2); }
...
function parseMarkdownTable(block) {
    ...
    const header = tableLines[0].split('|').slice(1, -1).map(s => s.trim());
    const rows = [];
...
function fixRowPaths(row, stateName) {
    ...
    if (row['adapter_path']) assertPrefix(row['adapter_path'], WHITELIST_ADAPTER_PREFIX, 'adapter_path');
    ...
    if (row['engine_key'] === 'ce02_identity_lock') {
        ...
        if (!notes.includes('ALIAS_OF=')) row['notes'] = (notes ? (notes + '; ') : '') + 'ALIAS_OF=ce23_identity_consistency';
    }
}
...
function ensurePlannedColumns(t) {
    ...
    for (const r of t.rows) {
        ...
        if (ap) { r['expected_adapter_path'] = ap; r['adapter_path'] = ''; }
        if (gp) { r['expected_gate_path'] = gp; r['gate_path'] = ''; }
    }
}
...
let out = src;
out = out.slice(0, SEALED.bi + '<!-- SSOT_TABLE:SEALED_BEGIN -->'.length) + sealedBlockNew + out.slice(SEALED.ei);
out = out.slice(0, INP.bi + '<!-- SSOT_TABLE:INPROGRESS_BEGIN -->'.length) + inpBlockNew + out.slice(INP.ei);
out = out.slice(0, PLAN.bi + '<!-- SSOT_TABLE:PLANNED_BEGIN -->'.length) + planBlockNew + out.slice(PLAN.ei);
