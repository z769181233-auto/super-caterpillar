...
const FILES = new Set(sh('git ls-files').split('\n').map(s => s.trim()).filter(Boolean));
...
function normalizeSealedOrInprogressRow(r) {
    ...
    r['seal_tag'] = stripTicks(r['seal_tag'] || '') && stripTicks(r['seal_tag'] || '') !== '-' ? codeCell(r['seal_tag']) : (stripTicks(r['seal_tag'] || '') === '-' ? '-' : '');
...
function normalizePlannedRow(r) {
    ...
    if (eap && eap.includes('/')) assertRootRelative(r['expected_adapter_path'], 'expected_adapter_path');
