...
if (!ALLOW_PATH || !SEAL_VG || !SEAL_PP || !SEAL_CE) {
    console.error("Usage: node promote_inprogress_allowlist_strict_v1.js <allowlist.txt> <seal_vg> <seal_pp> <seal_ce>");
    process.exit(1);
}

const allowlist = new Set(
    fs.readFileSync(ALLOW_PATH, "utf8")
        .split("\n")
        .map(s => s.trim())
        .filter(Boolean)
);
...
function renderTable(header, rows) {
    ...
    const body = rows.map(r => {
        const cells = header.map(h => {
            const v = r[h] || "";
            if (["engine_key", "job_type", "adapter_path", "gate_path", "seal_tag"].includes(h) && v && v !== "-") {
                return fmtCell(v);
            }
            return v;
        });
