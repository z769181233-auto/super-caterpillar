...
function sha256OfFile(fpath) {
  ...
  const cmd = process.platform === 'darwin' ? `shasum -a 256 "${fpath}"` : `sha256sum "${fpath}"`;
...
fs.writeFileSync(
  path.join(EVI_DIR, 'MANUAL_PROMOTE_AUDIT.json'),
  JSON.stringify(auditMetadata, null, 2)
);

if (inRows.length === 0) {
  fs.writeFileSync(
    path.join(EVI_DIR, 'PROMOTION_INDEX.json'),
    JSON.stringify({ ts: new Date().toISOString(), note: 'no inprogress rows' }, null, 2)
  );
...
fs.writeFileSync(
  path.join(EVI_DIR, 'PROMOTED_ENGINES.json'),
  JSON.stringify(
    promotable.map((x) => ({ engine_key: x.ek, gate_path: x.gp, seal_tag: engineTag[x.ek] })),
    null,
    2
  )
);
...
for (const r of tInp.rows) {
  ...
  r.ledger_required = stripTicks(r.ledger_required)
    ? stripTicks(r.ledger_required)
    : r.ledger_required;
