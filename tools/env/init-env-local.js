...
function main() {
  ...
  if (fileExists) {
    ...
    // 如果文件中有占位符行或重复行，需要修复
    const keysToFix = [
      ...new Set([...missingKeys, ...placeholderKeysInFile, ...duplicateKeysInFile]),
    ];
    ...
    const placeholderKeys =
      placeholderKeysInFile.length > 0
        ? placeholderKeysInFile
        : missingKeys.filter((key) => {
            const value = existingEnv[key];
            return value && value.trim() !== '' && isPlaceholderValue(key, value);
          });
    const trulyMissingKeys = missingKeys.filter(
      (key) =>
        !placeholderKeys.includes(key) && (!existingEnv[key] || existingEnv[key].trim() === '')
    );
