...
async function waitJobsExist(projectId: string, type: string, minCount: number, timeoutMs = 180000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const cnt = await prisma.shotJob.count({
            where: { projectId, type: type as any, traceId: TRACE_ID, createdAt: { gte: START_TS } } as any,
        });
...
async function waitJobsDone(projectId: string, type: string, timeoutMs = 600000) {
    ...
    while (Date.now() < deadline) {
        const pending = await prisma.shotJob.count({
            where: { projectId, type: type as any, traceId: TRACE_ID, status: { in: ['PENDING', 'RUNNING'] }, createdAt: { gte: START_TS } } as any,
        });
        const failed = await prisma.shotJob.count({
            where: { projectId, type: type as any, traceId: TRACE_ID, status: 'FAILED', createdAt: { gte: START_TS } } as any,
        });
...
async function main() {
    ...
    try {
        ...
        await prisma.project.upsert({
            where: { id: PROJECT_ID },
            update: {},
            create: { id: PROJECT_ID, name: 'Prod Pilot Sealed42', organizationId: ORGANIZATION_ID, ownerId: 'pilot-runner' } as any
        });
        ...
        execSync(`PROJECT_ID=${PROJECT_ID} npx ts-node tools/prod/run_character_turnaround.ts`, { stdio: 'inherit' });
        ...
        for (const s of cinemaScenes) {
            ...
            for (const sh of shots) {
                shotCount++;
                const framesTxt = path.join(process.cwd(), '.runtime', 'frames', String(sh.id), 'frames.txt');
        ...
        const scenes = await prisma.scene.findMany({ where: { episode: { projectId: PROJECT_ID } } as any });
        ...
        for (const j of finalJobs) {
            if (j.status === 'SUCCEEDED') {
                ...
                try {
                    execSync(`bash tools/gate_non_placeholder_video.sh "${absAssetPath}"`, { stdio: 'inherit' });
