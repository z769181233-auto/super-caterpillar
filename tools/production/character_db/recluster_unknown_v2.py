...
def recluster_unknown_v2():
    unknown_dir = "./dataset/unknown/"
    raw_dir = "./work/faces/raw/"
    output_dir = "./dataset/unknown_v2_clusters/"
    ...
    print(f"\n[Re-Clustering-V2] Found {len(unknown_pngs)} images in unknown/.")
    ...
    # 2. DBSCAN
    # User requirement: eps=0.45, min_samples=3, metric='cosine'
    print("--- Running DBSCAN (eps=0.45, min_samples=3, metric='cosine')...")
    db = DBSCAN(eps=0.45, min_samples=3, metric='cosine').fit(embeddings)
    cluster_labels = db.labels_
    
    unique_labels = set(cluster_labels)
    n_sub = len(unique_labels) - (1 if -1 in unique_labels else 0)
    print(f"--- Detected {n_sub} character sub-clusters in unknown.")
    ...
    for label in unique_labels:
        ...
        # User requirement: Output all >= 8 images clusters
        if label != -1 and len(indices) < 8:
            # We still move them but label them differently or just keep track
            print(f"--- Cluster sub_{label:03d} too small ({len(indices)}), skipping major export.")
        ...
        # Generate Preview for clusters with >= 3 images
    ...
    print(f"\n[Success] Unknown re-clustering complete. Results in {output_dir}")

if __name__ == "__main__":
    recluster_unknown_v2()
