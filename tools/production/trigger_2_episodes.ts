...
const apiClient = new ApiClient('http://localhost:3000', API_KEY, API_SECRET, 'production-trigger');
...
async function main() {
  ...
  console.log(
    `📊 监视命令: pnpm -w exec tsx tools/smoke/diag_db.ts --sql "SELECT id, type, status FROM shot_jobs WHERE project_id LIKE '${productionId}%';"`
  );
}
