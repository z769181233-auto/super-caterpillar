...
function generateNovel() {
    ...
    let size = 0;
    // 300 chars per chunk
    const chunk = '薛知盈坐在窗前，看着窗外的落花发呆。“姑娘，天凉了，加件衣裳吧。”春桃拿着一件披风走了进来。萧昀祈从书案房大步走来，面色凝重。“这表姑娘又跑了？”王嬷嬷在回廊下低声嘀咕。薛知盈叹了一口气，“只要能离开这里，去哪里都好。”\n'.repeat(2);

    let chapterCount = 0;
    while (size < TARGET_SIZE_MB * 1024 * 1024) {
        if (size % (50 * 1024) === 0) { // Every 50KB a chapter
            chapterCount++;
    ...
    writeStream.end();
    console.log(`Generated ${OUTPUT_FILE} (${(size / 1024).toFixed(2)} KB, ${chapterCount} chapters)`);
}
