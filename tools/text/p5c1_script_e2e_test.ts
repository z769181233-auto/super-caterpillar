...
async function runHardenedFinalE2E() {
    ...
    const episodes = await prisma.episode.findMany({ where: { buildId: build.id }, orderBy: { index: 'asc' } });
    ...
    for (const ep of episodes) {
        ...
        const scenes = await prisma.scene.findMany({ where: { episodeId: ep.id }, orderBy: { sceneIndex: 'asc' } });
...
runHardenedFinalE2E().catch(console.error).finally(() => prisma.$disconnect());
