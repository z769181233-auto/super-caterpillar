...
async function main() {
  ...
  while (offset < totalChars) {
    ...
    if (chunkIndex % 50 === 0) {
      console.log(
        `Imported Chunk ${chunkIndex}: offset ${offset}-${end}, hash ${textHash.substring(0, 8)}...`
      );
    }
  ...
  // 3. Generate Evidence
  const evidenceDir =
    '/Users/adam/Desktop/adam/毛毛虫宇宙/Super Caterpillar/docs/_evidence/p5c0_source_index';
  ...
  fs.writeFileSync(
    path.join(evidenceDir, 'source_index_summary.json'),
