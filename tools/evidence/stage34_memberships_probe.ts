...
async function main() {
  ...
  const dbUrl = (args['database-url'] || args['db'] || process.env.DATABASE_URL) as
    | string
    | undefined;
  const email = (args['email'] as string | undefined) ?? 'stage34-day1-test@example.com';

  if (!dbUrl)
    throw new Error('DATABASE_URL missing. Provide --database-url=... or set env DATABASE_URL');
  ...
  try {
    ...
    console.log(
      '[stage34_memberships_probe] organization_member_count=',
      typeof memberCount === 'number' ? memberCount : '(model not found)'
    );
    ...
    const memberCountA = await (prisma as any).organizationMember.count({
    ...
    console.log(
      '[probe] old_query_membership_in_default_org_count=',
      oldMembershipInDefaultOrgCount
    );
