...
async function main() {
  ...
  if (!dbUrl) {
    console.error(
      '[stage34_integrity_check] ERROR: DATABASE_URL is missing. Provide --database-url=... or set env DATABASE_URL.'
    );
    process.exit(2);
  }
  ...
  try {
    ...
    try {
      ...
      console.log(
        '[stage34_integrity_check] organization_member_count= (query failed)',
        e?.message ?? e
      );
    }
