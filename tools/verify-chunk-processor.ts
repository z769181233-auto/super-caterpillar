...
async function main() {
    const testFile = process.argv[2] || path.join(__dirname, '../docs/novels/source/test_novel_100k.txt');
    ...
    Array.from(chapterGroups.entries())
        ...
        .forEach(([chapterIdx, count]) => {
            const title = chunks.find(c => c.metadata.chapterIndex === chapterIdx && c.metadata.isChapterBoundary)?.metadata.chapterTitle || `章节 ${chapterIdx}`;
    ...
    chunks.slice(0, 3).forEach((chunk, i) => {
        ...
        console.log(`    章节边界: ${chunk.metadata.is ChapterBoundary ? '是' : '否'}`);
