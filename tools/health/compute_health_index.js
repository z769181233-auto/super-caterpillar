...
// 1. Dead Code
const dead = readFileSafe(outDir + '/dead_code.log')
  .trim()
  .split('\n')
  .filter(Boolean).length;

// 2. Console Logs (Separating Core and Total)
const consoleLogLines = readFileSafe(outDir + '/console.log')
  .trim()
  .split('\n')
  .filter(Boolean);
...
const EXCLUSIONS = [
  ...
  'build/',
  'src/generated/',
  'runtime/', // Exclude generated code (Prisma, etc.)
];
...
function parseCircularFromLog(text) {
  ...
  if (
    /(circular dependencies detected|cycle detected|found\s+\d+\s+cycles?|found\s+\d+\s+circular|Detected\s+cycle)/i.test(
      t
    )
  )
    return 1;
