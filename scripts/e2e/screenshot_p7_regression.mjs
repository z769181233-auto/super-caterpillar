...
async function run() {
  ...
  await noAuthPage
    .goto(`${BASE_URL}/zh/projects`, { timeout: 10000, waitUntil: 'networkidle' })
    .catch(() => {});
  ...
  // 点击 Builds tab（如存在）
  const buildsTab = page
    .locator('[data-tab="builds"], button:has-text("构建"), button:has-text("Builds")')
    .first();
  ...
  const evidenceTab = page
    .locator('[data-tab="evidence"], button:has-text("证据"), button:has-text("Evidence")')
    .first();
