...
const EXCLUDES = [
    'messages', '__tests__', '.snap', '.css',
    '/components/_legacy', '/app/[locale]/tasks', '/app/[locale]/studio',
    '/components/engines', '/components/project', '/views/', '/features/studio',
    '/app/[locale]/builds', '/app/[locale]/dev', '/app/[locale]/monitor',
    '/app/[locale]/solutions', '/app/[locale]/projects/[projectId]/import',
    '/app/[locale]/projects/[projectId]/page-old.tsx', '/app/[locale]/projects/[projectId]/pipeline',
        ...
        SUSPICIOUS_PATTERNS.forEach(regex => {
            ...
            while ((match = r.exec(code)) !== null) {
                ...
                if (CHINESE_REGEX.test(text) || ENGLISH_KEYWORDS.test(text)) {
                    console.error(`\x1b[31m[Hardcode Detected]\x1b[0m ${path.relative(ROOT_DIR, filePath)}:${idx + 1}`);
