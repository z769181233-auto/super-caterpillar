...
async function main() {
  ...
  try {
    ...
    await prisma.organization.create({
      data: { id: orgId, name: 'Safe Org', type: 'STUDIO', credits: 9999, ownerId: userId },
    });
    await prisma.organizationMember.create({
      data: { id: randomUUID(), organizationId: orgId, userId, role: MembershipRole.OWNER },
    });
    await prisma.project.create({
      data: { id: projectId, name: 'Safe Project', ownerId: userId, organizationId: orgId },
    });
    ...
    const token = jwt.sign({ sub: userId, orgId, roles: ['user'] }, ENV.JWT_SECRET, {
      expiresIn: '1h',
    });
    ...
    const blockRes = await fetch(`${API_URL}/api/projects/${projectId}/novel/import`, {
      ...
      body: JSON.stringify({
        title: 'Block Novel',
        rawText: 'This text contains violation keyword.',
      }),
    });
    ...
    if (!blockRecord)
      console.warn(
        '  ⚠️ BLOCK record not found (Might be rolled back if in tx, or implementation detail). Checking Audit...'
      );
