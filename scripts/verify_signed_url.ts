...
async function main() {
  ...
  try {
    ...
    const token = jwt.sign(
      { sub: userId, orgId, email: 'verify@example.com', roles: ['user'] },
      ENV.JWT_SECRET,
      { expiresIn: '1h' }
    );
    ...
    if (!structJson.includes(storageKey)) {
      console.warn(
        '⚠️ Asset not found in structure response (might be deep nested). Proceeding to direct refresh verification.'
      );
    } else {
    ...
    if (fallback) {
      throw new Error(
        'Verify Failed: API returned fallback response, gating or feature flag might be misconfigured.'
      );
    }
    ...
    // const fs = require('fs');
    try {
      fs.unlinkSync(path.join(process.cwd(), 'uploads', storageKey));
    } catch (e) {}
