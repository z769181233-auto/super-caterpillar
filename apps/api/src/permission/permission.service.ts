...
import { SystemPermission, ProjectPermission, SystemPermissions, ProjectPermissions } from './permission.constants';
...
export class PermissionService {
  ...
  async hasPermissions(params: {
    ...
    if (projectId) {
      const proj = await this.prisma.project.findUnique({ where: { id: projectId }, select: { organizationId: true } });
      orgId = proj?.organizationId || undefined;
    }

    // 2. 获取权限全集 (无硬编码 bypass，除非是 system admin 级别)
    ...
    if (!isGranted) {
      console.warn(`[PERM_DENIED] userId=${userId} context=${projectId || orgId || 'NONE'} missing=[${required.filter(p => !allPerms.has(p)).join(',')}]`);
    }
  ...
  async assertCanManageProject(userId: string, organizationId: string): Promise<void> {
    ...
    if (!hasAuth) {
      ...
      throw new ForbiddenException(`NO_PERMISSION_MANAGE_PROJECT (Debug: user=${userId} role=${role} perms=${perms.join(',')})`);
    }
  }
