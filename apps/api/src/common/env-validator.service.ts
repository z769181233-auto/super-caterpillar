...
export class EnvValidatorService implements OnModuleInit {
   ...
   * P0 级环境变量清单（所有环境必需）
   */
  private readonly P0_REQUIRED_VARS = ['DATABASE_URL', 'API_SECRET_KEY'] as const;

  /**
   * P1 级环境变量清单（推荐但非强制）
   */
  private readonly P1_RECOMMENDED_VARS = ['NODE_ENV', 'ENABLE_INTERNAL_JOB_WORKER'] as const;
