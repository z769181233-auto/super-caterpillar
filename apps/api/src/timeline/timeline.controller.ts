...
export class TimelineController {
    ...
    async createPreview(@Body() body: any, @Req() req: any) {
        ...
        if (!timelineData.shots || !Array.isArray(timelineData.shots) || timelineData.shots.length === 0) {
        ...
        const orgId = req.user?.orgId || req.user?.defaultOrganizationId || req.apiKeyOwnerOrgId || body.organizationId;
