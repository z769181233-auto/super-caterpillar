...
export class CharacterService {
    ...
    async create(projectId: string, dto: CreateCharacterDto) {
        ...
        if (this.prisma) {
            this.logger.log(`[DEBUG] Prisma keys: ${Object.keys(this.prisma).filter(k => !k.startsWith('_')).join(', ')}`);
            this.logger.log(`[DEBUG] characterProfile in prisma: ${'characterProfile' in this.prisma}`);
            this.logger.log(`[DEBUG] characterProfile value: ${typeof (this.prisma as any).characterProfile}`);
        }
    ...
    async recordAppearance(characterId: string, dto: RecordAppearanceDto) {
        ...
        if (dto.consistencyScore && dto.consistencyScore < 0.7) {
            this.logger.warn(
                `[${characterId}] Low consistency score detected: ${dto.consistencyScore}`
            );
    ...
    async checkAndTriggerTraining(characterId: string): Promise<void> {
        ...
        // 检查触发条件
        const minConsistencyScore = parseFloat(
            process.env.MIN_CONSISTENCY_SCORE || '0.7'
        );
        const minTrainingImages = parseInt(
            process.env.MIN_TRAINING_IMAGES || '10',
            10
        );
        const trainingCooldownHours = parseInt(
            process.env.TRAINING_COOLDOWN_HOURS || '24',
            10
        );
        ...
        const shouldTrain =
            ...
            (!character.loraLastTrained ||
                Date.now() - character.loraLastTrained.getTime() >
                trainingCooldownHours * 60 * 60 * 1000);
        ...
        try {
            ...
            const trainingImages = await this.collectTrainingImages(
                characterId,
                minConsistencyScore
            );
            ...
            await this.loraTrainingService.submitTraining(
                characterId,
                trainingImages
            );
        } catch (error: any) {
            this.logger.error(
                `[${characterId}] Failed to auto-trigger training: ${error.message}`
            );
        }
    }
}
