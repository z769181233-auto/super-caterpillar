...
import { AudioProvider, AudioSynthesisInput, AudioSynthesisOutput } from './providers/audio-provider.interface';
...
export class AudioService {
    ...
    async resolveProjectSettings(prisma: PrismaLike, projectId: string): Promise<AudioProjectSettings> {
    ...
    async generateAndMix(req: AudioGenerateRequest): Promise<AudioGenerateResult> {
        ...
        const mixKey = crypto.createHash('sha256').update(JSON.stringify(mixCacheObj)).digest('hex').slice(0, 16);
