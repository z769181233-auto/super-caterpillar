...
export class TextSafetyMetrics {
  ...
  static recordDecision(decision: 'PASS' | 'WARN' | 'BLOCK') {
    ...
    this.metrics.text_safety_decision_total.set(
      key,
      (this.metrics.text_safety_decision_total.get(key) || 0) + 1
    );
  }
  ...
  static getPrometheusOutput(): string {
    ...
    lines.push(
      '# HELP signed_url_access_denied_total Total failed access attempts to signed/storage resources'
    );
