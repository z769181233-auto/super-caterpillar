...
export class ProjectController {
  ...
  async dryRunStudioStoryboardImageGeneration(
    ...
    await this.projectService.checkOwnership(projectId, user.userId);
    const data = await this.projectStudioStoryboardAssetService.dryRunStoryboardImageGeneration(
      projectId,
      organizationId,
      body || {}
    );
  ...
  async reviewStudioStoryboardImage(
    ...
    await this.projectService.checkOwnership(projectId, user.userId);
    const data = await this.projectStudioStoryboardAssetService.reviewStoryboardImage(
      projectId,
      organizationId,
      user.userId,
      body || {}
  ...
  @Post(':projectId/storyboard-images/retry-plan')
  @Permissions(ProjectPermissions.PROJECT_READ)
  async getStudioStoryboardImageRetryPlan(
    @Param('projectId') projectId: string,
    @Body() body: any,
    @CurrentUser() user: AuthenticatedUser,
    @CurrentOrganization() organizationId: string | null
    ...
    await this.projectService.checkOwnership(projectId, user.userId);
    const data = await this.projectStudioStoryboardAssetService.getStoryboardImageRetryPlan(
      projectId,
      organizationId,
      body || {}
    );
