...
export class QualityBackfillSweeper {
  ...
  async backfillQualityScores() {
    ...
    try {
      ...
      this.logger.debug(
        `[QualitySweeper] Scanning ${successfulJobs.length} successful jobs for backfill...`
      );
      ...
      for (const job of successfulJobs) {
        ...
        if (!existingScore) {
          ...
          try {
            ...
            this.logger.error(
              `[QualitySweeper] Failed to backfill for shot ${job.shotId}: ${e.message}`
            );
          }
        }
      }
      ...
      if (backfilledCount > 0) {
        this.logger.log(
          `[QualitySweeper] Successfully backfilled ${backfilledCount} quality scores.`
        );
      }
    } catch (error: any) {
