...
export class QuotaGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const organizationId =
      request.apiKeyOwnerOrgId || user?.organizationId || request.headers['x-organization-id'];

    const nonce = request.hmacNonce || request.headers['x-nonce'] || request.headers['x-api-nonce'];
    const signature =
      request.hmacSignature || request.headers['x-signature'] || request.headers['x-api-signature'];
    const timestamp =
      request.hmacTimestamp || request.headers['x-timestamp'] || request.headers['x-api-timestamp'];
