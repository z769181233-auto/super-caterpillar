...
async function main() {
    ...
    if (!org) {
        const user = await prisma.user.create({ data: { email: `stress_${randomUUID().substring(0, 8)}@example.com`, passwordHash: 'dummy' } });
    ...
    if (!project) {
        project = await prisma.project.create({ data: { name: 'Stress Project', organizationId: org.id, ownerId: org.ownerId } });
    }
    ...
    if (!episode) {
        episode = await prisma.episode.create({ data: { name: 'E1', index: 1, seasonId: season.id, projectId: project.id } });
    }

    let scene = await prisma.scene.findFirst({ where: { episodeId: episode.id } });
    if (!scene) {
        scene = await prisma.scene.create({ data: { title: 'Scene 1', index: 1, episodeId: episode.id, projectId: project.id } });
    }

    let shot = await prisma.shot.findFirst({ where: { sceneId: scene.id } });
    if (!shot) {
        shot = await prisma.shot.create({ data: { title: 'Shot 1', index: 1, type: 'stress', sceneId: scene.id } });
    }
