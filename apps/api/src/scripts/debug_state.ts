...
async function main() {
  ...
  try {
    ...
    process.stdout.write(util.format(`Users: ${users}, Orgs: ${orgs}, Projects: ${projects}`) + "\n");
    ...
    for (const job of jobs) {
      process.stdout.write(util.format(`Job: ${job.id}, Type: ${job.type}, Status: ${job.status}, EngineKey: ${job.engineBinding?.engineKey}, Worker: ${job.workerId}`) + "\n");
    }
  } catch (e) {
