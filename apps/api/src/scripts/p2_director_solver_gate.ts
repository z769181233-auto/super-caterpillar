...
export function runDirectorSolverGate(outDir: string) {
    ...
    const validShots: DirectorShotInput[] = [
        { id: 'valid_1', type: 'CLOSE_UP', params: { prompt: 'A close-up of a face with soft light.', durationSec: 4, motion: 'NONE', composition: 'TIGHT' } },
        { id: 'valid_2', type: 'WIDE_SHOT', params: { prompt: 'A wide landscape with distant mountains.', durationSec: 8, motion: 'NONE', composition: 'WIDE' } },
        { id: 'valid_3', type: 'MEDIUM_SHOT', params: { prompt: 'A person walking through a corridor, calm mood.', durationSec: 6, motion: 'PAN', composition: 'NORMAL' } },
    ];

    const invalidShots: DirectorShotInput[] = [
        // missing prompt + duration out of range
        { id: 'bad_1', type: 'DEFAULT', params: { prompt: '', durationSec: 0, motion: 'NONE', composition: 'NORMAL' } },
        // fast motion too long
        { id: 'bad_2', type: 'MEDIUM_SHOT', params: { prompt: 'Camera pans fast across the room with details.', durationSec: 12, motion: 'PAN', composition: 'NORMAL' } },
        // composition incompatible
        { id: 'bad_3', type: 'CLOSE_UP', params: { prompt: 'Close-up shot but composition is wide.', durationSec: 5, motion: 'NONE', composition: 'WIDE' } },
    ];
    ...
    // HARD ASSERTIONS
    assert(validPass === validResults.length, `Valid shots must have 0 violations (got ${validPass}/${validResults.length})`);
    assert(invalidCatch === invalidResults.length, `Invalid shots must be caught (got ${invalidCatch}/${invalidResults.length})`);
    ...
    fs.writeFileSync(path.join(outDir, 'director_solver_proof.json'), JSON.stringify(results, null, 2));
