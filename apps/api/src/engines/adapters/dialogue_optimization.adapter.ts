...
export class DialogueOptimizationAdapter implements EngineAdapter {
    ...
    async invoke(input: EngineInvokeInput): Promise<EngineInvokeResult> {
        ...
        if (!dialogue) {
            return { status: 'FAILED' as any, error: { code: 'DIA_NO_TEXT', message: 'Missing dialogue input' } };
        }

        const inputHash = createHash('sha256').update(dialogue + persona).digest('hex');
        ...
        } catch (e) { this.logger.warn(`Cache read error: ${e}`); }
    ...
    private async recordCost(input: EngineInvokeInput, amount: number, extra: any = {}) {
        await this.costLedgerService.recordFromEvent({
            ...
            metadata: { type: 'dialogue_optimization', traceId: input.context.traceId || 'unknown', ...extra }
        });
    }
}
