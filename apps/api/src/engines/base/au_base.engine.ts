...
export interface AuBaseOutput {
    status: 'PASS' | 'FAIL';
    assetUrl: string; // Audio file URL
    meta: {
...
/**
 * AU Base Engine
 * 设计初衷：统一音频类引擎的 Cache (7d), Audit, Ledger 以及输出规范
 */
export abstract class AuBaseEngine {
    ...
    constructor(
        ...
        this.logger = new Logger(`${AuBaseEngine.name}[${name}]`);
    }
    ...
    protected generateCacheKey(payload: any): string {
        ...
        return `au_cache:${this.name}:v1:${hash}`;
    }
    ...
    async execute(input: EngineInvokeInput, payload: any): Promise<EngineInvokeResult> {
        ...
        try {
            ...
            const finalOutput: AuBaseOutput = {
                status: 'PASS',
                assetUrl: result.assetUrl,
                meta: {
                    source: 'generated',
                    implementation: 'au_base_v1',
                    engine: this.name,
                    cached: false,
                    ...result.meta
                },
            ...
            return {
                ...
                error: { code: 'AU_BASE_ERROR', message: e.message }
            };
        }
    }

    protected abstract processLogic(payload: any, input: EngineInvokeInput): Promise<{ assetUrl: string; meta?: any; metrics?: any }>;

    private async auditHelper(input: EngineInvokeInput, type: 'HIT' | 'MISS', resourceId: string) {
        await this.audit.log({
            action: `AU_${this.name.toUpperCase()}`,
            resourceId: resourceId,
            resourceType: 'au_result',
    ...
    private async recordCost(input: EngineInvokeInput, amount: number, extra: any = {}) {
        await this.cost.recordFromEvent({
            ...
            jobType: input.jobType || 'AU_RENDER',
            engineKey: this.name,
            costAmount: amount,
            billingUnit: 'job',
            quantity: 1,
            attempt: (input.context as any).attempt || 1,
            metadata: { type: 'au_base', traceId: input.context.traceId || 'unknown', ...extra }
        });
    }
}
