...
export class ScriptBuildService {
  ...
  async getShotSource(shotId: string, context: number = 400) {
    ...
    if (!shot || !shot.sourceRef || !shot.scriptBuild)
      throw new NotFoundException(`Shot ${shotId} or its source/build reference not found`);

    const source = shot.scriptBuild.storySource;
    const filePath = source.path;

    if (!fs.existsSync(filePath))
      throw new NotFoundException(`Source file not found at ${filePath}`);
