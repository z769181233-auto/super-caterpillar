...
async function runTests() {
  ...
  const user = await prisma.user.upsert({
    where: { id: 'sysadmin' },
    update: {},
    create: { id: 'sysadmin', email: 'sysadmin_balance@test.com', passwordHash: 'truth-secret' },
  });
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.billingLedger.create({
          data: {
            ...
            amount: costPerJob,
            idempotencyKey: `${job.id}_COMMITTED`,
          },
        });
      });
    } else {
  ...
  for (const a of finalAggs) {
    if (a.billingState === 'COMMITTED')
      sumCommittedBatch = BigInt(a._sum.amount?.toString() || '0');
  }
  ...
  if (
    ...
    cCommitted === 30 &&
    cFailed === 70 &&
    sumCommittedBatch === expectedBatchNet
  ) {
