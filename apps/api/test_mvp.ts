...
async function verifyAPI() {
  ...
  console.log(
    `[Outline] Episodes: ${outlineData.stats.episodes}, Scenes: ${outlineData.stats.scenes}`
  );
  ...
  console.log(
    `[Source] Excerpt Offset: ${sourceData.source.excerptStart} -> ${sourceData.source.excerptEnd}`
  );
  console.log(
    `[Source] Original Match Offset: ${sourceData.source.startOffset} -> ${sourceData.source.endOffset}`
  );
  console.log(
    `[Source] Text Excerpt Preview: \n"${sourceData.source.excerpt.substring(0, 200).replace(/\n/g, '\\n')}..."`
  );
