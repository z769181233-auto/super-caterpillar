...
describe('Stage 4 Flow (E2E)', () => {
    ...
    beforeAll(async () => {
        ...
        const projPerms = [
            'project.read', 'project.write', 'project.generate',
            'project.review', 'project.publish', 'project.delete'
        ];
    ...
    afterAll(async () => {
        ...
        if (userId) {
            ...
            await prisma.user.delete({ where: { id: userId } }).catch(e => console.warn('Failed to cleanup user', e));
            ...
            if (org) {
                await prisma.organization.delete({ where: { id: org.id } }).catch(e => console.warn('Failed to cleanup org', org.id));
            }
        }
        await app.close();
    });
    ...
    it('should execute Stage 4 flow successfully', async () => {
        ...
        try {
            ...
            console.log('Triggering Semantic Enhancement (this may take time due to LLM)...');
            ...
            console.log('✅ Stage 4 E2E Test Passed!');
        } finally {
            // Cleanup Project - Always delete the project to avoid pollution
            if (projectId) {
                await prisma.project.delete({ where: { id: projectId } }).catch(e => console.error('Cleanup failed', e));
            }
        }
