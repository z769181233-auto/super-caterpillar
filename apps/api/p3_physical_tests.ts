...
async function runTests() {
    ...
    const org = await prisma.organization.upsert({
        ...
        update: {},
        create: { id: '00000000-0000-0000-0000-000000000001', name: 'Test Org Physical', slug: 'test-org-phys-v2', ownerId: user.id }
    });
    const proj = await prisma.project.upsert({
        where: { id: '00000000-0000-0000-0000-000000000002' },
        update: {},
        create: { id: '00000000-0000-0000-0000-000000000002', name: 'Test Proj Physical', organization: { connect: { id: org.id } }, owner: { connect: { id: user.id } } }
    });
    ...
    const crashJobs = await Promise.all(
        Array.from({ length: 10 }).map((_, i) => prisma.shotJob.create({
            data: {
                id: randomUUID(),
                projectId: proj.id,
    ...
    log1 += `DB QUERY (Sample Job ` + crashJobs[0].id + `):\n`
        + ledgers1.map((l: any) => ` - ${l.billingState} (idempotency: ${l.idempotencyKey})`).join('\n') + `\n`;

    if (ledgers1.length === 2 && ledgers1[0].billingState === 'RESERVED' && ledgers1[1].billingState === 'RELEASED') {
    ...
    const results2 = await Promise.allSettled(Array(20).fill(0).map(() => dispatchTransaction()));
    const successCount = results2.filter(r => r.status === 'fulfilled' && r.value === 'SUCCESS_DISPATCH').length;
    const rejections = results2.filter(r => r.status === 'rejected').map(r => (r as PromiseRejectedResult).reason.message || String((r as PromiseRejectedResult).reason));
    ...
    await prisma.billingLedger.create({
        data: { projectId: proj.id, jobId: job3.id, billingState: 'RESERVED', amount: 300n, idempotencyKey: `${job3.id}_RESERVED` }
    });
    ...
    const successTx = async () => {
        return prisma.$transaction(async (tx) => {
            ...
            if (updated.count === 1) {
                await tx.billingLedger.create({
                    data: { projectId: proj.id, jobId: job3.id, billingState: 'COMMITTED', amount: 300n, idempotencyKey: `${job3.id}_COMMITTED` }
                });
    ...
    const failTx = async () => {
        return prisma.$transaction(async (tx) => {
            ...
            if (updated.count === 1) {
                await tx.billingLedger.create({
                    data: { projectId: proj.id, jobId: job3.id, billingState: 'RELEASED', amount: 0n, idempotencyKey: `${job3.id}_RELEASED` }
                });
