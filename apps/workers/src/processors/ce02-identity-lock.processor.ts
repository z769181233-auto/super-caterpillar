...
export async function processIdentityLockJob(ctx: {
    ...
    try {
        ...
        let anchor = await prisma.$transaction(async (tx) => {
            ...
            if (existingActive) {
                if (existingActive.status === 'PROCESSING') {
                    ...
                    if (ageMs < 5 * 60 * 1000) {
                        throw new Error(`Concurrent processing lock: Anchor ${existingActive.id} is PROCESSING`);
                    }
        ...
        for (const view of views) {
            logStructured('info', { action: `GENERATING_VIEW_${view.toUpperCase()}`, characterId, anchorId: currentAnchorId });
            ...
            const isValid = (metadata.width || 0) >= MIN_RESOLUTION && (metadata.height || 0) >= MIN_RESOLUTION;

            logEvidence('DECODE_ASSERT.log', `${new Date().toISOString()} [VALIDATE] ${targetRelPath} Width=${metadata.width} Format=${metadata.format} PASS=${isValid}`);

            if (!isValid) {
                throw new Error(`Validation failed for ${view}: Resolution ${metadata.width}x${metadata.height} < ${MIN_RESOLUTION}`);
            }
        ...
        logEvidence('IDENTITY_TRIVIEW_SHA256.txt', `${combinedHash}  [AGGREGATE_HASH] anchor=${currentAnchorId}`);
        ...
        const dbRow = await prisma.characterIdentityAnchor.findUnique({ where: { id: currentAnchorId } });
