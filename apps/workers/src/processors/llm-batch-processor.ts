...
export class LLMBatchProcessor {
    ...
    async processChunks(
        ...
        for (let i = 0; i < this.config.maxConcurrentRequests; i++) {
            workers.push(
                this.worker(queue, results, progress, onProgress)
            );
        }
     ...
     */
    private async processChunkWithOpenAI(
        chunk: Chunk
    ): Promise<LLMProcessedChunk> {
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: '你是一个专业的小说结构分析助手。你的任务是分析给定的文本片段，识别其中的场景和镜头结构。',
                    },
                    {
                        role: 'user',
                        content: prompt,
     ...
     */
    private async processChunkWithAnthropic(
        chunk: Chunk
    ): Promise<LLMProcessedChunk> {
        const prompt = this.buildPrompt(chunk);
