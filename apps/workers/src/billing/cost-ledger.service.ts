...
export class CostLedgerService {
  ...
  async recordCE06Billing(params: {
    ...
    try {
      ...
      console.log(
        `[CostLedger] ✅ Recorded: jobId=${jobId}, credits=${totalCredits.toFixed(4)}, tokens=${billingUsage.totalTokens}, model=${billingUsage.model}`
      );
    } catch (error: any) {
      // 幂等性：重复记录静默忽略
      if (error?.code === 'P2002') {
        console.warn(
          `[CostLedger] ⚠️  Duplicate skipped (idempotent): jobId=${jobId}, jobType=${jobType}`
        );
        return;
      }
