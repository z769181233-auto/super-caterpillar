...
export class BillingOutboxDispatcher {
  ...
  start(intervalMs: number = 10000) {
    process.stdout.write(
      util.format(`[BillingOutbox] Dispatcher started with interval ${intervalMs}ms`) + '\n'
    );
  ...
  async processPending() {
    ...
    try {
      ...
      process.stdout.write(
        util.format(`[BillingOutbox] Found ${pending.length} events to dispatch`) + '\n'
      );
      ...
      for (const record of pending) {
        try {
          ...
          process.stdout.write(
            util.format(`[BillingOutbox] ✅ Event dispatched: ${record.dedupeKey}`) + '\n'
          );
        } catch (error: any) {
          ...
          process.stderr.write(
            util.format(
              `[BillingOutbox] ❌ Dispatch failed for ${record.dedupeKey}: ${error.message}`
            ) + '\n'
          );
        }
      }
      ...
      process.stderr.write(
        util.format(`[BillingOutbox] ‼️ Dispatcher loop error: ${e.message}`) + '\n'
      );
    } finally {
      this.isProcessing = false;
    }
