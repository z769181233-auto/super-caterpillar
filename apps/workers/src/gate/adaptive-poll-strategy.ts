...
export class AdaptivePollStrategy {
  ...
  constructor(options?: { minInterval?: number; maxInterval?: number; backoffFactor?: number }) {
  ...
  public reportPollResult(foundJobs: boolean): number {
    if (foundJobs) {
      ...
      this.currentInterval = Math.min(this.currentInterval * this.backoffFactor, this.maxInterval);
    }
