...
export class ControlNetMapper {
   ...
   */
  static mapFromGraphState(
    graphState: any,
    refSheetId?: string
  ): { settings: ControlNetSettings; bindings: AssetBindings } {
    ...
    if (
      graphState.characters &&
      Array.isArray(graphState.characters) &&
      graphState.characters.length > 0
    ) {
  ...
  static validateBindings(bindings: AssetBindings): { valid: boolean; error?: string } {
    for (const key of Object.keys(bindings)) {
      ...
      if (!path.startsWith('assets/') && !path.startsWith('_dynamic/')) {
        return {
          valid: false,
          error: `Invalid (Non-Allowlisted) Path in binding [${key}]: ${path}. Must start with 'assets/' or '_dynamic/'`,
        };
      }
    }
    return { valid: true };
