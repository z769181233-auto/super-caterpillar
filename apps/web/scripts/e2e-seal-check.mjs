...
try {
  ...
  const output = execSync('grep -r -I -l "__E2E_MOCK_PASS__" . || true', {
    encoding: 'utf8',
    stdio: 'pipe',
  });
  ...
  for (const file of lines) {
    if (file.includes('.next') || file.includes('node_modules') || file.includes('tests-results'))
      continue;
    ...
    if (!isMiddleware && !isE2EScript && !isSelf) {
      console.error(
        `❌ [E2E SEAL VIOLATION] Found bypass token leak in production file: ${normalized}`
      );
      hasLeak = true;
    }
  }
  ...
  if (foundExpected < 2) {
    console.warn(
      '⚠️ [WARNING] E2E bypass token missing in either middleware or e2e scripts. The seal might drop.'
    );
  }
