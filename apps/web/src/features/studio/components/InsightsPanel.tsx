...
export function InsightsPanel({ insights, onSelectShot, auditConfig, t }: InsightsPanelProps) {
  ...
  const handleCurvePointClick = useCallback(
    (shotId: string) => {
  ...
  const characters =
    insights.topCharacters && insights.topCharacters.length > 0
      ? insights.topCharacters.map((c) => ({
          name: c.name,
          pct: Math.min(100, Math.round((c.count / (insights.topCharacters[0]?.count || 1)) * 65)),
        }))
      : [
          { name: '张若尘', pct: 65 },
          { name: '池瑶', pct: 25 },
      ...
      <div className="card studio-panel" style={{ padding: 12 }}>
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            justifyContent: 'space-between',
            marginBottom: rolesCollapsed ? 0 : 16,
          }}
        >
          <div className="panelTitle" style={{ fontSize: 11, opacity: 0.8 }}>
            {t('rolesTitle')}
          ...
          <button
            ...
            onMouseEnter={(e) => {
              ...
              (e.target as HTMLElement).style.color = 'var(--gold-primary)';
            }}
            onMouseLeave={(e) => {
              (e.target as HTMLElement).style.borderColor = 'var(--line-separator)';
