    ...
    return (
      <div key={n.id} className="treeGroup">
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div className="treeTitle" style={{ fontWeight: isEpisode ? 600 : 400 }}>
              ...
              {isShot && (
                <span style={{ opacity: 0.5, fontSize: '0.9em', marginLeft: 6 }}>
                  Shot {String(n.index).padStart(2, '0')}
                </span>
              )}
            </div>
          </div>
        ...
        {(isEpisode || isScene) && (isExpanded || forceExpand) && (
          <div className="treeChildren">{children.map((c) => renderNode(c, depth + 1))}</div>
        )}
      </div>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="panelBody scroll-thin" style={{ flex: 1, overflowY: 'auto' }}>
        ...
        />
        <div style={{ height: 16 }} />
        <div className="treeList">{tree.map((n) => renderNode(n, 0))}</div>
      </div>
    </div>
  );
}
