...
export function ReaderPanel({ shot, loading, onOpenEvidence, t }: ReaderPanelProps) {
  ...
  if (loading) {
    return (
      ...
      >
        <span className="gold-text" style={{ opacity: 0.5 }}>
          Loading...
        </span>
      </div>
    );
  ...
  return (
    <div
      className="card studio-panel"
      style={{ height: 'calc(100vh - 350px)', display: 'flex', flexDirection: 'column' }}
    >
      ...
      >
        ...
        <div className="readerText">{activeText}</div>

        {/* AI Revision Portal */}
        {shot.revisions && shot.revisions.length > 0 && (
          <div
            style={{ marginTop: 40, paddingTop: 20, borderTop: '1px dashed var(--line-separator)' }}
          >
            ...
            {selectedRevisionId && (
              ...
              >
                ...
                <div style={{ fontSize: 13, color: 'var(--gold-primary)' }}>
                  {shot.revisions.find((r) => r.id === selectedRevisionId)?.reason ||
                    'Tension balanced.'}
                </div>
              </div>
            )}
