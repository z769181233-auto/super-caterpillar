  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1rem' }}>
              ID: {project.id} · Org: {project.organizationId} · Created:{' '}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
        ...
        >
          <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            {t('statsUsage', { defaultValue: 'Usage' })}
          </div>
          <div style={{ fontSize: '1.5rem', fontWeight: 600, color: 'var(--text-primary)' }}>
            {project.stats.usage}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          >
            ...
            <Button variant="secondary" onClick={() => onSwitchTab('builds')}>
              {t('ctaViewAll', { defaultValue: 'View All' })}
            </Button>
          </div>
        </div>
      </Card>
    ...
    <Card style={{ padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <div>
          ...
          >
            {t('fingerprintStatus', { defaultValue: 'Fingerprint Status' })}
          </div>
        ...
        <div>
          <div
            style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}
          >
            {t('rulesVersion', { defaultValue: 'Rules Version' })}
          </div>
