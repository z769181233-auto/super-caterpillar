...
export function VideoPlayerPageContent() {
  ...
  const loadFirstVideo = async () => {
    try {
      ...
      const findVideo = (nodes: VideoNode[], pathInfo: VideoContext = {}): FoundVideo | null => {
        ...
        for (const node of nodes) {
          ...
          if (node.type === 'SHOT' || node.type === 'shot') {
            const url =
              node.videoUrl || node.assets?.find((asset) => asset.type === 'VIDEO')?.storageKey;
      ...
      const rootPayload: VideoNode[] = Array.isArray(structure.tree)
        ? (structure.tree as VideoNode[])
        : [];
