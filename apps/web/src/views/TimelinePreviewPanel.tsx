...
type PreviewStatus = 'IDLE' | 'STARTING' | 'PENDING' | 'RUNNING' | 'SUCCEEDED' | 'FAILED' | 'REPLAY_TEST_4004';
    ...
    return (
        <div className="p-6 bg-slate-900 text-white rounded-lg shadow-xl border border-slate-700">
            <div className="flex justify-between items-center mb-6">
                ...
                <span className={`px-3 py-1 rounded text-xs font-mono ${status === 'SUCCEEDED' ? 'bg-green-600' :
                        status === 'FAILED' ? 'bg-red-600' :
                            status === 'REPLAY_TEST_4004' ? 'bg-orange-600' :
                                'bg-blue-600'
                    }`}>
                    {status}
                </span>
