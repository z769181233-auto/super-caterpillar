      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="text-sm text-muted-foreground">
            监控 Jobs / Worker / Orchestrator，支持筛选与分页（含 429 退避）。
          </div>
        </div>
        <button className="px-3 py-2 rounded-md border hover:bg-accent" onClick={() => loadOnce()}>
          手动刷新
        </button>
      </div>
                ...
                {jobs.map((j) => {
                  return (
                    <tr key={j.id} className="border-t">
                      ...
                      <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0">{j.type}</td>
