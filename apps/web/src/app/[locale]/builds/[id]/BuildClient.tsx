...
export default function BuildClient({ initialData, buildId }: BuildClientProps) {
    ...
    const [expandedEpisodes, setExpandedEpisodes] = useState<Set<string>>(new Set([initialData?.episodes?.[0]?.id]));
    ...
    const filteredEpisodes = (data?.episodes || []).map((ep: any) => ({
        ...ep,
        scenes: (ep?.scenes || []).map((sc: any) => ({
            ...sc,
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-12)', padding: '0 var(--space-4)' }}>
            ...
            <header style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', marginTop: 'var(--space-8)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', opacity: 0.4, fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                    毛毛虫宇宙 · Creative Studio
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    <div>
                        <h1 className="text-luxury-title" style={{ marginBottom: '4px' }}>《{formatTitle(data?.build?.title)}》</h1>
                        <div className="text-luxury-status">结构化完成 · 可编辑剧本状态</div>
                    </div>
                    {/* Tooltip for Build ID (Simulated) */}
                    <div style={{ fontSize: '11px', color: 'var(--hsl-text-muted)', cursor: 'help' }} title={data?.build?.id}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-12)', padding: 'var(--space-4) 0', borderBottom: '1px solid var(--glass-border)' }}>
            ...
            <main className="workspace-grid" style={{ alignItems: 'start' }}>
                ...
                <section style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h2 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--hsl-text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>剧情大纲导航</h2>
                        <input
                            type="text"
                            placeholder="寻找特定情节..."
                            value={searchQuery}
                            ...
                            style={{
                                ...
                                color: 'var(--hsl-text-main)', padding: '4px 0', fontSize: '13px', outline: 'none', width: '200px'
                            }}
                        />
                    </div>

                    <div className="scroll-area" style={{ maxHeight: 'calc(100vh - 400px)', overflowY: 'auto', paddingRight: 'var(--space-4)' }}>
                        {filteredEpisodes.map((ep: any) => (
                            <div key={ep.id} style={{ marginBottom: 'var(--space-8)' }}>
                                ...
                                >
                                    <span style={{ fontSize: '11px', color: 'var(--gold-muted)', width: '24px' }}>{String(ep.index).padStart(2, '0')}</span>
                                    <span className="text-luxury-ep">{ep.title || `第 ${ep.index} 集`}</span>
                                    <span style={{ fontSize: '10px', color: 'var(--hsl-text-muted)', marginLeft: 'auto', opacity: 0.5 }}>
                                        {expandedEpisodes.has(ep.id) ? '收起' : '展开'}
                                    </span>
                                </div>

                                {expandedEpisodes.has(ep.id) && (
                                    <div style={{ marginTop: 'var(--space-4)', paddingLeft: '24px', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                                        {ep.scenes.map((sc: any) => (
                                            <div key={sc.id}>
                                                <div className="text-luxury-scene" style={{ marginBottom: 'var(--space-2)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                                                    <span style={{ width: '4px', height: '4px', background: 'var(--gold-muted)', borderRadius: '50%' }}></span>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                    {sc.shots.map((shot: any) => (
                                                        <div
                                                            key={shot.id}
                                                            onClick={(e) => { e.stopPropagation(); handleShotClick(shot); }}
                ...
                <section style={{ position: 'sticky', top: 'var(--space-8)' }}>
                    <div className="glass-panel" style={{ minHeight: '600px', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ padding: 'var(--space-6)', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--gold-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>分镜详情分析</span>
                        </div>

                        <div className="scroll-area" style={{ flex: 1, padding: 'var(--space-8)', display: 'flex', flexDirection: 'column', gap: 'var(--space-8)', overflowY: 'auto' }}>
                            {loadingDetails ? (
                                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flex: 1, color: 'var(--gold-muted)', fontSize: '13px' }}>
                                    正在调取文学原件...
                                </div>
                                ...
                                <>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                                        <h2 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--hsl-text-main)', lineHeight: 1.4 }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--gold-muted)', textTransform: 'uppercase' }}>文学溯源对照</div>
                                        <div className="serif-reading" style={{ padding: 'var(--space-6)', background: 'rgba(255,255,255,0.01)', borderRadius: 'var(--radius-sm)' }}>
                                            <LuxurySourcePreview
                                    ...
                                    <div style={{ marginTop: 'auto', paddingTop: 'var(--space-8)', borderTop: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'flex-end' }}>
                                        <button style={{ fontSize: '11px', color: 'var(--hsl-text-muted)', textDecoration: 'underline' }}>
                                            查看底层审计 Hash
                                        </button>
                                    </div>
                                </>
                            ) : (
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 'var(--space-4)', opacity: 0.3 }}>
                                    <div style={{ fontSize: '48px' }}>❦</div>
                                    <div style={{ fontSize: '14px', textAlign: 'center' }}>请在左侧选择剧本段落<br />开启文学创作审计</div>
                                </div>
                            )}
                        </div>
                    </div>
...
function LuxuryStat({ value, label }: { value: string | number, label: string }) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--hsl-text-main)' }}>{value}</div>
            <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--gold-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</div>
        </div>
    );
}
...
function LuxuryTag({ label, value }: { label: string, value: string }) {
    return (
        <div style={{
            ...
            background: 'var(--bg-main)', border: '1px solid var(--gold-muted)',
            borderRadius: 'var(--radius-sm)', overflow: 'hidden', fontSize: '11px'
        }}>
            <span style={{ padding: '2px 8px', background: 'var(--gold-muted)', color: 'var(--bg-main)', fontWeight: 700 }}>{label}</span>
            <span style={{ padding: '2px 8px', color: 'var(--gold-primary)', fontWeight: 600 }}>{value}</span>
        </div>
    );
}

function LuxurySourcePreview({ text, start, end, excerptStart }: { text: string; start: number; end: number; excerptStart: number }) {
