...
export default function ProjectDetailPage({ params }: { params: { projectId: string } }) {
  ...
  useEffect(() => {
    ...
    const isAnalyzing =
      ...
      sceneGraph?.statusSummary?.analysis === 'PENDING';
  ...
  const currentEpisode = currentSeason?.episodes?.find((e: any) => e.id === selectedEpisodeId);
  ...
  const isProjectEmpty = !isAnalyzing && (!sceneGraph?.tree || sceneGraph.tree.length === 0);
  ...
  const renderModuleContent = (
    module: 'overview' | 'structure' | 'pipeline' | 'assets' | 'tasks' | 'quality' | 'cost' | 'logs'
  ) => {
    switch (module) {
      case 'overview':
        return (
      ...
      case 'pipeline':
        ...
        return (
          <div className="w-full h-full bg-[hsl(var(--hsl-background))] overflow-y-auto">
            <ProductionTimeline
              statusSummary={
                sceneGraph?.statusSummary || { analysis: 'PENDING', render: 'PENDING' }
              }
        ...
        return (
          <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
            ...
            <div
              style={{
                width: '280px',
                borderRight: '1px solid hsl(var(--hsl-border))',
                backgroundColor: 'hsl(var(--hsl-background))',
                display: 'flex',
            ...
            >
              ...
              <div
                style={{
                  borderTop: '1px solid hsl(var(--hsl-border))',
                  padding: '16px',
                  maxHeight: '320px',
                  overflowY: 'auto',
                  backgroundColor: 'hsl(var(--hsl-card-bg))',
                }}
              >
