...
export default function VideoPlayerPage({ params }: { params: { id: string } }) {
    ...
    const loadFirstVideo = async () => {
        try {
            ...
            const findVideo = (nodes: any[], pathInfo: { season?: any, episode?: any, scene?: any } = {}): { url: string, shot: any, context: any } | null => {
        ...
        <div className="min-h-screen bg-black text-white flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-center p-4 relative bg-zinc-900">
                ...
                <div className="absolute top-0 left-0 right-0 p-8 bg-gradient-to-b from-black/80 to-transparent z-10 pointer-events-none flex justify-between items-start">
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-2 drop-shadow-lg">{project?.projectName || 'Project Video'}</h1>
                        <div className="flex gap-4 text-gray-300 text-sm font-mono tracking-wide">
                            {videoShot?.episodeIndex && <span className="bg-white/10 px-3 py-1 rounded border border-white/10">EP {videoShot.episodeIndex}</span>}
                            {videoShot?.sceneIndex && <span className="bg-white/10 px-3 py-1 rounded border border-white/10">SCENE {videoShot.sceneIndex}</span>}
                            {videoShot?.index && <span className="bg-white/10 px-3 py-1 rounded border border-white/10">SHOT {videoShot.index}</span>}
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="text-red-400 bg-red-900/20 p-8 rounded-xl border border-red-900/50 flex flex-col items-center">
                        ...
                        {error}
                        <div className="mt-6">
                            <Button onClick={() => router.push(`/projects/${params.id}`)} variant="primary">Back to Project</Button>
                        </div>
                    </div>
                ) : videoUrl ? (
