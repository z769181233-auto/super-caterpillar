...
type Node = {
    ...
    lastJob?: { id: string; status?: string; type?: string; engineKey?: string; createdAt?: string } | null;
    children?: Node[];
};
    ...
    const cls =
        s === 'PASS' ? 'bg-green-100 text-green-800 border-green-200' :
            s === 'WARN' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                s === 'FAIL' ? 'bg-red-100 text-red-800 border-red-200' :
                    s === 'PENDING' ? 'bg-gray-100 text-gray-600 border-gray-200' :
                        s === 'FORCED_PASS' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                            s === 'SKIPPED' ? 'bg-purple-100 text-purple-800 border-purple-200' :
...
export default function PipelinePage() {
    ...
    async function refresh() {
        ...
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const msg = e?.name === 'AbortError' ? '请求超时（已自动取消），请稍后重试' : (e?.message ? String(e.message) : '加载失败');
            setErr(msg);
        } finally {
            setLoading(false);
        }
    ...
    async function act(kind: 'retry' | 'skip' | 'force', nodeId: string) {
        ...
        try {
            ...
            if (kind === 'retry') {
                actionPromise = withTimeout(15000, (signal) => pipelineApi.retryNode(projectId, nodeId, reason || undefined, signal));
            } else if (kind === 'skip') {
                actionPromise = withTimeout(15000, (signal) => pipelineApi.skipNode(projectId, nodeId, reason.trim(), signal));
            } else if (kind === 'force') {
                actionPromise = withTimeout(15000, (signal) => pipelineApi.forcePassNode(projectId, nodeId, reason.trim(), signal));
            } else {
            ...
            const msg = err?.name === 'AbortError' ? '请求超时（已自动取消），请稍后重试' : (err?.message ? String(err.message) : '操作失败');
            setErr(msg);
        } finally {
            setBusyNodeId('');
        }
            ...
            <div className="flex items-end justify-between gap-4 flex-wrap">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        onClick={() => router.push(`/${locale}/tasks?projectId=${encodeURIComponent(projectId)}`)}
                    >
                                ...
                                {rows.map((n) => (
                                    <tr key={n.nodeId} className="border-t">
                                        ...
                                        <td className="p-3 text-xs text-red-600">
                                            {n.blockingReason ? n.blockingReason : <span className="text-muted-foreground">-</span>}
                                        </td>
                                            ...
                                            {n.lastJob ? (
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="text-muted-foreground">
                                                        {n.lastJob.status || '-'} / {n.lastJob.type || '-'} / {n.lastJob.engineKey || '-'}
                                                    </div>
                                                </div>
                                            ) : (
