    ...
    <div className="w-full max-w-5xl mx-auto py-12 px-4">
      <div className="relative flex justify-between items-center">
        ...
        {STEPS.map((step, index) => {
          ...
          if (isCompleted)
            stateClass =
              'bg-green-600 border-green-600 text-white shadow-[0_0_15px_rgba(22,163,74,0.5)]';
          if (isRunning)
            stateClass =
              'bg-blue-600 border-blue-600 text-white animate-pulse shadow-[0_0_15px_rgba(37,99,235,0.5)]';
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-4 relative group">
              ...
              <span
                className={`text-sm font-medium whitespace-nowrap absolute -bottom-8 ${isCompleted || isRunning ? 'text-white' : 'text-gray-500'} ${isDisabled ? 'text-gray-700' : ''}`}
              >
                {step.label}
              </span>
      ...
      <div className="mt-16 text-center border-t border-gray-800 pt-8">
        <h2 className="text-xl font-bold text-white mb-2">
          {runningStepIndex !== -1
            ? STEPS[runningStepIndex].label + ' 进行中'
            : completedStepIndex >= 3
              ? '生产阶段已完成'
