...
export const MainActionDispatcher: React.FC<MainActionDispatcherProps> = ({
  ...
  if (isAnalyzing || statusSummary?.analysis === 'ANALYZING') {
    return (
      <button
        disabled
        className="px-6 py-2 bg-gray-800 text-gray-400 rounded cursor-not-allowed flex items-center gap-2 font-medium"
      >
  ...
  if ((project as any).isVideoGenerating) {
    return (
      <button
        disabled
        className="px-6 py-2 bg-gray-800 text-yellow-500 rounded cursor-not-allowed flex items-center gap-2 font-medium"
      >
        <span className="animate-spin">⏳</span>
        视频生成中...
      </button>
