            ...
            <div className="flex justify-between items-start">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${statusColors[header.status]}`}>
                            {header.status}
                        </span>
                    </div>
                ...
                <div className="mt-4 bg-red-50 border border-red-200 rounded-md p-3 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm text-red-700 mt-1">
                            Currently blocked by: <strong>{header.stage.currentLabel}</strong>.
                            Reason: {header.blocking.reasonDetail || 'Unknown Issue'}
                        </div>
                    </div>
                </div>
            )}
...
function RiskBadge({ label, status }: { label: string, status: 'OK' | 'WARN' | 'FAIL' }) {
    ...
    return (
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium ${colors[status]}`}>
