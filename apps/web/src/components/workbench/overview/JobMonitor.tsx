    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-full flex flex-col relative">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Job Monitor</h3>
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <div className={`w-3 h-3 bg-white rounded-full transition-transform ${showDebug ? 'translate-x-4' : 'translate-x-0'}`} />
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-4 overflow-y-auto">
                ...
                {/* Failed */}
                {failed.map(job => (
                    <div key={job.id} className="bg-red-50 rounded-lg p-3 border border-red-100 flex items-center justify-between text-sm text-red-600">
                        <div className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-red-500"></span>
                            <span>Failed: {job.type}</span>
                        </div>
                        <button className="text-xs bg-white border border-red-200 px-2 py-1 rounded hover:bg-red-50">Retry</button>
                    </div>
                ))}
            </div>
        </div>
