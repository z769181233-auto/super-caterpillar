    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-full">
            ...
            <div className="mt-6 pt-4 border-t border-gray-100 text-xs text-gray-500 flex justify-between">
                ...
                <a href={stats.links.structureView} className="text-blue-600 hover:underline">View Tree →</a>
            </div>
        </div>
    );
}
...
function StatItem({ label, value, highlight }: { label: string, value: number, highlight?: boolean }) {
    return (
        <div className={`p-3 rounded-lg ${highlight ? 'bg-orange-50 border border-orange-100' : 'bg-gray-50'}`}>
