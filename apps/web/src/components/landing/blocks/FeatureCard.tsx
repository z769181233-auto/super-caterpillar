...
export function FeatureCard({ title, desc, icon, className = '', children, link }: FeatureCardProps) {
    const CardContent = (
        <div className={`
    ...
    `}>
            ...
            <p className="text-normal text-muted-foreground leading-relaxed line-clamp-3">
                {desc}
            </p>
            {children}
        </div>
    ...
    if (link) {
        return <Link href={`/projects?module=${link}`} className="block">{CardContent}</Link>;
    }
