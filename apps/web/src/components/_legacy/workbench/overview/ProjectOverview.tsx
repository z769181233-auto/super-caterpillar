...
export function ProjectOverview({ project, analysisStatus }: ProjectOverviewProps) {
    ...
    const handleAction = async (key: string, href?: string) => {
        ...
        if (key === 'RUN' || key === 'RETRY') {
            ...
            const currentStepNode = data?.flow.nodes.find(n => n.status === 'RUNNING') || data?.flow.nodes.find(n => n.status === 'PENDING');
            const currentStepId = currentStepNode?.key;

            if (href === 'api:analyze' || (!href && (currentStepId === 'STRUCTURE_ANALYSIS' || currentStepId === 'NOVEL_IMPORT'))) {
                try {
    ...
    if (!data) return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">Loading Workspace...</div>;
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="max-w-[1600px] mx-auto px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    */}
                    <StatCard title="Total Episodes" value={(project as any).episodes?.length || 0} icon="📺" />
                    <StatCard title="Total Scenes" value={(project as any).counts?.scenes || 'N/A'} icon="🎬" />
                ...
                {(project as any).hasVideo && (
                    <div className="mt-8 p-6 bg-gradient-to-r from-gray-900 to-black border border-gray-800 rounded-xl flex justify-between items-center">
                        ...
                        <a href={`/projects/${project.projectId}/video`} className="px-6 py-3 bg-green-600 hover:bg-green-500 text-white rounded-lg font-bold shadow-lg transition-all">
                            ▶ 看片 (Watch)
                        </a>
                    </div>
                )}
                    ...
                    <div className="col-span-8 flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Quality Gates</h3>
                            ...
                            {/* Cost Summary */}
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Cost Control</h3>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <a href={data.audit.href} className="text-xs text-blue-600 hover:underline">View All</a>
                            </div>
                            <div className="space-y-3">
                                {data.audit.recent.map((log, i) => (
                                    <div key={i} className="flex justify-between text-sm border-b last:border-0 border-gray-50 pb-2 last:pb-0">
                                        <div>
...
function StatCard({ title, value, icon, highlight = false }: { title: string, value: string | number, icon: string, highlight?: boolean }) {
    return (
        <div className={`p-6 rounded-xl shadow-sm border transition-all ${highlight ? 'bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200' : 'bg-white border-gray-100'}`}>
