      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-[hsl(var(--hsl-text-primary))] flex items-center gap-3">
            {projectName}
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium border ${analysisStatus === 'DONE' ? 'bg-green-500/10 text-green-400 border-green-500/30' :
              analysisStatus === 'ANALYZING' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' :
                analysisStatus === 'FAILED' ? 'bg-red-500/10 text-red-400 border-red-500/30' :
          ...
          <div className="mt-1 flex items-center gap-4 text-xs text-[hsl(var(--hsl-text-muted))]">
            {jobId && <span className="font-mono opacity-70">{t('job', { id: jobId.substring(0, 8) })}</span>}
            {analysisUpdatedAt && (
              <span>
                {t('updated', { time: new Date(analysisUpdatedAt).toLocaleString() })}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-sm font-semibold transition-all flex items-center gap-2 border ${isAnalyzeDisabled || isAnalyzing
          ...
          <button
            ...
            className={`rounded-lg border px-4 py-1.5 text-sm font-semibold transition-all flex items-center gap-2 ${isGenerateDisabled
              ? 'bg-gray-800/50 text-gray-500 border-gray-700 cursor-not-allowed'
        ...
        <div>
          {isAnalyzing && (
            <span className="flex items-center gap-2 text-blue-400">
              ...
              {t('status')}
              {jobId && <Link href="/studio/jobs" className="underline hover:text-blue-300 ml-2">{t('viewJob')}</Link>}
            </span>
          )}
        </div>
        <Link href="/studio/jobs" className="hover:text-[hsl(var(--hsl-primary-glow))] transition-colors border-b border-transparent hover:border-[hsl(var(--hsl-primary-glow))]">
          {t('viewAllJobs')} &rarr;
        </Link>
      </div>
    </div>
