#!/usr/bin/env bash
# ==============================================================================
# [GATE] P1-R1: CE02 Mother -> ce09_security_check (L0)
...
EVID_DIR="${EVID_ROOT}/p1_r1_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P1-R1: ce09_security_check (L0) START ---"
...
TRACE_ID="gate-P1-R1-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P1-R1-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "ce09_security_check",
  "payload": {"content": "敏感词测试"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
