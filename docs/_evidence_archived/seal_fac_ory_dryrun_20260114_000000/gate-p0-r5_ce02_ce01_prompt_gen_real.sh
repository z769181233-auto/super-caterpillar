#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R5: CE02 Mother -> ce01_prompt_gen (L0)
...
EVID_DIR="${EVID_ROOT}/p0_r5_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R5: ce01_prompt_gen (L0) START ---"
...
TRACE_ID="gate-P0-R5-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R5-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "ce01_prompt_gen",
  "payload": {"rawText": "测试文本"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
