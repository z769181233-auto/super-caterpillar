#!/usr/bin/env bash
# ==============================================================================
# [GATE] P0-R6: CE02 Mother -> audio_gen (L1)
...
EVID_DIR="${EVID_ROOT}/p0_r6_$(date +"%Y%m%d_%H%M%S")"
mkdir -p "${EVID_DIR}"
...
# ========== MAIN ==========
echo "--- [GATE] P0-R6: audio_gen (L1) START ---"
...
TRACE_ID="gate-P0-R6-$(date +"%Y%m%d_%H%M%S")"
JOB_ID="job-P0-R6-$(date +"%Y%m%d_%H%M%S")"

cat > "${EVID_DIR}/REQ.json" <<JSON
{
  "engineKey": "audio_gen",
  "payload": {"text": "你好，世界", "voice": "zh-CN-Standard-A"},
  "metadata": {
    "isVerification": true,
    "traceId": "${TRACE_ID}",
    "jobId": "${JOB_ID}",
