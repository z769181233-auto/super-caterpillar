...
- system*settings、billing*\* 等仍标记为待处理/UNKNOWN，未在本轮触及。
...
- schema 未齐项：system*settings / billing*\* / models 命名差异、audit_log legacy 表合并等仍属后续批次（DROP/TIGHTEN/UNKNOWN）。
...
2. **权限数据初始化**：准备最小角色/权限种子或管理接口，确保 AUTH/PROJECT\_\* 权限可配置。
