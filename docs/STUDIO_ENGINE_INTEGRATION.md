...
```tsx
<Link href={`/tasks/${taskId}/graph?${searchParams.toString()}`}>查看 Task Graph</Link>
```
...
<div>
  ...
  {confidence !== null && (
    <div style={{ fontSize: '0.75rem', color: '#666' }}>置信度: {confidence.toFixed(2)}</div>
  )}
</div>
```
...
<span
  style={{
    color: successRate >= 0.9 ? '#4CAF50' : successRate >= 0.7 ? '#FF9800' : '#F44336',
  }}
>
