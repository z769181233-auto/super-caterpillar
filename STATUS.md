...
## 当前风险
- 本轮只改 Studio v2 ShotScript 文本质量门槛，未扩展图片/视频生成或 worker 执行链路。
...
## 验证状态
- `pnpm --filter api exec jest src/project/project-studio-text-pipeline.acceptance.spec.ts src/project/project-studio-shot-script.service.spec.ts --runInBand` 通过，10 tests。
- `pnpm --filter api exec jest src/project/project-studio-episode-plan.service.spec.ts src/project/project-studio-director-script.service.spec.ts src/project/project-studio-shot-script.service.spec.ts --runInBand` 通过，22 tests。
...
## 原因
小说分析质量主线可以继续做 M12，但仍不进入图片、视频或大范围 Studio 新功能。
