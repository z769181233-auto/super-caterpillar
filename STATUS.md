...
## 当前任务
Common 导航文案 i18n display-only 切片。
...
- Common 导航文案 i18n display-only 切片已完成：补齐三语言 `Common.login` 与 `Common.enterWorkbench`。
...
## 当前风险
- 不能把本轮 i18n 文案扩展成 auth、项目创建/删除或导航逻辑改动。
- 不能恢复会启动 API/Web/Worker 或写库的 smoke 脚本。
- `stash@{0}` 仍包含多业务线历史改动，不能 `git stash pop/apply` 整包恢复。

## 已知问题
- 当前切片只修复导航栏 key 展示问题；不修复小说分析质量，不生成 StoryBible、CharacterBible、StoryboardAsset 或视频资产。

## 验证状态
- `node - <<'NODE' ... JSON.parse + Common key assertion ... NODE`: pass
- `pnpm --filter web exec tsc -p tsconfig.json --noEmit`: pass
- `pnpm --filter web exec eslint src/components/UserNav.tsx src/features/studio/components/StudioShell.tsx`: pass
