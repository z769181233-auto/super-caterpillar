...
返回格式为 JSON：
{
  ...
  "reason": "在这个场景中，角色表现出了..."
            ...
            console.warn(`[CE08] AI Character arc analysis failed, falling back to rule logic: ${error.message}`);
        }
    }

    // Fallback Rule-based logic
    const traits: CharacterTrait[] = [];
    if (text.includes('想') || text.includes('意识到')) traits.push({ name: 'internal_growth', value: 0.5, description: '初步自我觉醒' });
