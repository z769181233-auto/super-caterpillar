...
function generateFilterString(vfx: VFXPreset, intensity: number): string {
    const i = intensity;
    switch (vfx) {
        case 'grain': return `noise=alls=${Math.floor(i * 30)}:allf=t+u`;
        case 'vignette': return `vignette=PI/${Math.max(2, 6 - i * 4)}`;
        case 'sepia': return 'colorchannelmixer=.393:.769:.189:0:.349:.686:.168:0:.272:.534:.131';
        case 'scanlines': return 'drawbox=y=ih/2:w=iw:h=1:color=black@0.5:t=fill';
        case 'bloom': return 'unsharp=7:7:0.8:7:7:0.5';
        case 'glitch': return 'frei0r=filter_name=glitch0r'; // Assuming frei0r is available
        case 'dreamy': return 'gblur=sigma=2,unsharp=5:5:1.0:5:5:0.0';
        default: return 'null';
    }
}
