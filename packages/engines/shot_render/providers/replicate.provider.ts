...
async function httpRequest(url: string, options: https.RequestOptions, body?: string): Promise<string> {
            ...
            res.on('end', () => {
                if (res.statusCode && res.statusCode >= 400) {
                    ...
                    if (res.statusCode === 401) {
                        reject(new Error(`[AUTH_ERROR] HTTP 401: Invalid or missing Replicate token. Details: ${safeData}`));
                    } else {
...
async function pollPrediction(id: string, token: string, maxRetries = 60): Promise<ReplicatePrediction> {
...
export async function renderWithReplicate(
    ...
    const createResponse = await httpRequest(REPLICATE_API_URL, {
        method: 'POST',
        headers: {
            'Authorization': `Token ${token}`,
...
export const replicateProvider = {
    ...
    async render(prompt: string, options?: {
        width?: number;
        height?: number;
        seed?: number;
        negativePrompt?: string;
