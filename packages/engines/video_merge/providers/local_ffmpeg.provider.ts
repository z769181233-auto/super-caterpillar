...
export const localFfmpegProvider = {
    ...
    async merge(
        input: { framePattern?: string; framePaths?: string[]; fps: number; width: number; height: number },
        ctx: { jobId?: string } = {}
    ): Promise<VideoResult> {
        const outDir = process.env.ASSET_STORAGE_DIR || path.join(process.cwd(), "apps/workers/.runtime/assets_gate_p0r1");
        ...
        if (input.framePattern) {
            ...
            const concatContent = input.framePaths.map(p => `file '${p}'\nduration ${1 / input.fps}`).join('\n');
