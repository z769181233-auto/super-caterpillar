...
export class CircuitBreaker {
  ...
  private onSuccess(): void {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      this.logState(
        'SUCCESS',
        `HALF_OPEN successes: ${this.successes}/${this.config.successThreshold}`
      );
  ...
  private open(): void {
    ...
    this.logState(
      'OPEN',
      `Circuit opened. Next attempt at ${new Date(this.nextAttempt).toISOString()}`
    );
  }
  ...
  private withTimeout<T>(promise: Promise<T>, timeout: number): Promise<T> {
    return Promise.race([
      ...
      new Promise<T>((_, reject) => {
        setTimeout(() => {
          const error = new Error(`[TIMEOUT] ${this.config.name}: Request exceeded ${timeout}ms`);
