 ...
 */
export async function sceneCompositionRealEngine(
...
返回格式为 JSON：
{
  ...
  "reason": "将主体放在黄金分割点..."
      ...
      console.warn(
        `[SceneComposition] AI composition failed, falling back to rule logic: ${error.message}`
      );
    }
  }

  // Fallback Rule-based logic (Uniform Centering)
  const outputElements = elements.map(
    (e, i) =>
      ({
        ...e,
        x: 50 + i * 10,
